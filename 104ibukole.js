...
function init(){
    ...
    db.ref('draw_status').on('value', s=>{ 
        const d = s.val() || {}; 
        currentDate = d.date; 
        miniGameStatus = d.status; // <-- SE GUARDA EL ESTADO GLOBAL
        if(d.status==='active') { 
...
if(document.getElementById('start-purchase-btn')) document.getElementById('start-purchase-btn').onclick=()=>{
    if(miniGameStatus === 'paused') return alert("\u23F8\uFE0F La venta est\u00E1 pausada temporalmente."); // <-- BLOQUEO AQUÍ
...
window.confirmCurrentCard=async()=>{ 
    if(miniGameStatus === 'paused') return alert("\u23F8\uFE0F La venta ha sido pausada. No se pudo procesar la compra."); // <-- BLOQUEO DE SEGURIDAD AQUÍ
