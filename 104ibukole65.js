/* ============================================================
   ...
   Versión: Final - Promo Blindada + Torneo Express
   ============================================================ */
...
// Variables Torneo Express (Fase 4)
let torneoConfig = { price: 1000, basePrize: 10000, status: 'closed' };
        ...
        if(s.exists()) {
            ...
            if(buyBtn) {
                if(torneoConfig.status === 'open') {
                    ...
                    buyBtn.textContent = "VENTA CERRADA";
                }
            }
        }
    });
...
function init(){
    ...
    setInterval(() => {
        ...
        if(now > target && now.getHours() < 18) {
            if(display) display.textContent = "\u00A1EN JUEGO!";
            if(buyBtn && torneoConfig.status === 'open') {
                buyBtn.disabled = true;
                buyBtn.classList.add('opacity-50', 'cursor-not-allowed', 'bg-gray-400');
                buyBtn.classList.remove('bg-yellow-400', 'hover:bg-yellow-500');
                buyBtn.textContent = "CERRADO HASTA LAS 6PM";
            }
        } else {
            ...
            if(buyBtn && torneoConfig.status === 'open' && (now < target || now.getHours() >= 18)) {
                buyBtn.disabled = false;
                buyBtn.classList.remove('opacity-50', 'cursor-not-allowed', 'bg-gray-400');
                buyBtn.classList.add('bg-yellow-400', 'hover:bg-yellow-500');
                buyBtn.innerHTML = `COMPRAR TICKET - <span>${torneoConfig.price}</span> Bs`;
            }
        }
    }, 1000);
...
function listenTorneoPot() {
    ...
    db.ref(`bets_torneo_express/${currentDateStr}`).on('value', s => {
        ...
        const totalCollected = torneoSoldTickets * (torneoConfig.price || 1000);
        const potValue = totalCollected > (torneoConfig.basePrize || 10000) ? totalCollected : (torneoConfig.basePrize || 10000);
...
window.confirmTorneoPurchase = async () => {
    ...
    try {
        ...
        await db.ref(`bets_torneo_express/${currentDateStr}/${uid}`).push({
            id: cardId,
            numbers: numbers,
        ...
        db.ref(`bets_torneo_express/${currentDateStr}`).on('value', s => {
            ...
            db.ref('users').once('value', usersSnap => {
                ...
                s.forEach(userNode => {
                    ...
                    const userName = (usersData[uid] && usersData[uid].name) ? usersData[uid].name.split(' ')[0] : 'An\u00F3nimo';
