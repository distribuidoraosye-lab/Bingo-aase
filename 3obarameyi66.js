/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V9.2 - LÓGICA BLINDADA)
   Búsqueda por Ventana de Tiempo Matemática (Anti-fallos de fecha)
   ============================================================ */
...
// 2. Herramientas Matemáticas (Anti-Fallos de Fecha)
...
// Calcula los milisegundos exactos de inicio y fin del día activo
function getActiveWindow(offsetHours) {
    const now = Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0);
    const d = new Date(now);
    const cutoffToday = new Date(d.getFullYear(), d.getMonth(), d.getDate(), offsetHours, 0, 0, 0).getTime();
    
    let start, end;
    if (now >= cutoffToday) {
        start = cutoffToday;
        end = cutoffToday + 24 * 3600000;
    } else {
        start = cutoffToday - 24 * 3600000;
        end = cutoffToday;
    }
    return { start, end };
}

// Descifra la hora exacta del ticket creado por Firebase
function getTimestampFromPushKey(pushKey) {
    try {
        const PUSH_CHARS = '-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz';
        let timestamp = 0;
        for (let i = 0; i < 8; i++) { timestamp = timestamp * 64 + PUSH_CHARS.indexOf(pushKey.charAt(i)); }
        return timestamp;
    } catch(e) { return 0; }
}
...
// 3. Monitor de Racha Blindado
async function initStreakMonitor(uid) {
    
    // BINGO: Ventana de 8 PM a 8 PM
    dbRacha.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        let countB = 0;
        const { start, end } = getActiveWindow(20);
        
        if (s.exists()) {
            s.forEach(c => {
                if (c.val().payment_method === 'GRATIS') return; 
                const ts = getTimestampFromPushKey(c.key);
                // Si la huella de tiempo cae en nuestra ventana matemática, LO CUENTA.
                if (ts >= start && ts < end) countB++;
            });
        }
    ...
    // TORNEO: Ventana de 6 PM a 6 PM
    setInterval(() => checkTorneo(uid), 2000);
    checkTorneo(uid);
}

function checkTorneo(uid) {
    const { start, end } = getActiveWindow(18);
    
    // Buscamos en 3 fechas distintas para asegurarnos de no perder el ticket
    ...
    const foldersToCheck = [getCustomDateStr(18), formatD(yest), formatD(tom)];
    let countT = 0;

    let checks = foldersToCheck.map(folder => {
        return dbRacha.ref(`bets_torneo_express/${folder}/${uid}`).once('value').then(s => {
            if (s.exists()) {
                s.forEach(c => {
                    const ts = c.val().timestamp || getTimestampFromPushKey(c.key);
                    // Si el ticket entra en el rango matemático, LO CUENTA.
                    if (ts >= start && ts < end) countT++;
                });
            }
        });
    });
...
// 4. Lógica de UI (Actualización Visual)
