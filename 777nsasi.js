...
let currentVipResults = { lotto: {}, granjita: {}, selva: {} }; // EL ESCANER EN VIVO
let serverOffset = 0;
...
function initVIPSystem() {
    ...
    // Escuchar data del usuario
    db.ref(`users/${uid}`).on('value', s => {
        if (s.exists()) {
            const data = s.val();
            vipUserData.balance_general = data.balance || 0; // Bolivares reales
            vipUserData.saldo_vip = data.saldo_vip || 0;     // Dolares Boveda
...
function listenToTodayTicket() {
    ...
    // Escuchar el ticket publicado
    db.ref(`vip_tickets/${vipConfig.fecha_activa}`).on('value', s => {
        currentVipTicket = s.exists() ? s.val() : null;
        
        // EL ESCANER: Escuchar resultados en vivo marcados por el Admin
...
function renderVipTicketUI() {
    ...
    const buildCol = (title, imgPath, dataArray, lotteryKey) => {
        ...
        dataArray.forEach((idAnimal) => {
            ...
            // LA LECTURA DEL MARCADOR EN VIVO
...
window.comprarTicketVIP = async (montoUSD) => {
    ...
    if (!currentVipTicket) return alert("\u26A0\uFE0F El ticket de hoy aun no esta disponible.");
