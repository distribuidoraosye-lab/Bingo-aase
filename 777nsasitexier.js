/* ============================================================
   BINGO TEXIER - MODULO VIP BLACK (vip7rayos.js / 7rayostexier.js)
   Descripcion: Sistema de Tickets Premium, Doble Saldo, Retiro Fraccionado (25%) y Racha Visual.
   ============================================================ */

let vipConfig = { tasa: 0, fecha_activa: null };
let vipUserData = { saldo_vip: 0, balance_general: 0, last_transfer: 0, streak: 0, ticket_hoy: 0, last_vip_date: null };
let currentVipTicket = null;
let currentVipResults = { lotto: {}, granjita: {}, selva: {} }; 
let serverOffset = 0;
...
function injectVipHTML() {
    ...
    <div class="flex justify-between items-center mb-4">
        <button onclick="location.reload()" class="text-xs text-gray-400 hover:text-white transition"><i class="fas fa-arrow-left"></i> Volver al Menu</button>
    </div>
        ...
        <div class="p-4 flex-1">
            ...
            <div class="bg-gradient-to-b from-gray-800 to-black rounded-xl p-4 border border-gray-700 shadow-inner mb-5">
                <div class="flex justify-between items-center border-b border-gray-700 pb-3 mb-3">
                    <div>
                        <span class="text-[10px] text-gray-500 uppercase tracking-widest block mb-1">Tu Boveda VIP</span>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <div id="vip-streak-container" class="flex gap-2"></div>
                </div>
            </div>

            <div class="bg-gray-800 rounded-xl p-4 border border-gray-700 relative shadow-lg mb-5">
                <div class="flex justify-between items-center mb-4 border-b border-gray-700 pb-2">
                    <h4 class="font-bold text-yellow-500 text-sm tracking-wide"><i class="fas fa-ticket-alt mr-1"></i> TICKET DEL DIA</h4>
                ...
                <div id="vip-ticket-grid" class="grid grid-cols-3 gap-2"></div>
            </div>

            <div id="vip-time-locked" class="hidden text-center bg-gray-800 p-4 rounded-xl border border-gray-600 mb-5">
                <i class="fas fa-clock text-3xl text-red-500 mb-2"></i>
                <h4 class="text-white font-bold tracking-widest uppercase">VENTA CERRADA</h4>
                <p class="text-gray-400 text-xs mt-1">El horario para sellar el ticket VIP finalizo a las 7:50 AM.</p>
            </div>

            <div id="vip-preparing-next" class="hidden text-center bg-gray-800 p-4 rounded-xl border border-gray-600 mb-5">
                <i class="fas fa-cog fa-spin text-3xl text-yellow-500 mb-2"></i>
                <h4 class="text-white font-bold tracking-widest uppercase">NUEVO TICKET EN PREPARACION</h4>
                <p class="text-gray-400 text-xs mt-1">El sistema esta calculando los datos fijos de manana. Atento a la proxima publicacion.</p>
            </div>

            <div id="vip-buy-section" class="space-y-4 mb-5 hidden">
                ...
                <button onclick="comprarTicketVIP(5)" class="w-full relative overflow-hidden bg-gradient-to-r from-yellow-600 via-yellow-500 to-yellow-600 hover:from-yellow-500 hover:to-yellow-400 text-black font-black py-4 rounded-xl border-2 border-yellow-300 shadow-[0_0_15px_rgba(234,179,8,0.4)] transition transform active:scale-95 flex justify-between items-center px-6">
                    ...
                    <div class="absolute top-0 right-0 bg-red-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg uppercase"><i class="fas fa-fire mr-1"></i> Favorita</div>
                </button>
    ...
    <div id="modal-traslado" class="hidden fixed inset-0 bg-black/90 z- flex items-center justify-center p-4 backdrop-blur-md">
        <div class="bg-gray-900 border border-blue-500 rounded-xl w-full max-w-sm p-6 shadow-2xl relative">
            ...
            <div class="text-center mb-6">
                <div class="inline-block p-3 bg-blue-900/50 rounded-full border border-blue-500 mb-3 text-blue-400 text-3xl"><i class="fas fa-random"></i></div>
            ...
            <div class="bg-black rounded border border-gray-700 p-4 mb-6">
                <div class="flex justify-between text-sm mb-2"><span class="text-gray-500">Tu Saldo VIP:</span><span class="text-green-400 font-bold" id="modal-saldo-vip">$0.00</span></div>
                <div class="flex justify-between text-sm"><span class="text-gray-300 font-bold">Monto a pasar hoy (25%):</span><span class="text-white font-black text-lg" id="modal-monto-traslado">$0.00</span></div>
                ...
                <p id="modal-time-warning" class="text-red-400 text-[10px] text-center mt-3 hidden">Aun no han pasado 24 horas desde tu ultimo traslado.</p>
            </div>
            <button id="btn-procesar-traslado" onclick="procesarTraslado()" class="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-3 rounded shadow transition uppercase tracking-widest">PASAR AL SALDO GENERAL</button>
        </div>
    </div>
    ...
    <div id="modal-exito-traslado" class="hidden fixed inset-0 bg-black/90 z- flex items-center justify-center p-4 backdrop-blur-md">
        <div class="bg-gray-900 border border-green-500 rounded-xl w-full max-w-sm p-6 shadow-2xl text-center transform transition-all scale-100">
            <div class="inline-block p-4 bg-green-900/50 rounded-full border border-green-500 mb-4 text-green-400 text-4xl animate-bounce"><i class="fas fa-check-circle"></i></div>
            <h3 class="text-white font-black text-2xl tracking-widest uppercase mb-2">TRASLADO EXITOSO</h3>
            <p class="text-gray-300 text-sm mb-6">El dinero ya esta disponible en tu Saldo General (Bs).</p>
            <button onclick="irABingo()" class="w-full bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 text-black font-black py-4 rounded-xl shadow-[0_0_15px_rgba(234,179,8,0.4)] transition uppercase tracking-widest text-sm"><i class="fas fa-dice text-lg mr-2"></i> IR A COMPRAR CARTONES</button>
            <button onclick="document.getElementById('modal-exito-traslado').classList.add('hidden')" class="mt-4 text-gray-500 text-xs hover:text-white transition uppercase">Cerrar y volver al VIP</button>
        </div>
    </div>
    `;
...
function initVIPSystem() {
    ...
    db.ref(`users/${uid}`).on('value', s => {
        if (s.exists()) {
            ...
            vipUserData.last_vip_date = data.last_vip_date || null;
            actualizarUIUsuario();
        }
    });
}
...
function listenToTodayTicket() {
    ...
    db.ref(`vip_tickets/${vipConfig.fecha_activa}`).on('value', s => {
        ...
        db.ref(`vip_results/${vipConfig.fecha_activa}`).on('value', res => {
            currentVipResults = res.exists() ? res.val() : { lotto: {}, granjita: {}, selva: {} };
            renderVipTicketUI();
        });
    });
}
...
function actualizarUIUsuario() {
    ...
    const timeLockedMsg = document.getElementById('vip-time-locked');
    const alreadyPlayedMsg = document.getElementById('vip-preparing-next'); // Actualizado el ID

    let isLockedTime = false;
    if(vipConfig.fecha_activa) {
        const limitTime = new Date(`${vipConfig.fecha_activa}T07:50:00`).getTime();
        if (getServerTime() > limitTime) {
            isLockedTime = true;
        }
    }

    const alreadyPlayedToday = (vipUserData.last_vip_date === vipConfig.fecha_activa && vipUserData.ticket_hoy === 0);

    buySection.classList.add('hidden');
    timeLockedMsg.classList.add('hidden');
    alreadyPlayedMsg.classList.add('hidden');

    if(alreadyPlayedToday) {
        candado.style.display = 'none';
        alreadyPlayedMsg.classList.remove('hidden');
        statusBadge.textContent = "NUEVO TICKET EN PREPARACION";
        statusBadge.className = "text-[10px] font-black uppercase px-2 py-1 rounded bg-yellow-900 text-yellow-500 border border-yellow-600";
    } else if(vipUserData.ticket_hoy > 0) {
        candado.style.display = 'none';
        ...
        timeLockedMsg.classList.remove('hidden');
        statusBadge.textContent = "VENTA CERRADA";
        statusBadge.className = "text-[10px] font-black uppercase px-2 py-1 rounded bg-red-900 text-red-400 border border-red-600";
    } else {
        candado.style.display = 'flex';
...
function renderVipTicketUI() {
    ...
    if (!currentVipTicket) {
        grid.innerHTML = '<p class="text-center text-gray-500 col-span-3 py-6 text-xs uppercase tracking-widest"><i class="fas fa-clock mb-2 text-xl block"></i> Ticket en preparacion</p>';
        return;
    }
    ...
    const buildCol = (title, imgPath, dataArray, lotteryKey) => {
        ...
        dataArray.forEach((idAnimal) => {
            ...
            const isHit = currentVipResults[lotteryKey] && Object.values(currentVipResults[lotteryKey]).includes(idAnimal);
            
            const bgClass = isHit ? 'bg-green-900 border-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-black border-gray-600';
            const textClass = isHit ? 'text-green-400' : 'text-gray-300';
            const checkIcon = isHit ? '<i class="fas fa-check-circle text-green-500 absolute -top-2 -right-2 text-sm bg-black rounded-full"></i>' : '';

            colHtml += `<div class="relative border ${bgClass} rounded p-1 mb-2 flex flex-col items-center transition-all"><span class="text-xl leading-none mt-1">${aInfo.i}</span><span class="text-[10px] font-black mt-1 ${textClass}">${idAnimal}</span>${checkIcon}</div>`;
        });
...
window.comprarTicketVIP = async (montoUSD) => {
    ...
    if (!currentVipTicket) return alert("\u26A0 El ticket de hoy aun no esta disponible.");
    ...
    if(confirm(`Confirmas la compra del Ticket VIP de $${montoUSD}?\nSe descontaran Bs ${costoBs.toFixed(2)} de tu Saldo General.`)) {
        ...
        try {
            ...
            await db.ref(`users/${uid}/last_vip_date`).set(vipConfig.fecha_activa);
            ...
            alert("TICKET COMPRADO! Mucho exito en los sorteos.");
        } catch(e) {
...
window.abrirModalTraslado = () => {
    document.getElementById('vip-ticket-candado').style.display = 'none';
...
window.cerrarModalTraslado = () => {
    if(vipUserData.ticket_hoy === 0 && document.getElementById('vip-time-locked').classList.contains('hidden') && document.getElementById('vip-preparing-next').classList.contains('hidden')) {
        document.getElementById('vip-ticket-candado').style.display = 'flex';
    }
