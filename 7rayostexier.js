...
let vipUserData = { saldo_vip: 0, saldo_general: 0, last_transfer: 0, streak: 0, ticket_hoy: 0 };
...
function injectVipHTML() {
    ...
    <div class="bg-gray-900 border border-yellow-600 shadow-2xl rounded-xl overflow-hidden relative flex flex-col mb-6">
        ...
        <div class="p-4 flex-1">
            ...
            <div class="bg-gradient-to-b from-gray-800 to-black rounded-xl p-4 border border-gray-700 shadow-inner mb-5">
                <div class="flex justify-between items-center border-b border-gray-700 pb-3 mb-3">
                    ...
                    <div class="text-right">
                        ...
                        <span id="vip-general-display" class="text-lg font-bold text-white">$0.00</span>
                    </div>
                </div>
            ...
            <div id="vip-buy-section" class="space-y-3 mb-5">
                <p class="text-center text-xs text-gray-400 uppercase tracking-widest mb-2">\u00A1Gana x3 al instante!</p>
                
                <button onclick="comprarTicketVIP(2)" class="w-full relative overflow-hidden bg-gray-800 hover:bg-gray-700 text-white font-black py-4 rounded-xl border border-gray-600 shadow-md transition transform active:scale-95 flex justify-between items-center px-6">
                    <span class="text-gray-400"><i class="fas fa-play-circle mr-2"></i>TICKET $2</span>
                    <span class="text-green-400">COBRAS $6</span>
                </button>
                ...
                <button onclick="comprarTicketVIP(5)" class="w-full relative overflow-hidden bg-gradient-to-r from-yellow-600 via-yellow-500 to-yellow-600 hover:from-yellow-500 hover:to-yellow-400 text-black font-black py-4 rounded-xl border-2 border-yellow-300 shadow-[0_0_15px_rgba(234,179,8,0.4)] transition transform active:scale-95 flex justify-between items-center px-6">
                    ...
                    <span class="text-black font-black bg-white/20 px-2 py-1 rounded">COBRAS $15</span>
                ...
                <button onclick="comprarTicketVIP(10)" class="w-full relative overflow-hidden bg-gray-800 hover:bg-gray-700 text-white font-black py-4 rounded-xl border border-gray-600 shadow-md transition transform active:scale-95 flex justify-between items-center px-6">
                    <span class="text-gray-400"><i class="fas fa-gem mr-2"></i>TICKET $10</span>
                    <span class="text-green-400">COBRAS $30</span>
                </button>
            </div>
    ...
    <div id="modal-traslado" class="hidden fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
    ...
    <div id="modal-exito-traslado" class="hidden fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 backdrop-blur-md">
        <div class="bg-gray-900 border border-green-500 rounded-xl w-full max-w-sm p-6 shadow-2xl text-center transform transition-all scale-100">
            ...
            <p class="text-gray-300 text-sm mb-6">El dinero ya est\u00E1 disponible en tu Saldo General.</p>
...
function initVIPSystem() {
    ...
    db.ref(`users/${uid}`).on('value', s => {
        if (s.exists()) {
            const data = s.val();
            vipUserData.saldo_general = data.saldo || 0;
...
function actualizarUIUsuario() {
    ...
    document.getElementById('vip-general-display').textContent = `$${parseFloat(vipUserData.saldo_general).toFixed(2)}`;
...
window.comprarTicketVIP = async (montoUSD) => {
    ...
    if (vipUserData.saldo_general < costoBs) return alert(`\u274C Saldo General insuficiente. Necesitas Bs ${costoBs.toFixed(2)}.`);
    ...
    if(confirm(`\u00BFConfirmas la compra del Ticket VIP de $${montoUSD}?\nSe descontar\u00E1n Bs ${costoBs.toFixed(2)} de tu Saldo General.`)) {
        ...
        try {
            const uid = auth.currentUser.uid;
            await db.ref(`users/${uid}/saldo`).transaction(c => (c || 0) - costoBs);
            ...
            // Efecto Sonido Casino (Opcional si tienes el archivo, si no vibra)
...
window.procesarTraslado = async () => {
    ...
    const montoBs = montoTrasladoUSD * vipConfig.tasa; // Convertir a Bs para el Saldo General
    
    try {
        const uid = auth.currentUser.uid;
        // 1. Restar del VIP
        await db.ref(`users/${uid}/saldo_vip`).transaction(s => Math.max(0, (s || 0) - montoTrasladoUSD));
        // 2. Sumar al General (en Bs asumiendo que tu sistema base usa Bs)
        await db.ref(`users/${uid}/saldo`).transaction(c => (c || 0) + montoBs);
        // 3. Registrar hora del servidor
...
window.irABingo = () => {
    ...
    // Aqu\u00ED puedes agregar la funci\u00F3n que abra directamente el modal del bingo si la tienes.
};
