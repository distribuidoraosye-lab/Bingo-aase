...
window.closeCustomVideo = (type) => {
    ...
    // Reiniciar iconos y mostrar la capa para la próxima vez que abran el video
    const icon = document.getElementById(`play-icon-${type}`);
    if(icon) { icon.classList.remove('fa-pause'); icon.classList.add('fa-play', 'ml-1'); }
...
// Función del botón central (Play / Pausa) o al tocar la pantalla
...
// Función para adelantar (+10) y retroceder (-10)
...
/* ============================================================
   MÓDULO DE BOLSOS / SANES (VISTA USUARIO)
   ============================================================ */
...
window.openBolsosModal = async () => {
    ...
    try {
        ...
        if (userData.bolso_verificado === true) {
            const modalMercado = document.getElementById('modal-bolsos-mercado');
            if(modalMercado) modalMercado.style.display = 'flex';
            loadBolsos();
        } else {
            const modalVerif = document.getElementById('modal-verificacion-bolso');
            if(modalVerif) modalVerif.style.display = 'flex';
        }
    } catch (error) {
        console.error("Error validando usuario:", error);
    } finally {
        if (btn) {
            btn.innerHTML = '<span class="absolute top-0 right-0 bg-yellow-400 text-green-900 text-[9px] font-black px-3 py-1 rounded-bl-lg animate-pulse shadow-sm">AHORRO SEGURO</span><i class="fas fa-sack-dollar text-3xl mb-2 text-yellow-300 drop-shadow-md"></i><br><span class="font-black tracking-wider text-lg">BOLSOS VIP / SANES</span>';
        }
    }
};
...
window.sendWhatsAppVerification = () => {
    ...
    const numeroWhatsapp = "584220153364"; 
...
window.loadBolsos = () => {
    ...
    db.ref('bolsos').on('value', s => {
        container.innerHTML = '';
        if (!s.exists()) {
            container.innerHTML = '<p class="text-center text-gray-500 text-sm py-8 font-bold">No hay bolsos disponibles en este momento.</p>';
            return;
        }

        s.forEach(child => {
            const bolsoId = child.key;
            const b = child.val();
            ...
            let misCupos = [];
            Object.entries(participantes).forEach(([key, p]) => {
                if(p.uid === uid) misCupos.push(p);
            });

            const isFull = disponibles <= 0;
            
            let cardHtml = `
                <div class="bg-white p-4 rounded-xl shadow-md border-2 ${misCupos.length > 0 ? 'border-green-500' : (isFull ? 'border-gray-300' : 'border-yellow-400')} mb-4 relative overflow-hidden">
            `;

            if (misCupos.length > 0) {
                cardHtml += `<div class="absolute top-0 right-0 bg-green-500 text-white text-[10px] font-black px-3 py-1 rounded-bl-lg">\u2714\uFE0F PARTICIPANDO (${misCupos.length})</div>`;
            } else if (isFull) {
                cardHtml += `<div class="absolute inset-0 bg-gray-100/60 z-10 flex items-center justify-center backdrop-blur-[1px]"><span class="bg-gray-800 text-white font-black px-4 py-2 rounded-lg transform -rotate-12 border-2 border-gray-600 shadow-xl">LLENO</span></div>`;
            }

            cardHtml += `
                    <h3 class="font-black text-gray-800 text-lg mb-1">${b.nombre}</h3>
                    <div class="flex justify-between items-center mb-3">
                        <span class="text-xs font-bold text-gray-500">Premio: <span class="text-green-600 font-black">$${b.premio_usd}</span></span>
                        <span class="text-xs font-bold text-gray-500">Cuota: <span class="text-blue-600 font-black">$${b.costo_usd}</span></span>
                    </div>
                    <div class="bg-gray-50 p-2 rounded-lg mb-3 flex justify-between items-center border border-gray-200">
                        <span class="text-[10px] font-bold text-gray-500 uppercase">Cupos Ocupados</span>
                        <span class="text-xs font-black ${isFull ? 'text-gray-500' : 'text-orange-600'}">${ocupados} / ${b.cupos_totales}</span>
                    </div>
            `;
            ...
            if (misCupos.length > 0) {
                ...
                misCupos.forEach((miCupo, index) => {
                    const estado = miCupo.solvente === false 
                        ? `<span class="text-red-600 font-bold text-[10px]"><i class="fas fa-exclamation-circle mr-1"></i>En Mora</span>`
                        : `<span class="text-green-600 font-bold text-[10px]"><i class="fas fa-check-circle mr-1"></i>Al D\u00EDa</span>`;
                    const turnoInfo = miCupo.turno ? `Turno #${miCupo.turno}` : `Esperando Sorteo`;
                    
                    cardHtml += `
                        <div class="bg-blue-50 p-2 rounded flex justify-between items-center border border-blue-100">
                            <div>
                                <p class="text-[10px] font-bold text-gray-700">Mi Cupo ${index + 1}</p>
                                <p class="text-xs font-black text-blue-800">${turnoInfo}</p>
                            </div>
                ...
                if(!isFull) {
                     cardHtml += `<button onclick="joinBolso('${bolsoId}', '${b.nombre}', ${disponibles}, ${b.costo_usd})" class="w-full mt-2 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 border border-yellow-300 font-bold py-2 rounded-lg text-xs shadow-sm transition">Reservar Otro Cupo</button>`;
                }
                cardHtml += `</div>`;
            } else if (!isFull) {
                cardHtml += `
                    <button onclick="joinBolso('${bolsoId}', '${b.nombre}', ${disponibles}, ${b.costo_usd})" class="w-full bg-gradient-to-r from-yellow-400 to-yellow-500 hover:from-yellow-500 hover:to-yellow-600 text-yellow-900 font-black py-3 rounded-lg shadow-md transition transform active:scale-95">
                        PARTICIPAR AHORA
                    </button>
                `;
            }
...
window.joinBolso = async (bolsoId, nombreBolso, disponibles, costoUsd) => {
    ...
    const confirmMsg = `\u00BFEst\u00E1s seguro de apartar 1 cupo en el ${nombreBolso}?\n\n\u26A0\uFE0F Atenci\u00F3n: Al confirmar, ocupas el puesto y asumes el compromiso. El administrador descontar\u00E1 los $${costoUsd} (al cambio en Bs) de tu saldo de bingo. \u00A1Aseg\u00FArate de tener saldo positivo!`;
    ...
    try {
        ...
        alert("\u2705 \u00A1Felicidades! Tienes tu cupo reservado. Mant\u00E9n saldo positivo para no caer en mora.");
    } catch (error) {
        console.error("Error uni\u00E9ndose al bolso:", error);
