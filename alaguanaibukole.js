/* ============================================================
   ...
   Versión: Final - Promo Blindada (Depósito x3) + Fix Seguridad
   ============================================================ */
...
/* --- 3. LÓGICA DE VERIFICACIÓN (AQUÍ ESTÁ LA NUEVA PROMO BLINDADA) --- */
window.verifyPayment = async () => {
    ...
    try {
        ...
        if (found && found.s === 'valid') {
            ...
            // --- NUEVA PROMOCIÓN BLINDADA (COMPRA 3 O MÁS -> 1 GRATIS) ---
            let msgPromo = "";
            if (currentCardPrice > 0) {
                if (amtInp >= (currentCardPrice * 3)) {
                    ...
                    msgPromo = "<br>🎁 <b>¡GANASTE 1 CARTÓN GRATIS!</b>";
                }
            }
            // -------------------------------------------------------------
...
window.confirmCurrentCard=async()=>{ 
    ...
    if(purchaseState.currentCardIndex<purchaseState.totalQty) { renderBingoSel(); } 
        ...
        // VENTANA EMERGENTE DE COMPRA EXITOSA Y REDIRECCION
        const modalHtml = `
...
window.loadMyBingoHistory = () => {
    // Función ocultada lógicamente en la vista, se mantiene por estructura interna.
...
function renderHistoryTicket(type, b, container, dateLabel) {
    ...
    // CORRECCIÓN: Obtener imagen de la lotería
    const imgUrl = LOTTO_IMGS[b.lottery] || 'https://via.placeholder.com/50';
    const imgHtml = `<img src="${imgUrl}" class="w-8 h-8 rounded-full shadow-sm object-contain bg-white" alt="${b.lottery}">`;

    const animalsWithEmojis = animalsSafe.map(num => {
        const info = ANIMAL_MAP_LOTTO[String(num)] || { i: '?', n: '?' };
        const isHit = hitData.hits && hitData.hits.includes(String(num));
        const colorClass = isHit ? 'border-green-500 bg-green-50' : 'border-red-200 bg-red-50'; // Marcar rojo por defecto si no es hit
        
        return `<div class="mx-1 p-1 border-2 rounded ${colorClass} text-center min-w-[50px]">
                    <span class="text-lg">${info.i}</span><br>
                    <b class="text-xs">${num}</b>
                </div>`;
    }).join('');

    const tDiv = document.createElement('div'); 
    tDiv.className = "bg-white border rounded-xl p-3 mb-3 shadow-sm";
    tDiv.innerHTML = `
        <div class="flex justify-between items-center mb-2">
            <div class="flex items-center gap-2">
                ${imgHtml}
                <b class="text-xs">${b.lottery}</b>
            </div>
            <span class="text-[10px] font-mono bg-gray-100 px-2 py-1 rounded text-gray-600">${b.time}</span>
        </div>
        <div class="flex justify-center my-2">${animalsWithEmojis}</div>
        <div class="text-[10px] font-bold mt-2 text-center p-1 bg-gray-50 rounded">${hitData.resultsHtml}</div>
        <div class="text-right text-[10px] text-gray-400 mt-1">Bs ${b.amount}</div>
    `;
    container.appendChild(tDiv);
}

function calculateHitsHistory(ticketType, timeStr, userAnimals) {
    if (!historyResults) return { hitCount: 0, hits: [], resultsHtml: '<span class="text-orange-500">Pendiente...</span>' };
    ...
    lotteriesToCheck.forEach(lotto => {
        let win = null; 
        if (historyResults[lotto]) { 
            const keys = [cleanTime, `0${cleanTime}`]; 
            for (let k of keys) { if (historyResults[lotto][k]) { win = String(historyResults[lotto][k]); break; } } 
        }
        if (win) { 
            const isMatch = userSet.has(win);
            if (isMatch) { hitCount++; hits.push(win); }
            ticketResults.push(`<span class="${isMatch ? 'text-green-600 font-bold' : 'text-red-500'}">${lotto.split(' ')[0]}: ${win}</span>`); 
        }
    });
    return { hitCount, hits, resultsHtml: ticketResults.length > 0 ? ticketResults.join(' | ') : 'Sin resultados' };
}
...
window.loadMyDailyBets = () => {
    ...
    animRef.on('value', s => {
        ...
        if(!s.exists()){ listAnimalitos.innerHTML = '<p class="text-center text-gray-400 text-xs py-2">Sin jugadas.</p>'; return; }
        
        s.forEach(c => { 
            const b = c.val(); 
            // CORRECCIÓN: Lookup del emoji y nombre
            const info = ANIMAL_MAP_LOTTO[String(b.animal)] || {i: '❓', n: 'Animal'};
            
            const tDiv = document.createElement('div'); 
            tDiv.className = "bg-white border rounded-lg mb-1 p-2 flex items-center justify-between shadow-sm"; 
            tDiv.innerHTML = `
                <div class="flex items-center gap-2">
                    <span class="text-xl">${info.i}</span>
                    <div>
                        <p class="font-bold text-gray-800 text-[10px] leading-tight">${b.lottery}</p>
                        <p class="text-[9px] text-gray-500">${b.time} - ${info.n}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="font-black text-blue-700 text-[10px]">Bs ${b.amount}</p>
                    <p class="text-[8px] font-bold ${b.status === 'PENDING' ? 'text-orange-500' : 'text-green-500'}">${b.status}</p>
                </div>
            `; 
...
/* 2. NOTIFICACIÓN TOAST (ARRIBA A LA DERECHA) */
