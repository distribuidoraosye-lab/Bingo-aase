/* ============================================================
   BINGO TEXIER - MÓDULO RACHA (V39.1 - BLINDAJE + CASHOUT + TEXTOS)
            ...
            <div id="racha-main-box" style="background-color: #111827; border-radius: 1rem; padding: 4px; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5); border: 2px solid #eab308; margin-bottom: 24px; transition: opacity 0.3s ease;">
                <div style="background: linear-gradient(to right, #b91c1c, #ea580c, #b91c1c); padding: 14px; text-align: center; border-bottom: 1px solid #1f2937; border-radius: 0.8rem 0.8rem 0 0;">
                    <h3 style="font-weight: 900; color: white; font-size: 16px; text-transform: uppercase; margin: 0; text-shadow: 1px 1px 3px rgba(0,0,0,0.5);">
                        \u{1F3AF} RETO DE RACHA DIARIA \u{1F3AF}
                    </h3>
                </div>
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, activeRow, nivelMinimo, uid) {
    ...
    if(badgeEl) {
        badgeEl.innerHTML = diaSalvado ? "\u{2705} DÍA COMPLETADO" : "\u{1F3AF} FALTA JUGADA DE HOY";
        badgeEl.style.color = diaSalvado ? "#4ade80" : "#22d3ee";
    }
    
    // TABLA DE REGLAS (Con textos restaurados)
        ...
        rulesBox.innerHTML = `
            <div style="width: 100%; background-color: #111827; border: 1px solid #374151; border-radius: 8px; padding: 10px; font-size: 10px;">
                ...
                <div style="display: flex; flex-direction: column; gap: 4px;">
                    ${renderPremioRow(1, "Juega de 1 a 2 cartones diarios", "Completa los 7 días de racha para cobrar este premio.", "$7", activeRow)}
                    ${renderPremioRow(2, "Juega de 3 a 5 cartones diarios", "Completa los 7 días de racha para cobrar este premio.", "$21", activeRow)}
                    ${renderPremioRow(3, "Juega 6 o más cartones diarios", "Completa los 7 días de racha para cobrar este premio.", "$52", activeRow)}
                </div>
                ...
                <div style="background: rgba(234, 88, 12, 0.1); border-left: 3px solid #ea580c; padding: 6px; border-radius: 4px; margin-top: 6px;">
                    <p style="font-size: 8px; color: #fb923c; text-align: left; line-height: 1.3; font-weight: bold; margin: 0;">
                        \u{26A0}\u{FE0F} Importante: Si juegas un día para un premio mayor (ej. 6 cartones) y al día siguiente juegas menos (ej. 1 cartón), tu racha bajará automáticamente para adaptarse al premio más bajo. ¡Mantén tu nivel de juego constante!
                    </p>
                </div>
            </div>
        `;
    ...
    if(grid) {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            let icon = '\u{1F512}'; let style = 'background: #1f2937; border: 2px solid #374151; color: #4b5563;'; let anim = '';
            if (isToday) {
                if (diaSalvado) { icon = '\u{2705}'; style = 'background: #16a34a; border: 2px solid #4ade80; color: white; transform: scale(1.1); box-shadow: 0 0 10px #16a34a;'; anim = 'anim-verde'; }
                else { icon = '\u{1F3AF}'; style = 'background: rgba(34, 211, 238, 0.1); border: 2px solid #22d3ee; color: #22d3ee;'; anim = 'anim-diana'; }
            } else if (isPast) { icon = '\u{2705}'; style = 'background: #065f46; border: 2px solid #059669; color: #a7f3d0;'; }
            else if (i === 6) { icon = '\u{1F381}'; style = 'border: 2px dashed #4338ca; color: #818cf8;'; }
...
function renderPremioRow(lvl, texto, explicacion, monto, activeRow) {
    ...
    return `<div style="display: flex; flex-direction: column; padding: 6px 8px; border-radius: 4px; gap: 3px; ${style}"><div style="display: flex; justify-content: space-between; align-items: center;"><span style="color: white; font-weight: bold;">${texto}</span><span style="color: #4ade80; font-weight: 900; font-size: 12px;">${monto}</span></div><p style="font-size: 8px; color: #d1d5db; font-weight: bold; line-height: 1.1; margin: 0;">${explicacion}</p></div>`;
}

// 5. CÁLCULO Y RENDERIZADO DEL CASHOUT
function calcularMontoCashout(dias, nivel) {
    let basePremioDolares = 0;
    if (nivel === 1) basePremioDolares = 7;
    else if (nivel === 2) basePremioDolares = 21;
    else if (nivel === 3) basePremioDolares = 52;
...
// 6. ACCIÓN DEL CASHOUT (PAGO Y REINICIO SEGURO A DÍA 0)
window.ejecutarCashoutRacha = async (uid, montoBs) => {
    ...
    try {
        ...
        // 2. Limpiar la racha (Reiniciar progreso a 0). Se usa currentMasterDate para que no sume Día 1 por los cartones de hoy.
        const resetData = {
            dias: 0,
            last_active: currentMasterDate,
        ...
        alert(`\u{2705} ¡Cashout exitoso! Se han sumado ${montoBs.toFixed(2)} Bs a tu saldo.`);
        
    } catch (error) {
        alert("\u{274C} Error al procesar el Cashout: " + error.message);
        if(btn) { 
            btn.disabled = false; 
