/* ============================================================
   ...
   - Corrección de fecha para evitar bucles.
   - Candado anti-doble cobro (Transaction).
   - Panel de Estatus Reactivo (Bloqueado -> Desbloqueado).
   ============================================================ */
...
// 2. INICIO Y VIGILANCIA DE FIREBASE
    ...
    if (user) {
        ...
        dbHL.ref('config/hora_loca').on('value', snapAdmin => {
            ...
            dbHL.ref(`users/${hl_uid}/welcome_bonus_status`).once('value', snapBono => {
                ...
                // Fecha blindada (DD-MM-YYYY) para evitar el bug de recarga
                const d_hl = new Date();
                ...
                // EL VIGILANTE DE COMPRAS (Verifica el Saldo)
                dbHL.ref(`users/${hl_uid}/balance`).on('value', snapBalance => {
                    ...
                    if (hl_estadoReto === 'locked' && saldoActual < hl_balanceInicial) {
                        hl_estadoReto = 'claiming'; // Bloqueo local instantáneo
                        liberarPremioFinal();
                    }
                    if (hl_estadoReto === 'locked' && saldoActual > hl_balanceInicial) {
...
// 3. CONTROL REACTIVO DE INTERFAZ
