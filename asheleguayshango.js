...
const extractBankAmounts = (text) => {
            const regex = /(?:Bs\.?\s*)?(\d{1,3}(?:\.\d{3})*,\d{2})/gi;
            let match;
            let amounts = [];
            while ((match = regex.exec(text)) !== null) {
                let cleanNum = match.replace(/\./g, '').replace(',', '.');
                amounts.push(parseFloat(cleanNum));
            }
            return amounts;
        };
        ...
        if (logSnap.exists()) {
            logSnap.forEach(child => {
                ...
                // 1. Validar que la referencia exista en el mensaje
                if (!msgBanco.toLowerCase().includes(refInput.toLowerCase())) return;
                
                // 2. Extraer los montos reales del mensaje del banco
                const bankAmounts = extractBankAmounts(msgBanco);
                
                // 3. Comparación matemática blindada (amtInp es el monto que puso el usuario)
                const matchMonto = bankAmounts.includes(amtInp);
