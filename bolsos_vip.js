// 1. INYECTAR EL HTML DE LAS VENTANAS FLOTANTES DESDE EL JS
const modalesHTML = `
            ...
            <div class="p-4 bg-white text-gray-700">
                ...
                <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-200 mb-4 text-xs text-gray-700">
                    ...
                    <ul class="list-disc pl-4 space-y-1">
                        ...
                        <li>La cuota se descuenta automaticamente de tu saldo Bingo.</li>
    ...
    <div id="modal-bolsos-mercado" class="hidden modal-overlay" style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.9); z-index: 9999; align-items: center; justify-content: center;">
        <div class="modal-content h-[85vh] flex flex-col" style="background: white; width: 95%; max-width: 450px; border-radius: 15px;">
            <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 p-4 rounded-t-xl text-yellow-900 shadow-md flex justify-between items-center">
                ...
                <button onclick="document.getElementById('modal-bolsos-mercado').style.display='none'" class="text-yellow-900"><i class="fas fa-times text-2xl"></i></button>
            </div>
...
window.openBolsosModal = async () => {
    if (!auth.currentUser) return alert("Debes iniciar sesion primero.");
...
window.sendWhatsAppVerification = () => {
    ...
    const mensaje = `Hola Texier! Quiero verificarme para participar en los Bolsos VIP. Mi nombre de usuario es: *${nombre}*. Envio mi foto de cedula y selfie:`;
        ...
        s.forEach(child => {
            ...
            if (misCupos.length > 0) {
                ...
                misCupos.forEach((miCupo, index) => {
                    const estado = miCupo.solvente === false ? `<span class="text-red-600 font-bold text-[10px]"><i class="fas fa-exclamation-circle mr-1"></i>En Mora</span>` : `<span class="text-green-600 font-bold text-[10px]"><i class="fas fa-check-circle mr-1"></i>Al Dia</span>`;
                ...
                cardHtml += `<button onclick="joinBolso('${bolsoId}', '${b.nombre}', ${disponibles}, ${b.costo_usd})" class="w-full bg-yellow-400 text-yellow-900 font-black py-3 rounded-lg shadow-md">PARTICIPAR AHORA</button>`;
            }
...
window.joinBolso = async (bolsoId, nombreBolso, disponibles, costoUsd) => {
    if (!auth.currentUser) return;
    if (disponibles <= 0) return alert("Este bolso ya esta lleno.");
    const uid = auth.currentUser.uid; const nombre = auth.currentUser.displayName || 'Usuario';
    if (!confirm(`Seguro de apartar 1 cupo en el ${nombreBolso}?\n\nAl confirmar asumes el compromiso. El admin descontara los $${costoUsd} (al cambio) de tu saldo Bingo. Asegurate de tener saldo!`)) return;
    try {
        await db.ref(`bolsos/${bolsoId}/participantes`).push({ uid: uid, name: nombre, cuotas_pagadas: 0, solvente: true, fecha_ingreso: firebase.database.ServerValue.TIMESTAMP });
        alert("Exito! Tienes tu cupo reservado. Manten saldo positivo para no caer en mora.");
