/* ============================================================
   ...
   VERSIÓN DEFINITIVA - RELOJES 100% SINCRONIZADOS (BINGO + DEPÓSITOS)
   ============================================================ */
...
// LA FECHA MAESTRA: Sincronizada con el Bingo (Aplica Salto 8:00 PM)
function getSystemDate() {
    const d = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
    if(d.getHours() >= 20) d.setDate(d.getDate() + 1); // Salto a las 8PM 
...
async function initStreakMonitor(uid) {
    const dateStr = getSystemDate(); // Fecha Comercial (Ej: si son las 9pm, ya es mañana)

    // LECTURA DE DEPÓSITOS (Alineando la fecha física con la comercial)
    dbRacha.ref('referencias_usadas').orderByChild('uid').equalTo(uid).on('value', s => {
        let depositado = 0;
        if (s.exists()) {
            s.forEach(ref => {
                const data = ref.val();
                const timeVal = data.date || data.timestamp || data.fecha || data.time; 
                if (timeVal) {
                    const depDate = new Date(timeVal);
                    if(depDate.getHours() >= 20) depDate.setDate(depDate.getDate() + 1); // Lo alineamos al corte de caja
                    ...
                    if(depDateStr === dateStr) {
                        depositado += parseFloat(data.amt || data.amount || data.monto || 0);
                    }
                }
            });
        }
    ...
    // LECTURA BINGO ESTELAR (Ya viene con la fecha comercial)
    ...
    // LECTURA TORNEO EXPRESS (Mismo nodo de fecha comercial)
    dbRacha.ref(`bets_torneo_express/${dateStr}/${uid}`).on('value', s => {
        ...
        if (s.exists()) {
            s.forEach(c => { 
                const val = c.val();
                if(val.amount) {
                    jugadoT += parseFloat(val.amount);
                } else {
                    const tPrice = (typeof torneoConfig !== 'undefined' && torneoConfig.price > 0) ? torneoConfig.price : 35;
                    jugadoT += tPrice;
                }
            });
        }
...
function updateStreakUI(uid, dateStr) {
    ...
    // CÁLCULO ESTRICTO POR JUGADA ($1 = 1 Cartón = 35 Bs por defecto)
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        const d = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
        if(d.getHours() >= 20) d.setDate(d.getDate() + 1); // Asegurar el cálculo del día de ayer
        ...
        document.getElementById('today-deposit-text').textContent = "Bs " + streakState.todayDeposit.toLocaleString('es-VE', {minimumFractionDigits: 2});
        document.getElementById('today-played-text').textContent = "Bs " + streakState.todayPlayed.toLocaleString('es-VE', {minimumFractionDigits: 2});
