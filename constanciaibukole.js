/* ============================================================
   ...
   V9 FINAL - LOBBY INTELIGENTE + TASA DINÁMICA + LECTURA OMNÍVORA
   ============================================================ */

const dbRacha = firebase.database();
let configTasa = 35; // Valor por defecto por si falla la conexión
...
// 0. ESCUCHAR LA TASA DESDE EL ADMIN EN TIEMPO REAL
dbRacha.ref('config/racha_config/tasa_cambio').on('value', s => {
    if(s.exists()) {
        configTasa = parseFloat(s.val());
    }
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // MAGIA DE UX: Ocultar racha al entrar a un juego
    setTimeout(() => {
...
async function initStreakMonitor(uid) {
    ...
    // LECTOR DE DEPÓSITOS OMNÍVORO (Lee amt, amount o monto)
    ...
    // LECTOR BINGO (Se ajusta a la Tasa del Admin)
        ...
        if (s.exists()) {
            s.forEach(c => {
                ...
                if(val.date === dateStr && val.payment_method !== 'GRATIS') {
                    jugadoB += parseFloat(val.precio_carton || configTasa);
                }
            });
        }
    ...
    // LECTOR TORNEO (Se ajusta a la Tasa del Admin)
    dbRacha.ref(`bets_torneo_express/${dateStr}/${uid}`).on('value', s => {
        let jugadoT = 0;
        if (s.exists()) {
            s.forEach(c => { 
                const val = c.val();
                jugadoT += parseFloat(val.amount || val.monto || configTasa);
            });
        }
...
function updateStreakUI(uid, dateStr) {
    // CÁLCULO INTELIGENTE BASADO EN LA TASA EN TIEMPO REAL
    const usdDepositado = streakState.todayDeposit / configTasa;
    const usdJugado = streakState.todayPlayed / configTasa;
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        const isTomorrow = dReal.getHours() >= 20; // ¿Ya pasaron las 8 PM?
        
        // ACTUALIZACIÓN DE TEXTOS (HOY vs MAÑANA)
