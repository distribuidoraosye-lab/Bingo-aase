/* ============================================================
   ...
   VERSIÓN DEFINITIVA - RELOJ SINCRONIZADO Y LECTURA ROBUSTA
   ============================================================ */
...
// 1. Interfaz Premium (Textos claros y diseño amplio)
...
// FECHA DEL SISTEMA (Sin saltos raros, lee el día real)
...
async function initStreakMonitor(uid) {
    ...
    // LECTURA ROBUSTA DE DEPÓSITOS
    dbRacha.ref('referencias_usadas').orderByChild('uid').equalTo(uid).on('value', s => {
        let depositado = 0;
        if (s.exists()) {
            s.forEach(ref => {
                const data = ref.val();
                // Busca la fecha sin importar cómo la guarde la BD
                ...
                if (timeVal) {
                    ...
                    if(depDateStr === dateStr) {
                        // Suma la cantidad sin importar si la llamaste amt, amount o monto
