...
// ==========================================
//  LÓGICA DE VERIFICACIÓN (FINAL FIX)
// ==========================================
window.verifyPayment = async () => {
    ...
    // PROCESAMIENTO INTELIGENTE DE MONTO
    let rawAmount = document.getElementById('pay-amount').value.trim();
    if (rawAmount.includes('.') && rawAmount.includes(',')) {
        rawAmount = rawAmount.replace(/\./g, '');
    }
    rawAmount = rawAmount.replace(',', '.');
    const partes = rawAmount.split('.');
    if (partes.length > 2) {
        const decimales = partes.pop();
        const entero = partes.join('');
        rawAmount = entero + '.' + decimales;
    }
    const amountInput = parseFloat(rawAmount);

    const p_prefijo = document.getElementById('pay-phone-prefix').value;
    const p_numero = document.getElementById('pay-phone-number').value.trim();
    const fullPhone = p_prefijo + p_numero;
    ...
    if(!p_numero || p_numero.length < 7) { alert("Ingresa los 7 dígitos de tu teléfono."); return; }
    if(refInput.length < 5) { alert("Referencia mínima de 5 números."); return; }
    ...
    feedback.innerHTML = "Cruzando datos con el Banco...";
    ...
    try {
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => {
                ...
                const checkMatch = (data, key) => {
                    ...
                    const formattedAmt = amountInput.toLocaleString('es-VE', { minimumFractionDigits: 2 });
                    const simpleAmt = amountInput.toString().replace('.', ',');
                    const hasMonto = txt.includes(formattedAmt) || txt.includes(simpleAmt);
        ...
        if (matchFound) {
            if (matchFound.status === 'already_mine') { showError("YA ACREDITADO", "Este pago ya fue cargado."); return; }
            if (matchFound.status === 'fraud') { showError("ERROR", "Esta referencia ya fue usada."); return; }
            try {
                ...
                showSuccess(amountInput, "Saldo acreditado.");
            } catch (e) { showError("ERROR", "Intenta de nuevo."); }
        } else {
            showError("NO HALLADO", "No se encontró el pago automático.");
            btnSupport.onclick = () => {
                const msg = `Hola Soporte. Pago no verificado automáticamente: %0A- Ref: ${refInput} %0A- Monto: ${amountInput} Bs %0A- Tlf: ${fullPhone} %0A- Usuario: ${auth.currentUser.displayName}`;
