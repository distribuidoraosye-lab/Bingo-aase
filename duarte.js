/* ============================================================
   MAPAS DE DATOS Y CONFIGURACIÓN
   ============================================================ */
...
// ==========================================
//  LÓGICA DE VERIFICACIÓN (PAGO MÓVIL + BLOQUEO 3H)
// ==========================================
window.verifyPayment = async () => {
    ...
    // 1. CHEQUEO DE BLOQUEO TEMPORAL (3 HORAS)
    if (uData.bloqueo_hasta && now < uData.bloqueo_hasta) {
        const resto = uData.bloqueo_hasta - now;
        const minsTotal = Math.ceil(resto / 60000);
        const h = Math.floor(minsTotal / 60);
        const m = minsTotal % 60;
        alert(`⚠️ SISTEMA BLOQUEADO: Has fallado 5 veces consecutivas. Podrás intentar recargar de nuevo en ${h} horas y ${m} minutos.`);
        return;
    }

    const refInput = document.getElementById('pay-ref').value.trim();
    // Limpieza de monto (acepta formatos 1.250,00 y 1250.00)
    ...
    if(refInput.length < 5 || isNaN(amtInp) || amtInp <= 0) return alert("Por favor ingresa referencia (mínimo 5 dígitos) y monto válido.");

    btn.disabled = true; btn.innerHTML = '<i class="fas fa-circle-notch fa-spin mr-2"></i> VERIFICANDO...';
    fb.classList.remove('hidden'); fb.innerHTML = "Cruzando datos con el banco...";
    if(btnSupport) btnSupport.classList.add('hidden');

    try {
        // 2. EVITAR DUPLICADOS (Referencia ya usada)
        const used = await db.ref(`referencias_usadas/${refInput}`).once('value');
        if (used.exists()) { showError("ERROR", "Esta referencia ya fue registrada anteriormente."); return; }

        // 3. BUSCAR EN MONITOR (Sin validar teléfono, solo ref + monto)
        ...
        if (logSnap.exists()) {
            logSnap.forEach(child => {
                const check = (d, k) => {
                    if (!d || !d.mensaje_banco) return null;
                    const txt = d.mensaje_banco.toLowerCase();
                    const hasRef = txt.includes(refInput);
                    
                    const fmt1 = amtInp.toLocaleString('es-VE', { minimumFractionDigits: 2 });
                    const fmt2 = amtInp.toString().replace('.', ',');
                    const hasMonto = txt.includes(fmt1) || txt.includes(fmt2);
                    
                    if (hasRef && hasMonto) {
                        return (d.status === 'claimed') ? { k, s: 'fraud' } : { k, s: 'valid' };
                    }
                    return null;
                };
                let res = check(child.val(), child.key);
                if (!res && typeof child.val() === 'object') Object.keys(child.val()).forEach(sk => { const r = check(child.val()[sk], child.key+"/"+sk); if(r) res = r; });
                if (res) found = res;
            });
        }

        if (found && found.s === 'valid') {
            // APROBADO: Marcar como usado y acreditar
            await db.ref(`pagos_entrantes/${found.k}`).update({ status:'claimed', claimed_by:uid, claimed_at:firebase.database.ServerValue.TIMESTAMP });
            await db.ref(`referencias_usadas/${refInput}`).set({ uid, timestamp:firebase.database.ServerValue.TIMESTAMP });
            await userRef.update({ intentos_fallidos: 0, bloqueo_hasta: null });
            ...
            fb.innerHTML = `✅ Saldo acreditado correctamente.`;
            setTimeout(() => location.reload(), 2000);
        } else {
            // FALLO: Incrementar contador y bloquear si es necesario
            const f = (uData.intentos_fallidos || 0) + 1;
            if (f >= 5) {
                const tiempoBloqueo = now + (3 * 60 * 60 * 1000); // 3 Horas exactas
                await userRef.update({ intentos_fallidos: 0, bloqueo_hasta: tiempoBloqueo });
                alert("⚠️ Has fallado 5 veces consecutivas. Por seguridad, el sistema de recargas automáticas se ha bloqueado para ti por 3 horas.");
                location.reload();
            } else {
                await userRef.update({ intentos_fallidos: f });
                showError("NO ENCONTRADO", `No hallamos el pago. Te quedan ${5-f} intentos antes del bloqueo temporal.`);
            }
        }
    } catch (e) { btn.disabled = false; btn.innerHTML = 'Error'; console.error(e); }
    ...
    function showError(t, d) { 
        ...
        if(btnSupport) {
            ...
            btnSupport.onclick = () => {
                const msg = `Soporte: Pago no hallado automáticamente.%0ARef: ${refInput}%0AMonto: ${amtInp}%0AUsuario: ${auth.currentUser.displayName}`;
                window.open(`https://wa.me/584220153364?text=${msg}`, '_blank');
            };
        }
    }
};
...
// ==========================================
//  RESTO DEL CÓDIGO (FUNCIONES GLOBALES)
// ==========================================
