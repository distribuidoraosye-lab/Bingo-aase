/* ============================================================
   BINGO TEXIER - MÓDULO HORA LOCA Y SALDO BONO (V4.0)
   - Lógica de inyección idéntica al Bono de Bienvenida
   - Banner superior animado con Countdown
   ============================================================ */
...
// 1. INYECTAR HTML (BANNER Y MODALES)
function inyectarEstructuraHL() {
    if (document.getElementById('modal-ruleta-hl')) return; // Evitar duplicados
    ...
    // Banner Superior (Se inyecta al principio del contenedor principal)
    const bannerHTML = `
        <div id="hl-top-banner" class="hidden w-full bg-gradient-to-r from-purple-800 via-pink-600 to-purple-800 text-white p-2 text-center shadow-[0_4px_15px_rgba(219,39,119,0.5)] border-b-2 border-yellow-400 relative overflow-hidden" style="display:none; z-index: 40;">
            ...
            <p class="relative z-10 text-[10px] font-bold mt-1">Multiplica tu saldo. Termina en: <span id="hl-banner-timer" class="text-yellow-300 font-mono text-sm font-black ml-1">00:00:00</span></p>
        </div>
    `;
            ...
            <div class="bg-white p-6 rounded-2xl w-11/12 max-w-sm text-center shadow-[0_0_50px_rgba(255,255,255,0.2)]">
                ...
                <div class="text-left bg-gray-100 border border-gray-300 rounded-lg p-3 mb-5">
                    ...
                    <ul class="text-[11px] text-gray-700 font-bold space-y-2">
                        <li><i class="fas fa-check text-green-500 mr-1"></i>Para liberarlo, juega <b>5 cartones HOY</b> con tu saldo real.</li>
                        <li><i class="fas fa-ban text-orange-500 mr-1"></i>El Saldo Bono es ÚNICAMENTE para jugar.</li>
                        <li><i class="fas fa-clock text-red-500 mr-1"></i>A la medianoche se reinicia y se pierde.</li>
                    </ul>
                </div>
...
// 2. LÓGICA PRINCIPAL (Estilo Bienvenida)
function arrancarLectorHoraLoca() {
    if (typeof firebase === 'undefined' || !firebase.auth().currentUser) {
        setTimeout(arrancarLectorHoraLoca, 1000); // Esperar si Firebase no está listo
        return;
    }

    const db = firebase.database();
    hl_uid = firebase.auth().currentUser.uid;
    hl_fechaHoy = obtenerFechaLocalHL();
    inyectarEstructuraHL();

    // 2.1 Leer Admin en vivo
    db.ref('config/hora_loca').on('value', snapAdmin => {
        ...
        // 2.2 Verificar si es Nuevo Usuario (Jerarquía)
        db.ref(`users/${hl_uid}/welcome_bonus_status`).once('value', snapBono => {
            let estadoBienvenida = snapBono.val();
            if (estadoBienvenida === 'pending' || estadoBienvenida === 'active') {
                return; // Silencio total, está en Bienvenida
            }
            ejecutarRuletaUsuario(db); // Si pasó, ejecutamos lógica normal
        });
    });
}
    ...
    if (admin_HoraLocaActiva && admin_TerminaEn > Date.now()) {
        ...
        hl_intervaloTimer = setInterval(() => {
            ...
            if (falta <= 0) {
                ...
                document.getElementById('hl-banner-timer').innerText = 
...
function ejecutarRuletaUsuario(db) {
    db.ref(`users/${hl_uid}/hora_loca_data`).on('value', snap => {
        let data = snap.val() || {};
        
        if (data.date !== hl_fechaHoy) {
            if (admin_HoraLocaActiva) {
                db.ref(`users/${hl_uid}/hora_loca_data`).set({ date: hl_fechaHoy, status: 'pending', saldo_bono: 0, premio: 0, progreso: 0 });
                document.getElementById('modal-ruleta-hl').style.display = 'flex';
            }
            return;
        }

        hl_estadoReto = data.status || 'expired';
...
window.aceptarRetoHL = function() {
    firebase.database().ref(`users/${hl_uid}/hora_loca_data`).update({ status: 'active', premio: hl_premioAtrapado, progreso: 0 });
    ...
    if(hl_estadoReto === 'active' && typeof userBalance !== 'undefined') {
        ...
        if(userBalance < hl_lastB) {
            ...
            if(hl_progresoReto >= hl_metaReto) {
                firebase.database().ref(`users/${hl_uid}/hora_loca_data`).update({ status: 'completed', saldo_bono: firebase.database.ServerValue.increment(hl_premioAtrapado), progreso: hl_progresoReto });
                alert(`¡RETO SUPERADO! Tus ${hl_premioAtrapado} Bs han sido añadidos a tu Saldo Bono.`);
            } else {
                firebase.database().ref(`users/${hl_uid}/hora_loca_data`).update({ progreso: hl_progresoReto });
            }
...
function procesarBono(t) {
    ...
    firebase.database().ref(`users/${hl_uid}/hora_loca_data/saldo_bono`).set(hl_saldoBono - costo);
    firebase.database().ref('draw_status/date').once('value').then(s => {
        let f = obtenerFechaLocalHL(s.val());
        if (t === 'bingo') {
            firebase.database().ref(`bingo_to_grade/estelar/${f}/bets`).push().set({ uid: hl_uid, name: document.getElementById('user-display-name').innerText, phone: "N/A", date: f, status: "pending", payment_method: "BONO", animals: window.currentBingoCard || });
            ...
            firebase.database().ref(`bets_torneo_express/${f}/${hl_uid}`).push().set({ animalitos: window.selectedTorneoAnimals, tipo_pago: "SALDO_BONO", timestamp: firebase.database.ServerValue.TIMESTAMP });
            alert("¡Ticket comprado con Saldo Bono!"); location.reload();
        }
    });
}

// INICIADOR A PRUEBA DE FALLOS
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(arrancarLectorHoraLoca, 500));
} else {
    setTimeout(arrancarLectorHoraLoca, 500);
}
