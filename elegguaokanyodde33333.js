...
/* ============================================================
   MÓDULO DE RETIROS (INYECCIÓN DINÁMICA - CERO HTML)
   ============================================================ */

window.abrirModalRetiros = async () => {
    if (!auth.currentUser) return;
    ...
    if (depSnap.exists()) {
        let lastDepTime = 0;
        depSnap.forEach(d => { if (d.val().date > lastDepTime) lastDepTime = d.val().date; });
        ...
        if (timeSinceDeposit < DEPOSIT_COOLDOWN_MS) {
            ...
            alert(`\u26D4 SEGURIDAD: Acabas de depositar. Debes jugar tu saldo o esperar ${minsLeft} minutos para retirar.`);
            return;
        }
    }
