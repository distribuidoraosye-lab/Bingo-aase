/* ============================================================
   ...
   Versión: Final - Promo Blindada + Torneo Express Independiente + Video Custom + Pausa + Cajero Dinámico
   ============================================================ */
...
// --- VARIABLES MÓDULO RETIROS 2.0 ---
const MIN_WITHDRAWAL = 300;
const FLASH_FEE_PERCENT = 0.15;
const DEPOSIT_COOLDOWN_MS = 2 * 60 * 60 * 1000;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NUEVO EVENTO DE RETIRO DINÁMICO ---
...
function init(){
    ...
    // --- NUEVO: Escuchar retiros pendientes y PM guardado ---
    db.ref('withdrawal_requests').orderByChild('uid').equalTo(auth.currentUser.uid).on('value', s => {
        hasPendingWithdrawal = false;
        if (s.exists()) { s.forEach(child => { if (child.val().status === 'PENDING') hasPendingWithdrawal = true; }); }
    });
    db.ref(`users/${auth.currentUser.uid}/pago_movil_guardado`).on('value', s => {
        userSavedPM = s.exists() ? s.val() : null;
    });
    // --------------------------------------------------------
    ...
    // --- LÓGICA DE ESCUCHA DE BINGO (CON PAUSA VISUAL) ---
...
/* ============================================================
   CONTROLADORES DE VIDEO - YOUTUBE IFRAME API
   ============================================================ */
let playerTorneo;
let playerLoterias;
...
function onPlayerStateChange(event, type) {
    ...
    if (event.data === YT.PlayerState.PLAYING) {
        if(icon) { icon.classList.remove('fa-play', 'ml-1'); icon.classList.add('fa-pause'); }
        if(overlay) { overlay.classList.add('opacity-0'); } 
    } else if (event.data === YT.PlayerState.PAUSED) {
        if(icon) { icon.classList.remove('fa-pause'); icon.classList.add('fa-play', 'ml-1'); }
        if(overlay) { overlay.classList.remove('opacity-0'); }
    } else if (event.data === YT.PlayerState.ENDED) {
        closeCustomVideo(type); 
    }
}

window.openCustomVideoModal = (type) => {
    document.getElementById(`video-${type}-modal`).classList.remove('hidden');
    document.getElementById(`video-${type}-modal`).style.display = 'flex';
};

window.closeCustomVideo = (type) => {
    document.getElementById(`video-${type}-modal`).classList.add('hidden');
    document.getElementById(`video-${type}-modal`).style.display = 'none';
    let player = type === 'torneo' ? playerTorneo : playerLoterias;
...
window.seekCustomVideo = (type, seconds) => {
    let player = type === 'torneo' ? playerTorneo : playerLoterias;
...
/* ============================================================
   ...
   Versión: 2.1 (Anti-Fraude, Flash/Estándar, Horarios Dinámicos Admin)
   ============================================================ */
...
let hasPendingWithdrawal = false;
let userSavedPM = null;
let withdrawHours = { start: 10, end: 23 }; // Horario por defecto
...
function initWithdrawalModule() {
    ...
    // Escuchar Horarios del Admin Panel
    db.ref('config/withdraw_hours').on('value', s => {
        if(s.exists()) withdrawHours = s.val();
    });
...
async function abrirModalRetiros() {
    ...
    if (hora < withdrawHours.start || hora >= withdrawHours.end) {
        alert(`\u26A0\uFE0F FUERA DE HORARIO: Los retiros se procesan de ${withdrawHours.start}:00 a ${withdrawHours.end}:00. Tu saldo est\u00E1 seguro, por favor vuelve dentro del horario.`);
        return;
    }
...
function renderWithdrawalUI() {
    ...
    if (!userSavedPM) {
        contentDiv.innerHTML = `
            <div class="bg-yellow-50 p-3 rounded-lg border border-yellow-200 mb-4 text-center shadow-sm">
                <p class="text-[11px] font-bold text-yellow-800 leading-tight">\u26A0\uFE0F Por seguridad, vincula tu Pago M\u00F3vil \u00DANICO. Solo podr\u00E1s retirar a esta cuenta para evitar fraudes.</p>
            </div>
            <input id="new-pm-tlf" type="tel" class="w-full p-3 border border-gray-300 rounded-lg mb-3 text-sm focus:border-blue-500 outline-none" placeholder="Tel\u00E9fono (Ej: 04121234567)">
            <input id="new-pm-cedula" type="text" class="w-full p-3 border border-gray-300 rounded-lg mb-3 text-sm focus:border-blue-500 outline-none" placeholder="C\u00E9dula/RIF (Ej: V12345678)">
            <select id="new-pm-banco" class="w-full p-3 border border-gray-300 rounded-lg mb-4 text-sm focus:border-blue-500 outline-none"></select>
            <button type="button" onclick="guardarNuevoPM()" class="w-full bg-green-600 hover:bg-green-700 text-white font-black p-3 rounded-lg shadow-lg transition-transform active:scale-95">GUARDAR PAGO M\u00D3VIL</button>
        `;
        ...
        contentDiv.innerHTML = `
            <div class="bg-blue-50 p-3 rounded-lg mb-4 text-center border border-blue-200 shadow-inner">
                <p class="text-[10px] text-blue-500 font-bold uppercase mb-1">Tu Cuenta de Retiro Oficial</p>
                <p class="text-sm font-black text-blue-900 leading-tight">${userSavedPM.banco} <br> ${userSavedPM.telefono}</p>
                <p class="text-[11px] font-bold text-blue-700 mt-1">${userSavedPM.cedula}</p>
            </div>
            
            <div class="mb-4">
                <label class="block text-xs font-bold text-gray-500 mb-1 text-center">Monto a retirar (Min: ${MIN_WITHDRAWAL} Bs):</label>
                <input id="w-amount-input" type="number" min="${MIN_WITHDRAWAL}" class="w-full p-4 border-2 border-gray-300 rounded-xl text-2xl font-black text-center focus:border-blue-500 outline-none shadow-sm" placeholder="0.00">
            </div>
            
            <div class="space-y-3">
                <button type="button" onclick="procesarRetiroFase2('flash')" class="w-full bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-500 hover:to-orange-600 text-white p-3 rounded-xl shadow-lg border-b-4 border-orange-700 text-left relative overflow-hidden transition-transform transform hover:scale-[1.02] active:scale-95">
                    <div class="flex justify-between items-center relative z-10">
                        <div>
                            <h4 class="font-black text-red-900 flex items-center text-sm"><i class="fas fa-bolt text-lg mr-1 animate-pulse"></i> RETIRO FLASH</h4>
                            <p class="text-[9px] font-bold text-orange-100 uppercase tracking-widest mt-0.5">En ~15 minutos</p>
                        </div>
                        <div class="bg-red-900/20 px-2 py-1 rounded text-right">
                            <span class="text-[10px] text-red-900 font-black block leading-none mb-0.5">Comisi\u00F3n 15%</span>
                            <span class="text-[10px] text-white font-bold leading-none">Recibes: <span id="flash-preview">0.00</span> Bs</span>
                        </div>
                    </div>
                </button>

                <button type="button" onclick="procesarRetiroFase2('normal')" class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 p-3 rounded-xl border-2 border-gray-300 text-left transition-transform transform hover:scale-[1.02] active:scale-95 shadow-sm">
                    <div class="flex justify-between items-center">
                        <div>
                            <h4 class="font-bold text-gray-800 flex items-center text-sm"><i class="fas fa-clock mr-1 text-gray-500"></i> Retiro Est\u00E1ndar</h4>
                            <p class="text-[9px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">Hasta 48 Horas</p>
                        </div>
                        <div class="bg-green-100 border border-green-200 px-2 py-1 rounded text-right">
                            <span class="text-[10px] text-green-700 font-black block leading-none mb-0.5">0% Comisi\u00F3n</span>
                            <span class="text-[10px] text-gray-800 font-bold leading-none">Recibes: <span id="normal-preview">0.00</span> Bs</span>
                        </div>
                    </div>
                </button>
            </div>
            ...
            <div class="mt-3 text-center">
                <p class="text-[10px] text-gray-400 font-medium"><i class="fas fa-shield-alt mr-1"></i> Transacci\u00F3n segura procesada por Bingo Texier</p>
            </div>
        `;

        document.getElementById('w-amount-input').addEventListener('input', function() {
            const val = parseFloat(this.value) || 0;
            document.getElementById('flash-preview').textContent = (val * (1 - FLASH_FEE_PERCENT)).toFixed(2);
            document.getElementById('normal-preview').textContent = val.toFixed(2);
        });
    }
}

window.guardarNuevoPM = async () => {
...
window.procesarRetiroFase2 = async (tipo) => {
