...
/* --- VARIABLES GLOBALES --- */
let userBalance=0, serverTimeOffset=0, currentCardPrice=0, currentDate=null, globalLimit=0, totalSold=0, estadoVentasBingo='abierta';
...
function init(){
    ...
    // --- NUEVO: ESCUCHADOR DE PAUSA DEL ADMIN ---
    db.ref('bingo/estado_ventas').on('value', s => { 
        estadoVentasBingo = s.val() || 'abierta'; 
        updateBingoButtonState(); 
    });
}
...
function updateBingoButtonState() {
    ...
    // --- MODIFICADO: AHORA INCLUYE EL BLOQUEO POR PAUSA DEL ADMIN ---
    if (!isSellingHours || !isCorrectDate || miniGameStatus !== 'active' || estadoVentasBingo === 'pausada') {
        ...
        } else if (estadoVentasBingo === 'pausada') {
            buyBtn.innerHTML = "VENTA PAUSADA \u23F8\uFE0F" + badgeHtml;
        } else {
...
if(document.getElementById('start-purchase-btn')) document.getElementById('start-purchase-btn').onclick=()=>{
    ...
    if(miniGameStatus !== 'active') return alert("\u23F8\uFE0F El sorteo no est\u00E1 activo."); 
    // --- MODIFICADO: RESTRICCIÓN DE CLIC ---
    if(estadoVentasBingo === 'pausada') return alert("\u23F8\uFE0F Las ventas est\u00E1n pausadas temporalmente desde la administraci\u00F3n.");
...
window.confirmCurrentCard=async()=>{ 
    // --- MODIFICADO: DOBLE CHEQUEO ANTES DE PROCESAR PAGO ---
    if(miniGameStatus === 'paused' || estadoVentasBingo === 'pausada') return alert("\u23F8\uFE0F La venta ha sido pausada desde administraci\u00F3n. No se pudo procesar la compra."); 
    ...
    if(purchaseState.currentCardIndex<purchaseState.totalQty) { renderBingoSel(); } 
        ...
        // --- MODIFICADO: CORRECCIÓN DEL z- QUE SE COLGABA (ahora es z-50) ---
        const modalHtml = `
            <div id="purchase-success-modal" class="fixed inset-0 bg-black/80 z-50 flex items-center justify-center">
