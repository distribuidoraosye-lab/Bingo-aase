...
// ==========================================
//  LÓGICA DE VERIFICACIÓN MEJORADA (ANTI-FRAUDE & FORMATO BDV)
// ==========================================
window.verifyPayment = async () => {
    if(!auth.currentUser) return;
    
    // --- 1. CONFIGURACIÓN Y DATOS DE USUARIO ---
    const uid = auth.currentUser.uid;
    const userRef = db.ref(`users/${uid}`);
    const uData = (await userRef.once('value')).val() || {};
    const now = Date.now();

    // Chequeo de bloqueo temporal (3 horas)
    if (uData.bloqueo_hasta && now < uData.bloqueo_hasta) {
        const resto = Math.ceil((uData.bloqueo_hasta - now) / 60000);
        return alert(`⚠️ ESPERA: Sistema bloqueado por intentos fallidos. Podrás intentar de nuevo en ${resto} minutos.`);
    }

    // --- 2. PREPARACIÓN Y LIMPIEZA DE DATOS ---
    const refInput = document.getElementById('pay-ref').value.trim();
    
    // Limpieza inteligente: Acepta 1.000,00 o 1000.00 o 1000,00
    // Elimina puntos de miles y asegura que el decimal sea un punto para JS
    let rawAmt = document.getElementById('pay-amount').value.trim()
        .replace(/\./g, '')  // Quita puntos (1.000 -> 1000)
        .replace(',', '.');  // Cambia coma a punto (1000,00 -> 1000.00)
    ...
    if(refInput.length < 4 || isNaN(amtInp) || amtInp <= 0) {
        return alert("Por favor revisa la referencia y el monto introducido.");
    }

    // UI: Mostrar estado "Cargando"
    btn.disabled = true; 
    btn.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> VERIFICANDO...';
    fb.classList.remove('hidden'); 
    fb.innerHTML = "Analizando transacciones bancarias...";

    try {
        // --- 3. VERIFICAR SI LA REFERENCIA YA FUE USADA ---
        const used = await db.ref(`referencias_usadas/${refInput}`).once('value');
        if (used.exists()) { 
            showError("DENEGADO", "Esta referencia ya fue registrada y utilizada anteriormente."); 
            return; 
        }

        // --- 4. GENERAR FORMATOS DE BÚSQUEDA (ANTI-ERROR) ---
        // BDV envía: "Bs.1.000,00" o a veces "Bs.1000,00"
        const exacto = amtInp.toLocaleString('es-VE', { minimumFractionDigits: 2 }); // Ej: "1.000,00" o "0,10"
        const simple = amtInp.toFixed(2).replace('.', ','); // Ej: "1000,00" (sin punto de mil)
        const crudo = amtInp.toFixed(2); // Ej: "1000.00"

        // FUNCIÓN SEGURA: Verifica que el monto sea exacto y no parte de otro número
        // Evita que "1,00" haga match dentro de "21,00"
        const containsExactAmount = (fullText, amountStr) => {
            const txt = fullText.toLowerCase();
            const search = amountStr.toLowerCase();
            const idx = txt.indexOf(search);
            
            if (idx === -1) return false;

            // Revisamos caracteres vecinos (antes y después)
            const prevChar = idx > 0 ? txt[idx - 1] : ' ';
            const nextChar = idx + search.length < txt.length ? txt[idx + search.length] : ' ';
            const isDigit = (c) => c >= '0' && c <= '9';

            // Si tiene un número o punto/coma pegado, NO es válido
            if (isDigit(prevChar) || prevChar === ',' || prevChar === '.') return false;
            if (isDigit(nextChar)) return false;

            return true;
        };

        // --- 5. BUSCAR EN EL MONITOR DE PAGOS ---
        ...
        if (logSnap.exists()) {
            logSnap.forEach(child => {
                ...
                if (!data || !data.mensaje_banco) return;
                
                const msgBanco = data.mensaje_banco;
                
                // Paso A: ¿Contiene la referencia?
                if (!msgBanco.toLowerCase().includes(refInput)) return;

                // Paso B: ¿Contiene el monto exacto (seguro)?
                const matchMonto = containsExactAmount(msgBanco, exacto) || 
                                   containsExactAmount(msgBanco, simple) || 
                                   containsExactAmount(msgBanco, crudo);

                if (matchMonto) {
                    found = { k: child.key, s: (data.status === 'claimed' ? 'fraud' : 'valid') };
                }
            });
        }
        ...
        if (found && found.s === 'valid') {
            ...
            updates[`users/${uid}/intentos_fallidos`] = 0; // Resetear intentos
            ...
            showError("ERROR", "Esta referencia pertenece a un pago ya reclamado.");
        } else {
            // === FALLO: INCREMENTAR CONTADOR DE INTENTOS ===
            const f = (uData.intentos_fallidos || 0) + 1;
            if (f >= 5) {
                // Bloquear por 3 horas si falla 5 veces
                const bloqueo = now + (3 * 60 * 60 * 1000);
                await userRef.update({ intentos_fallidos: 0, bloqueo_hasta: bloqueo });
                alert("⛔ Has superado el límite de intentos fallidos. Tu sistema de recarga se ha bloqueado por 3 horas.");
                location.reload();
            } else {
                await userRef.update({ intentos_fallidos: f });
                showError("NO ENCONTRADO", `No encontramos un pago de Bs ${exacto} con ref ${refInput}. Intento ${f}/5.`);
            }
        }

    } catch (e) { 
        console.error("Error en verifyPayment:", e);
        showError("ERROR DE CONEXIÓN", "Intenta nuevamente o contacta a soporte.");
    }
    ...
    function showError(t, d) { 
        ...
        fb.innerHTML = `⚠️ <b>${t}:</b> ${d}`;
    }
};

// ==========================================
//  FUNCIONES GLOBALES (TICKER, RELOJ, BINGO, ANIMALITOS)
// ==========================================
