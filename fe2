/* ============================================================
   ...
   Versión: Blindada BDV + Soporte Activo
   ============================================================ */
...
/* --- 3. LÓGICA DE VERIFICACIÓN MEJORADA (SOPORTE Y DEBUG) --- */
window.verifyPayment = async () => {
    ...
    // Función interna para mostrar errores y activar soporte
    const showError = (titulo, detalle) => { 
        ...
        // MOSTRAR BOTÓN DE SOPORTE (Fix para que aparezca)
        if(btnSupport) {
    ...
    try {
        ...
        // Ocultar soporte al iniciar intento
        ...
        // --- D. FORMATOS DE BÚSQUEDA (ANTI-ERROR) ---
        // Formateador manual seguro para Venezuela (siempre 1.000,00)
        const formatVE = (num) => {
        ...
        const exacto = formatVE(amtInp);       // Ej: "1,00" o "1.000,00" (Forzado VE)
        const simple = amtInp.toFixed(2).replace('.', ','); // Ej: "1000,00"
        const crudo = amtInp.toFixed(2);       // Ej: "1000.00"

        // Función de coincidencia exacta (evita confundir 1,00 con 21,00)
        ...
        // Muestra el error real (ej: db permission, network, etc.)
