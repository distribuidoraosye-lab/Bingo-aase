/* ============================================================
   ...
   Versión: Final - Fix Pagos + Fix Animalitos/Tripleta
   ============================================================ */
...
/* --- VARIABLES GLOBALES (Corregidas para incluir Animalitos) --- */
...
// Variables necesarias para Animalitos, Tripletas y Dupletas (Recuperadas)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initTicker(); 
    // Carga de configuraciones de juego
...
/* --- 3. LÓGICA DE VERIFICACIÓN (FIX PARA "Bs." DEL BDV) --- */
window.verifyPayment = async () => {
    ...
    try {
        ...
        if(refInput.length < 4 || isNaN(amtInp) || amtInp <= 0) {
            return alert("Por favor revisa la referencia y el monto.");
        }
        ...
        // --- C. VERIFICAR REFERENCIA USADA ---
        const used = await db.ref(`referencias_usadas/${refInput}`).once('value');
        if (used.exists()) { 
            showError("DENEGADO", "Esta referencia ya fue utilizada."); 
            return; 
        }

        // --- D. FORMATOS DE BÚSQUEDA ---
        const formatVE = (num) => {
        ...
        const exacto = formatVE(amtInp);       // Ej: "1,00" o "1.000,00"
        const simple = amtInp.toFixed(2).replace('.', ','); // Ej: "1000,00"
        const crudo = amtInp.toFixed(2);       // Ej: "1000.00"

        // FUNCIÓN MEJORADA: Permite que el caracter anterior sea '.' si es texto (Bs.)
        const containsExactAmount = (fullText, amountStr) => {
            ...
            // NO PERMITIR si tiene un dígito o coma pegados
            if (isDigit(prevChar) || prevChar === ',') return false;
            
            // SI ES UN PUNTO, verificamos que NO sea separador de miles
            // (Si es separador de miles, tendría un dígito antes. Si es "Bs.", tiene una letra antes)
            if (prevChar === '.') {
                 const prevPrevChar = idx > 1 ? txt[idx - 2] : ' ';
                 if (isDigit(prevPrevChar)) return false; // "1.000" -> FALSO
            }
        ...
        if (logSnap.exists()) {
            logSnap.forEach(child => {
                ...
                // 1. ¿Contiene la referencia?
                if (!msgBanco.toLowerCase().includes(refInput)) return;

                // 2. ¿Contiene el monto exacto?
        ...
        showError("ERROR DEL SISTEMA", e.message || "Fallo desconocido");
    }
};

/* --- 4. FUNCIONES GLOBALES (TICKER, RELOJ, BINGO, ANIMALITOS) --- */
