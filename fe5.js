/* ============================================================
   ...
   Versión: Final - Fix Pagos + Fix Animalitos/Tripleta + GESTIÓN RETIROS PRO
   ============================================================ */
...
let activeWithdrawalInterval = null; // Para el contador de retiro
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('withdraw-btn').onclick=()=>{
        ...
        // OPCIONAL: Cargar historial al abrir este modal si lo deseas
        renderFullTransactionHistory(); 
    };
    
    // --- MODIFICACIÓN DE ENVÍO DE RETIRO PARA INCLUIR HORA ESTIMADA ---
    document.getElementById('withdraw-form').onsubmit=async(e)=>{
        ...
        // Reglas de horario: 10am (10) a 11pm (23)
        if (currentHour >= 10 && currentHour < 23) {
            // Horario Laboral: 1 hora máximo
            estimatedTime.setHours(now.getHours() + 1);
        } else if (currentHour < 10) {
            // Madrugada: Esperar a las 10am de hoy
            estimatedTime.setHours(10, 0, 0, 0);
        } else {
            // Después de las 11pm: Esperar a las 10am de mañana
...
function init(){
    ...
    // --- NUEVO: MONITOR DE RETIROS E HISTORIAL ---
    monitorWithdrawals();
    renderFullTransactionHistory();
...
window.verifyPayment = async () => {
    ...
    try {
        ...
        if (found && found.s === 'valid') {
            ...
            fb.innerHTML = `✅ Recarga de Bs ${exacto} exitosa.`;
...
/* ============================================================
   NUEVAS FUNCIONES: GESTIÓN DE RETIROS, POPUPS Y NOTIFICACIONES
   ============================================================ */

/* 1. MONITOR DE ESTADO Y BANNER FLOTANTE */
function monitorWithdrawals() {
    const uid = auth.currentUser.uid;
    // Escuchar retiros
    db.ref('withdrawal_requests').orderByChild('uid').equalTo(uid).limitToLast(1).on('value', s => {
        if(!s.exists()) {
        ...
        s.forEach(snap => {
            const data = snap.val();
            // Lógica del Popup de Cambio de Estado (Al entrar o cambiar)
            const lastStatus = localStorage.getItem('last_w_status_' + snap.key);
            
            if (data.status !== 'PENDING' && lastStatus !== data.status) {
                // El estado cambió a APROBADO o RECHAZADO
                showStatusPopup(data.status, data.amount);
                localStorage.setItem('last_w_status_' + snap.key, data.status);
            }

            // Lógica del Banner Flotante (Solo si está pendiente)
        ...
        if (diff > 0) {
            ...
            container.innerHTML = `
                <div class="flex justify-between items-center max-w-md mx-auto">
                    <div class="text-sm">
                        <p class="font-bold text-yellow-300"><i class="fas fa-clock"></i> Retiro en proceso</p>
                        <p class="text-xs text-gray-300">Tiempo estimado:</p>
                    </div>
            ...
            // TIEMPO AGOTADO (Opciones A y C)
            container.classList.remove('bg-blue-900');
            ...
            container.innerHTML = `
                <div class="text-center max-w-md mx-auto">
                    <p class="text-xs font-bold text-yellow-300 mb-1">⚠️ Alta demanda de pagos</p>
                    <p class="text-xs mb-2">Tu pago está garantizado y es el siguiente en cola.</p>
                    <a href="https://wa.me/584220153364?text=${encodeURIComponent(msgWsp)}" 
                       target="_blank"
                       class="block w-full bg-green-500 text-white text-sm font-bold py-1 px-4 rounded-full shadow hover:bg-green-600 transition">
                       <i class="fab fa-whatsapp"></i> Contactar Soporte Prioritario
                    </a>
                </div>
            `;
        }
...
/* 3. HISTORIAL UNIFICADO (PERFIL) */
async function renderFullTransactionHistory() {
    // Busca un contenedor en el HTML con este ID. Si no existe, no hace nada.
    // DEBES AGREGAR <div id="transaction-history-container"></div> en tu HTML del perfil.
    ...
    // Obtener Depósitos (buscando en referencias usadas)
    // Nota: Esto es menos eficiente si hay muchas, pero funcional para MVP.
    // Idealmente guardar 'deposits/{uid}'
    ...
    if(withdrawalsSnap.exists()) {
        withdrawalsSnap.forEach(s => {
            history.push({
                type: 'withdrawal',
                amount: s.val().amount,
                date: s.val().request_timestamp,
                status: s.val().status // PENDING, APPROVED, REJECTED
            });
        });
    }
    ...
    if(depositsSnap.exists()) {
        depositsSnap.forEach(s => {
            history.push({
                ...
                amount: s.val().amt,
                date: s.val().date,
                status: 'APPROVED' // Siempre aprobados si están aquí
            });
        });
    }
        ...
        history.forEach(h => {
            ...
            if (h.type === 'deposit') {
                ...
                if (h.status === 'APPROVED') {
                    ...
                    icon = '<i class="fas fa-clock text-yellow-500 text-xl fa-spin"></i>'; // Reloj girando
                    colorClass = 'border-l-4 border-yellow-500';
                }
            }

            html += `
            <div class="bg-white p-3 rounded shadow-sm flex justify-between items-center ${colorClass}">
                <div class="flex items-center space-x-3">
                    ${icon}
                    <div>
                        <p class="font-bold text-gray-800">${h.type === 'deposit' ? 'Recarga' : 'Retiro'}</p>
                        <p class="text-xs text-gray-500">${dateStr}</p>
                    </div>
                </div>
                <div class="font-bold ${h.type === 'deposit' ? 'text-green-600' : 'text-red-600'}">
                    ${h.type === 'deposit' ? '+' : '-'} Bs ${h.amount.toLocaleString('es-VE')}
                </div>
            </div>`;
        });
    }
...
/* 4. POPUP AL ENTRAR (SweetAlert Style manual) */
function showStatusPopup(status, amount) {
    ...
    if (status === 'APPROVED') {
        content = `
            <div class="bg-white rounded-lg p-6 max-w-sm w-full text-center animate-bounce-in">
                <div class="mx-auto w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mb-4">
                    <i class="fas fa-check text-3xl text-green-600"></i>
                </div>
                <h2 class="text-2xl font-bold text-gray-800 mb-2">¡Pago Enviado!</h2>
                <p class="text-gray-600 mb-4">Tu retiro de <b>Bs ${amount}</b> ha sido procesado exitosamente a tu cuenta bancaria.</p>
                <button onclick="this.closest('.fixed').remove()" class="w-full bg-green-600 text-white py-2 rounded font-bold hover:bg-green-700">ENTENDIDO</button>
            </div>
        `;
        ...
        content = `
            <div class="bg-white rounded-lg p-6 max-w-sm w-full text-center">
                ...
                <h2 class="text-2xl font-bold text-gray-800 mb-2">Solicitud Rechazada</h2>
                <p class="text-gray-600 mb-4">Tu retiro de <b>Bs ${amount}</b> no pudo procesarse. El saldo ha sido devuelto a tu cuenta.</p>
                <button onclick="this.closest('.fixed').remove()" class="w-full bg-gray-600 text-white py-2 rounded font-bold hover:bg-gray-700">CERRAR</button>
            </div>
        `;
    }
