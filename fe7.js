...
window.verifyPayment = async () => {
    ...
    try {
        ...
        if (found && found.s === 'valid') {
            ...
            // Esta lógica es segura porque se ejecuta SOLO si el banco validó el dinero (if found.s === 'valid')
            let msgPromo = "";
            if (currentCardPrice > 0) {
                // Verificamos si el monto depositado cubre 3 cartones o más
                if (amtInp >= (currentCardPrice * 3)) {
                    // Calculamos el valor actual o 0 y le sumamos 1
            ...
            setTimeout(() => location.reload(), 3000); // Damos un segundo más para que lean el premio
...
window.placeBet = async () => {
    ...
    try {
        if(isTripletaMode) {
            ...
            // SE ELIMINÓ LA LÍNEA QUE DABA CRÉDITO GRATIS AQUÍ PARA EVITAR EXPLOITS
...
function monitorWithdrawals() {
    ...
    db.ref('withdrawal_requests').orderByChild('uid').equalTo(uid).limitToLast(5).on('value', s => {
        ...
        s.forEach(snap => {
            ...
            // Banner Inferior (Solo pendientes)
...
/* 4. HISTORIAL UNIFICADO (FIX ORDEN + FIX NaN) */
    ...
    if(withdrawalsSnap.exists()) {
        withdrawalsSnap.forEach(s => {
            history.push({
                type: 'withdrawal',
                amount: s.val().amount,
                date: s.val().request_timestamp || s.val().timestamp || 0, // Fallback a 0
                status: s.val().status
            });
        });
    }
    ...
    if(depositsSnap.exists()) {
        depositsSnap.forEach(s => {
            history.push({
                type: 'deposit',
                amount: s.val().amt,
                date: s.val().date || 0, // Fallback a 0
                status: 'APPROVED'
            });
        });
    }
