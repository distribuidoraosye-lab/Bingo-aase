...
// IMÁGENES LOTERÍAS (Para mostrar en el ticket)
const LOTTO_IMGS = {
...
window.loadMyBingoHistory = () => {
    ...
    const listCards = document.getElementById('carton-list'); listCards.innerHTML=''; 
    db.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(auth.currentUser.uid).once('value',s=>{
        let found = false; s.forEach(c=>{ if(c.val().date===currentDate){ found = true; const gridHtml = c.val().numbers.map(n => { const animal = ANIMAL_MAP_BINGO[n]; return `<div class='animal-cell'><span style='font-size:14px; display:block;'>${animal.i}</span>${n}</div>`; }).join(''); const d=document.createElement('div'); d.className="bingo-card"; d.innerHTML=`<div class='text-center font-bold text-gray-700 mb-1'>ID: ${c.val().id}</div><div class='animal-grid'>${gridHtml}</div>`; listCards.appendChild(d); }});
...
function updateModeUI() {
    ...
    if(isTripletaMode) {
        ...
        selectLottery('Lotto Activo', 0); document.getElementById('selected-lottery-title').textContent = "DUPLETA (Lotto, Granjita, Selva)";
    }
...
function renderLottoTime(name) {
    ...
    let start = 8;
    if (isTripletaMode || isDupletaMode || name === 'Mega Animal 40') start = 9; else if (name === 'Lotto Rey') start = 8.5;
...
function renderLottoGrid(isGuacharo) {
    ...
    keys.forEach(k => {
        ...
        d.onclick = () => {
            if(isTripletaMode || isDupletaMode) {
                const max = isTripletaMode ? 3 : 2; const css = isTripletaMode ? 'tripleta-mark' : 'dupleta-mark';
                if(selectedLottoAnimals.has(k)) { selectedLottoAnimals.delete(k); d.classList.remove('selected', css); } 
                else if(selectedLottoAnimals.size < max) { selectedLottoAnimals.add(k); d.classList.add('selected', css); }
            } else {
...
function updateLottoTotal() {
    if(isTripletaMode) {
        ...
        document.getElementById('lotto-total-display').textContent = complete ? `Tripleta Lista` : `Selecciona 3`;
        ...
        document.getElementById('lotto-total-display').textContent = complete ? `Dupleta Lista` : `Selecciona 2`;
        const btn = document.getElementById('lotto-play-btn'); btn.disabled=!complete; btn.classList.toggle('opacity-50', !complete);
    } else {
...
window.placeBet = async () => {
    ...
    try {
        if(isTripletaMode) {
            ...
            await db.ref(`bets_tripletas/${currentDateStr}/${auth.currentUser.uid}`).push({ lottery: selectedLotto, time, animals: Array.from(selectedLottoAnimals), amount: cost, status: 'PENDING', timestamp: firebase.database.ServerValue.TIMESTAMP });
            ...
            await db.ref(`users/${auth.currentUser.uid}/balance`).transaction(c => (c||0) - cost);
            await db.ref(`bets_dupletas/${currentDateStr}/${auth.currentUser.uid}`).push({ lottery: "Dupleta", time, animals: Array.from(selectedLottoAnimals), amount: cost, status: 'PENDING', timestamp: firebase.database.ServerValue.TIMESTAMP });
...
// --- LOGICA DE RESULTADOS Y ACIERTOS ---
function calculateHits(ticketType, timeStr, userAnimals) {
    if (!dailyResults) return { hitCount: 0, hits: [], results: [] };
    // Normalizar hora (ej "9:00 AM" a "9-00-AM" y "09:00 AM" a "9-00-AM")
    // Se quita el cero inicial si existe y se reemplazan caracteres
    ...
    lotteriesToCheck.forEach(lotto => {
        ...
        if (winningAnimal) {
            const isHit = userSet.has(winningAnimal);
            if (isHit) { hitCount++; hits.push(winningAnimal); }
            // Agregar al display de resultados
            ...
            ticketResults.push(`<div class="flex flex-col items-center"><img src="${logo}" class="w-6 h-6 rounded-full border border-gray-300"><span class="text-[10px] font-bold ${isHit ? 'text-green-600':'text-gray-500'}">${animalInfo.i} ${winningAnimal}</span></div>`);
             ...
             // Si no ha salido, mostrar interrogación o logo gris
             let logo = LOTTO_IMGS[lotto];
             if(logo && !logo.startsWith('http')) logo = 'https://bingotexier.com/archivos/imagenes/' + logo;
...
window.loadMyDailyBets = () => {
    ...
    const listAnimalitos = document.getElementById('my-bets-list'), listTripletas = document.getElementById('popup-tripletas-list'), listDupletas = document.getElementById('popup-dupletas-list');
    ...
    tripRef.on('value', s => {
        listTripletas.innerHTML = ''; if(!s.exists()) { listTripletas.innerHTML='<div class="text-center text-gray-400 p-4">No tienes tripletas jugadas hoy.</div>'; } else {
            ...
            trips.reverse().forEach(b => {
                ...
                // CALCULAR ACIERTOS Y RESULTADOS
                const hitData = calculateHits('tripleta', b.time, animalsSafe);
                const animalsWithEmojis = animalsSafe.map(num => { 
                    const strNum = String(num); const info = ANIMAL_MAP_LOTTO[strNum] || {i:'\u2753', n:'?'}; 
                    const isHit = hitData.hits.includes(strNum);
                    return `<div class="flex flex-col items-center mx-1 p-1 rounded border ${isHit ? 'border-green-500 bg-green-100' : 'border-gray-200'}"><span class="text-lg">${info.i}</span><span class="font-bold text-gray-800 text-xs">${strNum}</span></div>`; 
                }).join('');

                const tDiv = document.createElement('div'); tDiv.className = "bg-white border-2 border-purple-300 rounded-lg p-2 shadow-md mb-2";
                tDiv.innerHTML = `<div class="flex justify-between items-center border-b border-gray-200 pb-1 mb-1"><span class="font-bold text-purple-700 text-xs">TRIPLETA - ${b.time}</span>${stHtml}</div>
                <div class="flex justify-center mb-1">${animalsWithEmojis}</div>
                <div class="bg-gray-50 p-1 rounded mt-1 border border-gray-100"><p class="text-[9px] text-gray-500 text-center mb-1">RESULTADOS:</p><div class="flex justify-center gap-2">${hitData.resultsHtml}</div></div>
                <div class="text-center border-t pt-1 mt-1"><span class="text-xs font-bold text-blue-600">Aciertos: ${hitData.hitCount}/3</span></div>`;
    ...
    dupRef.on('value', s => {
        listDupletas.innerHTML = ''; if(!s.exists()) { listDupletas.innerHTML='<div class="text-center text-gray-400 p-4">No tienes dupletas jugadas hoy.</div>'; } else {
            ...
            dups.reverse().forEach(b => {
                ...
                tDiv.innerHTML = `<div class="flex justify-between items-center border-b border-gray-200 pb-1 mb-1"><span class="font-bold text-blue-700 text-xs">DUPLETA - ${b.time}</span>${stHtml}</div>
                <div class="flex justify-center mb-1">${animalsWithEmojis}</div>
                <div class="bg-gray-50 p-1 rounded mt-1 border border-gray-100"><p class="text-[9px] text-gray-500 text-center mb-1">RESULTADOS:</p><div class="flex justify-center gap-2">${hitData.resultsHtml}</div></div>
                <div class="text-center border-t pt-1 mt-1"><span class="text-xs font-bold text-blue-600">Aciertos: ${hitData.hitCount}/2</span></div>`;
