/* ============================================================
   BINGO TEXIER - MÓDULO BONO BIENVENIDA (RULETA EMOCIONAL V5)
   - Filtro Usuarios Viejos (vía referencias_usadas)
   - Z-Index Blindado (Evita sobremontaje de racha)
   - Bóveda Rediseñada (Premio Doble)
   - Extracción de Teléfono Real
   ============================================================ */
...
// 4. INICIALIZACIÓN Y FILTRO DE USUARIOS VIEJOS
function initBonoModule() {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            // OBTENER TELÉFONO REAL DIRECTO DE LA BASE DE DATOS
            ...
            dbBono.ref('draw_status/date').on('value', snap => {
                ...
                if(currentBonoDate !== "00-00-0000") procesarFiltroYLogica(user.uid);
            });
        }
    });
}

async function procesarFiltroYLogica(uid) {
    // 1. Revisamos el estado del bono de este usuario
    const snapBono = await dbBono.ref(`users/${uid}/bono_bienvenida`).once('value');
    
    // Si ya fue marcado como usuario viejo, abortamos.
    if (snapBono.exists() && snapBono.val().status === 'old_user') return;
    // Si ya lo cobró, ocultamos bóveda y abortamos.
    ...
    // 2. Si no tiene el nodo creado, ES LA PRIMERA VEZ QUE ENTRA.
    // Vamos a verificar si tiene recargas previas (es un usuario viejo)
    if (!snapBono.exists()) {
        const refSnap = await dbBono.ref('referencias_usadas').orderByChild('uid').equalTo(uid).once('value');
        if (refSnap.exists()) {
            // ¡Alerta! Es un cliente viejo. Lo marcamos en silencio y no mostramos ruleta.
            await dbBono.ref(`users/${uid}/bono_bienvenida`).set({ status: 'old_user' });
            return;
        } else {
            // Es 100% NUEVO. Lanzamos la Ruleta Emocional.
            document.getElementById('modal-ruleta-bienvenida').style.display = 'flex';
            return;
        }
    }

    // 3. Si llega aquí, es porque tiene el bono 'active' o 'completed'. Procedemos a leer sus jugadas.
