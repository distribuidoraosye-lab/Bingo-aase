/* ============================================================
   BINGO TEXIER - MÓDULO BONO BIENVENIDA (RULETA EMOCIONAL V7 - DEFINITIVA)
   - Limpieza de Caché al cerrar sesión (Evita cruces)
   - Filtro Infalible por Fecha de Creación (CreationTime)
   - Lógica Pasiva + Fechas Seguras (Heredado de V38 Anti-MaxRetry)
   ============================================================ */
...
// 1. FECHAS BLINDADAS (SISTEMA V38)
...
async function procesarFiltroYLogica(userObj) {
    ...
    // Ya no se envía data pre-cargada. La función leerá fresco.
    monitorBonoData(uid);
}

// 5. MONITOR Y LÓGICA PASIVA (ANTI-MAXRETRY)
function monitorBonoData(uid) {
    dbBono.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        ...
        syncTimerBono = setTimeout(() => { calcularDiasBono(uid); }, 800);
    });
...
function calcularDiasBono(uid) {
    dbBono.ref(`users/${uid}/bono_bienvenida`).once('value', s => {
        let data = s.val();
        if (!data || data.status !== 'active') return;

        let totalHoy = bonoState.totalTickets;
        let diaSalvado = (totalHoy >= 1);
        ...
        // EVITA ENVIOS INUTILES A FIREBASE
