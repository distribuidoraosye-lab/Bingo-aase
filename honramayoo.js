...
// -------------------------------------------------------------
// SECCIÓN 1: BONO DE BIENVENIDA Y LÓGICA DE FECHAS REESCRITA
// -------------------------------------------------------------
...
function normalizeDateBono(str) {
    ...
    if (p.length === 3) {
        // Si el sistema manda YYYY-MM-DD (ej. 2026-05-07), lo voltea a DD-MM-YYYY
        if (p.length === 4) {
            return `${p.padStart(2, '0')}-${p.padStart(2, '0')}-${p}`;
        }
        // Si ya viene DD-MM-YYYY, solo asegura que tengan dos dígitos
        return `${p.padStart(2, '0')}-${p.padStart(2, '0')}-${p}`;
    }
    return clean;
}
...
function addDaysSafeBono(dateStr, daysToAdd) {
    ...
    let d = parseInt(p, 10); 
    let m = parseInt(p, 10) - 1; // En JavaScript los meses son de 0 a 11
    let y = parseInt(p, 10);
    
    // Se pone al mediodía para evitar problemas de zona horaria
...
function getFirebaseVipDate(dateStr) {
    ...
    let p = dateStr.split('-');
    if(p.length === 3) return `${p}-${p}-${p}`; // Estructura limpia garantizada
    return dateStr;
}
...
function initVipModule(uid, masterDate) {
    ...
    // Leer la racha pasivamente (Mismo orden intocable de tu original)
...
function renderVipButton() {
    ...
    // CICLO PARA LAS IMÁGENES EXACTO AL DEL ADMIN
