...
// -------------------------------------------------------------
// SECCIÓN 1: BONO DE BIENVENIDA (ORIGINAL INTACTO)
// -------------------------------------------------------------

// 1. FECHAS BLINDADAS (ACTUALIZADAS A NUEVA LÓGICA)
function normalizeDateBono(str) {
    if (!str) return "00-00-0000";
    let clean = String(str).replace(/\//g, '-').trim();
    let p = clean.split('-');
    
    // Simplificado para que siempre entienda DD-MM-YYYY
    if (p.length === 3) {
        if (p.length === 4) { // Si entra YYYY-MM-DD lo voltea
...
function addDaysSafeBono(dateStr, daysToAdd) {
    ...
    // Corregido: se asignan los índices para que no asuma 1907
    let d = parseInt(p, 10); 
    let m = parseInt(p, 10) - 1; 
    let y = parseInt(p, 10);
    
    let dateObj = new Date(y, m, d, 12, 0, 0); // 12 PM para evitar fallos de zona horaria
...
function monitorBonoData(uid, dataBono) {
    dbBono.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        ...
        // Actualizar VIP si hubo cambios
...
// 3. RECUPERACIÓN DE DATOS (ARREGLO DE COMAS)
function getFirebaseVipDate(dateStr) {
    if (!dateStr || dateStr === "00-00-0000") return "00-00-0000";
    let p = dateStr.split('-');
    // Corregido: Retorna con los guiones bien puestos en vez de comas
    if(p.length === 3) return `${p}-${p}-${p}`;
    return dateStr;
}
...
function injectVipModuleUI() {
    ...
    // Crear el contenedor del botón y colocarlo ARRIBA del dashboard
...
function checkVipPermissions() {
    ...
    // Verificamos si compró hoy usando bonoState.totalTickets de la sección 1
...
function renderVipButton() {
    ...
    let logosHtml = `
        <div class="flex -space-x-2 mt-1 justify-center">
            <img src="${VIP_LOTTERIES.img}" class="w-6 h-6 rounded-full border border-gray-400 shadow-sm opacity-90">
            <img src="${VIP_LOTTERIES.img}" class="w-6 h-6 rounded-full border border-gray-400 shadow-sm opacity-90">
            <img src="${VIP_LOTTERIES.img}" class="w-6 h-6 rounded-full border border-gray-400 shadow-sm opacity-90">
        </div>
    `;
...
function renderVipContent(tab) {
    ...
    const fbDate = getFirebaseVipDate(dateToFetch);
