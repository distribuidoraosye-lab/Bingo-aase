/* ============================================================
   ...
   Versión: Final - Promo Blindada (Depósito x3) + Fix Seguridad + UI Historial
   ============================================================ */
...
window.selectLottery = (name, payout) => {
    ...
    if(!isTripletaMode && !isDupletaMode) {
        ...
        let btnId = 'btn-lotto-activo'; if(name.includes('Granjita')) btnId='btn-la-granjita'; if(name.includes('Guacharo')) btnId='btn-el-guacharo'; if(name.includes('Ruleta')) btnId='btn-ruleta-activa'; if(name.includes('Mega')) btnId='btn-mega-animal-40'; if(name.includes('Selva')) btnId='btn-selva-plus';
...
function renderLottoTime(name) {
    ...
    let start = 8; if (isTripletaMode || isDupletaMode || name === 'Mega Animal 40') start = 9; 
    let end = 19;
...
window.placeBet = async () => {
    ...
    try {
        if(isTripletaMode) {
            ...
            // Validación de límites de sistema
...
/* --- FUNCIÓN AUXILIAR PARA EXTRAER LA IMAGEN DIRECTO DEL DOM --- */
function getLottoImg(name) {
    let id = '';
    if(name === 'Lotto Activo') id='btn-lotto-activo';
    if(name === 'La Granjita') id='btn-la-granjita';
    if(name === 'Ruleta Activa') id='btn-ruleta-activa';
    if(name === 'Mega Animal 40') id='btn-mega-animal-40';
    if(name === 'Selva Plus') id='btn-selva-plus';
    if(name === 'El Guacharo') id='btn-el-guacharo';
    if(id) {
        const img = document.querySelector(`#${id} img`);
        if(img && img.src) return img.src;
    }
    return LOTTO_IMGS[name] || '';
}
...
function calculateHitsHistory(ticketType, timeStr, userAnimals) {
    ...
    lotteriesToCheck.forEach(lotto => {
        ...
        if (win) { 
            ...
            const imgPath = getLottoImg(lotto);
...
window.loadMyDailyBets = () => {
    ...
    animRef.on('value', s => {
        ...
        s.forEach(c => { 
            ...
            const imgPath = getLottoImg(b.lottery);
            const imgTag = imgPath ? `<img src="${imgPath}" class="w-6 h-6 inline-block rounded-full border border-gray-300">` : '';
            
            // Traducción y color de estados
            let sTxt = b.status || 'PENDING';
            let sCol = 'text-gray-600';
            let sUpper = sTxt.toUpperCase();
            if (sUpper === 'PENDING') { sTxt = 'Pendiente'; sCol = 'text-yellow-600'; }
            else if (sUpper === 'WIN' || sUpper === 'WON') { sTxt = 'Ganador'; sCol = 'text-green-600'; }
            else if (sUpper === 'LOST' || sUpper === 'LOSE') { sTxt = 'Perdedor'; sCol = 'text-red-600'; }
            else { sCol = 'text-blue-600'; }
            
            const tDiv = document.createElement('div'); 
            tDiv.className = "bg-white border rounded mb-1 p-2 text-xs flex items-center gap-3"; 
            tDiv.innerHTML = `<span class="text-2xl">${aInfo.i}</span> <div class="flex-1"><div class="flex items-center gap-1 mb-1">${imgTag} <b>${b.lottery}</b> - ${b.time}</div> Animal: <b>${b.animal} (${aInfo.n})</b> | Bs ${b.amount} | <b class="${sCol}">${sTxt}</b></div>`; 
