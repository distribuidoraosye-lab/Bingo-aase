/* ============================================================
   ...
   Versión: Final - Promo Blindada + Torneo Express Optimizada
   ============================================================ */
...
function init(){
    ...
    // --- LEER CONFIGURACIÓN DEL TORNEO EXPRESS Y ACTUALIZAR ESTADOS ---
    ...
    // --- NUEVO CRONÓMETRO: SOLO TOCA EL BOTÓN SI ES ESTRICTAMENTE NECESARIO ---
    setInterval(() => {
        const now = new Date(Date.now() + serverTimeOffset);
        ...
        if(now > target && now.getHours() < 18) {
            ...
            if(diff > 0 && display) {
                ...
                // Solo reescribe si el texto cambió (evita parpadeos)
...
// --- CARGA INMEDIATA: Separamos el llamado para no hacer esperar al usuario ---
function syncTime(){
    updateGameDate(); // Llama con la hora local de inmediato
    fetch('https://worldtimeapi.org/api/timezone/America/Caracas')
        .then(r => r.json())
        .then(d => {
            serverTimeOffset = new Date(d.datetime).getTime() - Date.now();
            updateGameDate(); // Ajusta silenciosamente en segundo plano
        }).catch(e => {});
}
...
// --- PUENTE DEL POZO: Calcula siempre con los datos más frescos ---
...
function listenTorneoPot() {
    ...
    db.ref(`bets_torneo_express/${currentDateStr}`).on('value', s => {
        ...
        renderTorneoPotDisplay(); // Se actualiza solo si hay tickets nuevos
    });
}

// --- FRENO DEL BOTÓN: Solo se toca si cambia el precio o se cierra el reloj ---
function updateTorneoButtonState() {
    ...
    // Si el estado es exactamente igual al de hace 1 segundo, no toques el HTML
