/* ============================================================
   ...
   V6 - BARRA ESTRICTA, UPSELL AGRESIVO Y BINGO+TORNEO
   ============================================================ */
        ...
        <div class="bg-gray-900 rounded-2xl p-1 shadow-2xl border-2 border-yellow-500 relative overflow-hidden mt-2 mb-6">
            ...
            <div class="bg-gradient-to-r from-red-700 via-orange-600 to-red-700 p-4 text-center border-b border-gray-800 shadow-inner">
                <h3 id="projected-prize-text" class="font-black text-white text-lg tracking-tighter uppercase animate-pulse drop-shadow-lg leading-tight">
                    🔥 ¡JUEGA DE $1 A $6 DIARIOS Y GANA HASTA $60 GRATIS AL DÍA 7! 🔥
                </h3>
            </div>
            ...
            <div class="bg-gray-800 p-4 border-t border-gray-700">
                ...
                <p id="rollover-warning" class="text-[10px] text-red-400 text-center mt-3 font-black uppercase tracking-widest hidden animate-pulse">
                    ⚠️ ¡ALERTA! Juega todo lo depositado para asegurar el día.
                </p>
            </div>
        ...
        <div id="modal-rules-racha" class="hidden fixed inset-0 bg-black/95 z- flex items-center justify-center p-4">
            <div class="bg-gray-900 border-2 border-yellow-500 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
                ...
                <div class="space-y-3 text-xs text-gray-300">
                    <p><b class="text-white uppercase">¿CÓMO GANAR?</b><br>Juega 7 días seguidos cumpliendo el mínimo diario. El premio final se basa en el nivel del día que MENOS hayas apostado en la semana.</p>
...
function updateStreakUI(uid, dateStr) {
    // 1 Cartón = $1 (Métrica Dinámica)
    const cardPrice = (typeof currentCardPrice !== 'undefined' && currentCardPrice > 0) ? currentCardPrice : 35;
    
    const usdDepositado = streakState.todayDeposit / cardPrice;
    const usdJugado = streakState.todayPlayed / cardPrice;
    
    // Evaluamos el nivel BASADO EN LO QUE JUGÓ Y DEPOSITÓ
    const baseCalculo = Math.min(usdDepositado, usdJugado);
    let nivelAlcanzadoHoy = 0;

    if (baseCalculo >= 6) { nivelAlcanzadoHoy = 3; }
    else if (baseCalculo >= 3) { nivelAlcanzadoHoy = 2; }
    else if (baseCalculo >= 1) { nivelAlcanzadoHoy = 1; }
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // EL PERDÓN (Si fue penalizado pero volvió a jugar)
        ...
        // CÁLCULO ESTRICTO DE LA BARRA
        let visualDay = data.dias;
        let barFillPercentage = (visualDay / 7) * 100; // La barra NO se mueve hasta que se sella el día

        if (!isPenalized && rolloverOk) {
            visualDay = Math.min(7, data.dias + 1); 
            barFillPercentage = (visualDay / 7) * 100; // AHORA SÍ AVANZA LA BARRA
        ...
        if (isPenalized) {
            ...
            // UPSELL Y ADVERTENCIAS
            if (usdDepositado > 0 && !rolloverOk) {
                warnEl.classList.remove('hidden');
                badgeEl.innerHTML = "<span class='text-yellow-400'>⏳ FALTA JUGAR</span>";
                titleEl.innerHTML = `⚠️ ¡JUEGA LO DEPOSITADO PARA ASEGURAR EL DÍA! ⚠️`;
            } else if (rolloverOk) {
                warnEl.classList.add('hidden');
                if (nivelAlcanzadoHoy === 3) {
                    badgeEl.innerHTML = "<span class='text-yellow-400'>🥇 DÍA ORO ASEGURADO</span>";
                    titleEl.innerHTML = `🏆 ¡VAS POR EL PREMIO MAYOR DE $60! ¡MANTÉN EL RITMO! 🏆`;
                } else if (nivelAlcanzadoHoy === 2) {
                    badgeEl.innerHTML = "<span class='text-gray-300'>🥈 DÍA PLATA ASEGURADO</span>";
                    titleEl.innerHTML = `🚀 ¡AUMENTA TU JUEGO A $6 HOY Y GANA EL PREMIO MÁXIMO DE $60! 🚀`;
                } else {
                    badgeEl.innerHTML = "<span class='text-orange-400'>🥉 DÍA BRONCE ASEGURADO</span>";
                    titleEl.innerHTML = `🔥 ¡JUEGA $5 MÁS HOY Y ASEGURA EL PREMIO DE $60 AL DÍA 7! 🔥`;
                }
            } else {
                // Estado 0 absoluto (No ha depositado ni jugado)
                warnEl.classList.add('hidden');
                badgeEl.innerHTML = "<span class='text-gray-400'>ESPERANDO DEPÓSITO...</span>";
                let projected = data.dias > 0 ? (data.nivel_minimo === 3 ? 60 : (data.nivel_minimo === 2 ? 30 : 10)) : 60;
                titleEl.innerHTML = data.dias > 0 
                    ? `🔥 ¡VUELVE A JUGAR HOY PARA SEGUIR POR TUS $${projected}! 🔥`
                    : `🔥 ¡JUEGA DE $1 A $6 DIARIOS Y GANA HASTA $60 GRATIS AL DÍA 7! 🔥`;
            }
        }

        document.getElementById('streak-bar-7days').style.width = barFillPercentage + '%';
        renderStreakDaysGrid(data.dias, isPenalized, rolloverOk, usdDepositado);
    });
}

function renderStreakDaysGrid(diasCompletados, isPenalized, rolloverOk, usdDepositado) {
    ...
    for(let i=1; i<=7; i++) {
        ...
        if (i <= diasCompletados) {
            ...
            // Día en curso (Peleando hoy)
            if (isPenalized) {
                icon = '🚫';
                cssText = 'text-red-500 animate-pulse scale-110';
                subText = 'Perdido';
            } else if (usdDepositado > 0 && !rolloverOk) {
                icon = '⏳';
                cssText = 'text-yellow-400 animate-pulse scale-125';
                subText = 'Falta Jugar';
            } else if (rolloverOk) {
                icon = '✅';
                ...
                icon = '🔥';
                cssText = 'text-orange-500 opacity-80';
                subText = 'Hoy';
            }
        } else if (i === 7) {
            icon = '🎁';
            cssText = 'text-gray-400 opacity-50'; 
        }

        grid.innerHTML += `
            <div class="flex flex-col items-center justify-center w-10 h-12 rounded-lg">
                <div class="text-xl transition-all duration-300 ${cssText}">${icon}</div>
                <span class="text-[8px] font-black mt-1 uppercase ${i <= diasCompletados + 1 && !isPenalized ? 'text-white' : 'text-gray-500'} text-center leading-tight">${subText}</span>
            </div>
        `;
    }
}
