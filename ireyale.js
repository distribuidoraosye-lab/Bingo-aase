...
function updateCountdown() {
    ...
    // Reglas de horario para el reloj (Define la meta más cercana)
    if (now.getHours() < 18) {
...
// 4. ACTUALIZADOR DINÁMICO DE PREMIOS EN UI
...
function evaluateActiveDraw() {
    ...
    // Detectar cuál fue el último sorteo configurado como "En Juego"
    if (tsG > tsE) {
    ...
    if (activeData) {
        ...
        // Poblar cartones activos SOLAMENTE del sorteo que está rodando
        ...
        if(activeData.cartones_snapshot) {
            Object.values(activeData.cartones_snapshot).forEach(c => { 
                if(c.numbers) { 
                    ...
                    c.drawType = currentDrawType; // Marcamos el cartón con el tipo de sorteo actual
...
function updateNextCards() {
    ...
    // Obtenemos la fecha de "hoy" (o mañana si pasaron las 9PM)
    const d = new Date();
    ...
    // Mezclamos TODOS los cartones pendientes para el próximo día
    // 1. Estelares
    Object.values(aprobadosEstelar).forEach(c => { 
        if(c.numbers && c.date === dateStr) { 
            c.numbers = c.numbers.map(Number); 
            c.drawType = 'ESTELAR'; // Los identificamos para renderizarlos distinto
    ...
    Object.values(aprobadosGratis).forEach(c => { 
        if(c.numbers && c.date === dateStr) { 
            c.numbers = c.numbers.map(Number); 
            c.drawType = 'GRATIS'; // Los identificamos de verde
            nextSoldCards.push(c); 
            if(currentUser && c.uid === currentUser.uid) myNextCards.push(c); 
        } 
    });
...
// 6. FUNCIONES DE VIDEO Y REPRODUCCIÓN (Exportadas a Window)
...
function evaluateSystemState() {
    ...
    // Definir si mostramos el RELOJ (NEXT) o el VIDEO (ACTIVE)
    ...
    if (viewMode === 'NEXT') {
        ...
        if (videoId && btnToggleCountdown) {
            ...
            btnToggleCountdown.onclick = window.startVideoPlayback; // Apuntamos directo
        }
        
    } else {
        // MODO ACTIVE (VIDEO EN JUEGO)
        ...
        if (shouldDefaultToNext && isPlayingVideo) {
            if (btnToggle) {
                ...
                btnToggle.onclick = window.backToWaiting;
            }
        } else {
            if (btnToggle) btnToggle.classList.add('hidden');
        }
...
// 8. ESCUCHA DE DATOS (ANTENAS FIREBASE)
function initData() {
...
// 10. DIBUJADO DE RANKING Y PAVOSO
