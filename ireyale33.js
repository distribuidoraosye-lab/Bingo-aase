// ============================================================================
// BINGO TEXIER - SISTEMA INTEGRAL DUAL (ESTELAR + GRATIS) - ireyale3.js
// ============================================================================
...
// --- 1. INYECCIÓN DEL CSS Y HTML ---
const appCSS = `
    body { font-family: 'Inter', sans-serif; background-color: #0f172a; color: white; padding: 0; margin: 0; min-height: 100vh; display: flex; flex-direction: column; align-items: stretch; justify-content: flex-start; }
...
<div class="w-full max-w-md mx-auto flex-1 flex flex-col bg-slate-900 relative">
    <div id="video-container-wrapper" class="relative bg-black border-b border-yellow-500 shadow-lg w-full aspect-video flex items-center justify-center group shrink-0">
        ...
        <div id="live-badge" class="hidden absolute top-2 left-2 z-40 bg-red-600 text-white text-[8px] font-black px-2 py-1 rounded flex items-center gap-1 shadow-lg border border-red-500 tracking-widest">
            <div class="w-1.5 h-1.5 bg-white rounded-full animate-ping"></div> EN VIVO
        </div>
        ...
        <div id="my-cards-area" class="flex-1 flex flex-col">
            ...
            <div id="prizes-waiting-box" class="m-4 bg-gradient-to-b from-gray-900 to-black border-2 border-green-500/80 rounded-xl p-4 shadow-[0_0_20px_rgba(34,197,94,0.2)] relative overflow-hidden float-anim">
                ...
                <div class="relative z-10">
                    ...
                    <div class="grid grid-cols-1 gap-1.5 text-xs font-bold" id="waiting-prizes-list"></div>
                </div>
            </div>
...
// Insertar CSS y HTML al instante sin opacidad
const styleElem = document.createElement('style');
styleElem.innerHTML = appCSS;
document.head.appendChild(styleElem);

document.body.className = ''; // Limpiar el cascarón
document.body.innerHTML = appHTML;

// Conectar el click del blocker del video
const blocker = document.querySelector('.video-blocker');
if(blocker) blocker.addEventListener('click', () => window.togglePause());

// Inyectar API de YouTube si no existe
...
// --- 2. CONFIGURACIÓN FIREBASE Y VARIABLES GLOBALES ---
...
function updateNextCards() {
    ...
    // Carga de cartones combinada para la sala de espera
...
// --- 6. FUNCIONES DE VIDEO Y REPRODUCCIÓN (PARA HTML) ---
    ...
    if (isVoiceEnabled) {
        [audioBingo, audioRonda2, audioRonda3].forEach(a => {
            ...
            if (playPromise !== undefined) {
                playPromise.then(() => { a.pause(); a.currentTime = 0; a.muted = false; }).catch(e => { a.muted = false; });
            }
        });
        if(player && player.getCurrentTime) {
            if (player.getCurrentTime() < 5) { audioRonda1.currentTime = 0; audioRonda1.play().catch(e=>{}); }
        }
    }
    ...
    progressInterval = setInterval(() => {
        if(player && player.getCurrentTime) {
            ...
            if (isPlayingVideo && globalSorteoTimestamp > 0 && dur > 0) {
                if (Date.now() - globalSorteoTimestamp >= dur * 1000) { document.getElementById('live-badge').classList.add('hidden'); }
            }
            checkTimeline(cur);
        }
    }, 300); 
...
// --- 7. ESTADO MAESTRO DE LA PANTALLA ---
...
// --- 9. RENDERIZADO VISUAL DE CARTONES ---
