/* ============================================================
   BINGO TEXIER - MÓDULO EL CORRAL (Corregido)
   ============================================================ */

// Inicialización de variables de Firebase (Seguridad por si faltan otros scripts)
...
// 1. INYECTAR EL HTML (Solo si no existe en el HTML estático)
...
// 2. FUNCIONES DE NAVEGACIÓN (Accesibles globalmente)
window.openCorralSection = function() {
    console.log("Abriendo sección Corral...");
    ...
    const sv = document.getElementById('section-corral'); 
    if(sv) { 
...
window.cerrarCorral = function() {
    const sv = document.getElementById('section-corral'); if(sv) sv.style.display = 'none';
...
if(document.readyState === 'loading') { 
    document.addEventListener('DOMContentLoaded', injectCorralApp); 
} else { 
    injectCorralApp(); 
}
...
function initCorralSystem() {
    ...
    db.ref('corral_game/config').on('value', s => {
        if (s.exists()) {
            corralConfig = s.val();
            const pozoDisp = document.getElementById('corral-pozo-display');
            const rondaDisp = document.getElementById('corral-ronda-display');
            if(pozoDisp) pozoDisp.textContent = corralConfig.pozo;
            if(rondaDisp) rondaDisp.textContent = corralConfig.ronda;
    ...
    if (corralPlayer.status === 'vivo' && corralPlayer.corral === 0) {
        area.innerHTML = `
            <div class="text-center w-full">
                ...
                <p class="text-orange-400 text-xs font-bold mb-6">Ronda ${corralConfig.ronda} • Elige sabiamente</p>
