/* ============================================================
   BINGO TEXIER - MÓDULO HORA LOCA Y SALDO BONO (V1.0 AISLADA)
   - Bóveda de Reto Diario (5 Cartones reales para desbloquear)
   - Billetera "Saldo Bono" exclusiva para Bingo y Torneo Express
   - Expiración estricta a las 8:00 PM
   - Inyección 100% DOM (Cero modificaciones al HTML original)
   ============================================================ */
    ...
    const modalReto = `
        <div id="modal-reto-hl" class="hidden fixed inset-0 flex items-center justify-center" style="display: none; background: rgba(0,0,0,0.95); z-index: 9999999 !important;">
            <div class="bg-white p-6 rounded-2xl w-11/12 max-w-sm text-center shadow-[0_0_50px_rgba(255,255,255,0.2)]">
                <div class="text-6xl mb-2 text-purple-500">&#x1F512;</div>
                ...
                <div class="text-left bg-gray-100 border border-gray-300 rounded-lg p-3 mb-5">
                    <p class="text-xs font-black text-red-600 uppercase mb-2 border-b border-gray-300 pb-1">&#x26A0;&#xFE0F; REGLAS ESTRICTAS:</p>
                    <ul class="text-[11px] text-gray-700 font-bold space-y-2">
                        ...
                        <li><i class="fas fa-exclamation-triangle text-orange-500 mr-1"></i>El Saldo Bono es ÚNICAMENTE para jugar. No se puede retirar.</li>
    ...
    if (boxRef && !document.getElementById('boveda-hl-box')) {
        const bovedaHtml = `
            <div id="boveda-hl-box" style="display: none; background: linear-gradient(135deg, #4c1d95, #2e1065); border: 2px solid #a855f7; border-radius: 0.8rem; padding: 16px; margin-bottom: 20px; box-shadow: 0 10px 20px rgba(0,0,0,0.5); text-align: center;">
                <h3 style="color: #c084fc; font-size: 14px; font-weight: 900; text-transform: uppercase; margin: 0 0 5px 0;">&#x1F512; SALDO BONO ATRAPADO</h3>
...
function initHoraLocaSistema() {
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            // Verificador Diario y de las 8PM
            dbHoraLoca.ref(`users/${hl_uid}/hora_loca_data`).on('value', snap => {
                ...
                if (esDespuesDeLas8PM()) {
                    ...
                    return; // Si es más de las 8, no hay ruleta ni reto
                }

                // Reseteo Diario (Si es un día nuevo)
...
window.iniciarGiroHL = function() {
    ...
    if (rng > 0.98) { valor = 50000; anguloPremio = 360 - 157.5; }
    else if (rng > 0.85) { valor = 10000; anguloPremio = 360 - 67.5; }
    else if (rng > 0.50) { valor = 6000; anguloPremio = 360 - 112.5; }
    else { valor = 3000; anguloPremio = 360 - 22.5; }
...
// 5. REGISTRAR COMPRAS REALES PARA EL RETO (Hook Global)
// Interceptamos Firebase para sumar el progreso cuando el user balance baje.
...
setInterval(() => {
    if(hl_estadoReto === 'active' && typeof userBalance !== 'undefined') {
        ...
        if(userBalance < hl_lastBalance) {
            // Hubo un gasto de dinero real (Bingo o Torneo)
            let diferencia = hl_lastBalance - userBalance;
            // Asumiendo que 1 cartón = aprox. el costo mínimo. Simplificamos: 1 transacción = +1 progreso. 
            // Para ser exactos, sumamos +1 al progreso por cada click exitoso de compra real.
            hl_progresoReto++;
            ...
            hl_lastBalance = userBalance; // Recargas
        }
    }
}, 1000);
...
function interceptarBotonesDePago() {
    // Solo intercepta si tiene saldo bono, si no, usa el flujo nativo
...
function mostrarModalPago(tipoJuego, funcionOriginal, contexto, argumentos) {
    ...
    // Modal Inyectado para decidir con qué pagar
    ...
    const modalHTML = `
        <div id="${idModal}" class="fixed inset-0 flex items-center justify-center bg-black/80" style="z-index: 9999999;">
            <div class="bg-white p-6 rounded-2xl w-11/12 max-w-sm text-center shadow-2xl">
                <h3 class="text-xl font-black text-gray-800 mb-2">&#x1F4B8; MÉTODO DE PAGO</h3>
    ...
    document.getElementById(`btn-pago-real-${idModal}`).onclick = () => {
        ...
        funcionOriginal.apply(contexto, argumentos); // Ejecuta el JS original
    };
...
// 7. PROCESAMIENTO SEGURO DEL SALDO BONO (Bypass al JS viejo)
function procesarPagoConBono(tipoJuego) {
    let costoTicket = 0;
    
    // Buscar costo en el DOM según el juego
    if (tipoJuego === 'bingo') {
    ...
    if (hl_saldoBono < costoTicket) {
        alert("&#x26A0;&#xFE0F; Saldo Bono insuficiente. Elige Saldo Real.");
        return;
    }

    // Restamos el Saldo Bono
    dbHoraLoca.ref(`users/${hl_uid}/hora_loca_data/saldo_bono`).set(hl_saldoBono - costoTicket);

    // Inyectamos el Ticket Directamente a Firebase (Copiando el formato de Texier)
    dbHoraLoca.ref('draw_status/date').once('value').then(snapDate => {
        ...
        let userPhone = "N/A"; // Si tienes la variable global userPhone, úsala.

        if (tipoJuego === 'bingo') {
            // El cartón de Bingo lo extraemos de la variable window (si existe) o generamos array.
            ...
            dbHoraLoca.ref(`bingo_to_grade/estelar/${fechaSorteo}/bets`).push().set({
                ...
                payment_method: "BONO", // Marca para Admin (Gratis)
            ...
            dbHoraLoca.ref(`bets_torneo_express/${fechaSorteo}/${hl_uid}`).push().set({
                ...
                tipo_pago: "SALDO_BONO", // Marca para Admin
