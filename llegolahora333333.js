/* ============================================================
   BINGO TEXIER - MÓDULO HORA LOCA Y SALDO BONO (V3.0 DEFINITIVA)
   - Limpieza automática a la Medianoche (Cambio de fecha)
   - Jerarquía: Se apaga si el usuario es Nuevo (Bono de Bienvenida)
   - Probabilidades en vivo desde el Admin
   ============================================================ */
...
function inyectarInterfazHL() {
            ...
            <div class="bg-white p-6 rounded-2xl w-11/12 max-w-sm text-center shadow-[0_0_50px_rgba(255,255,255,0.2)]">
                ...
                <div class="text-left bg-gray-100 border border-gray-300 rounded-lg p-3 mb-5">
                    ...
                    <ul class="text-[11px] text-gray-700 font-bold space-y-2">
                        ...
                        <li><i class="fas fa-clock text-red-500 mr-1"></i>Si no lo usas HOY, a la medianoche se pierde.</li>
                    </ul>
                </div>
    ...
    const boxRef = document.getElementById('racha-module-container'); // Lo ponemos cerca de la racha
...
function initHoraLocaSistema() {
    dbHoraLoca = firebase.database(); // Inicialización segura por dentro
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            dbHoraLoca.ref('config/hora_loca').on('value', snapAdmin => {
                ...
                // REGLA DE USUARIO NUEVO: Verificamos si tiene el bono de bienvenida
                dbHoraLoca.ref(`users/${hl_uid}/welcome_bonus_status`).once('value', snapBono => {
                    let estadoBienvenida = snapBono.val();
                    // Si es nuevo (tiene bono pending o active), APAGAMOS la Hora Loca para él
                    if (estadoBienvenida === 'pending' || estadoBienvenida === 'active') {
                        document.getElementById('modal-ruleta-hl').style.display = 'none';
                        return; // Se aborta la Hora Loca, lo dejamos concentrado en su Bienvenida
                    }
                    // Si ya se lo gastó o no lo tiene, seguimos normal
...
function revisarEstadoUsuario() {
    dbHoraLoca.ref(`users/${hl_uid}/hora_loca_data`).on('value', snap => {
        ...
        // Limpieza Automática (Si cambió la fecha, se borra lo de ayer)
        if (data.date && data.date !== hl_fechaHoy) {
            if (admin_HoraLocaActiva) {
                ...
                // Si la fecha cambió y el admin lo tiene apagado, solo blanqueamos
                dbHoraLoca.ref(`users/${hl_uid}/hora_loca_data`).set({
...
window.aceptarRetoHL = function() {
    dbHoraLoca.ref(`users/${hl_uid}/hora_loca_data`).update({
        status: 'active', premio: hl_premioAtrapado, progreso: 0
    });
...
function mostrarModalPago(tipoJuego, funcionOriginal, contexto, argumentos) {
    ...
    const modalHTML = `
        <div id="${idModal}" class="fixed inset-0 flex items-center justify-center bg-black/80" style="z-index: 9999999;">
            <div class="bg-white p-6 rounded-2xl w-11/12 max-w-sm text-center shadow-2xl">
                ...
                <button id="btn-pago-real-${idModal}" class="w-full bg-green-500 text-white font-black py-4 rounded-xl shadow-md mb-3">SALDO REAL<br><span class="text-[10px]">Suma a racha</span></button>
                <button id="btn-pago-bono-${idModal}" class="w-full bg-purple-500 text-white font-black py-4 rounded-xl shadow-md mb-3">SALDO BONO (${hl_saldoBono} Bs)<br><span class="text-[10px]">NO suma a racha</span></button>
    ...
    document.getElementById(`btn-pago-real-${idModal}`).onclick = () => {
        document.getElementById(idModal).remove(); funcionOriginal.apply(contexto, argumentos); 
    };
    document.getElementById(`btn-pago-bono-${idModal}`).onclick = () => {
        document.getElementById(idModal).remove(); procesarPagoConBono(tipoJuego);
    };
}
    ...
    dbHoraLoca.ref('draw_status/date').once('value').then(snapDate => {
        ...
        if (tipoJuego === 'bingo') {
            dbHoraLoca.ref(`bingo_to_grade/estelar/${fechaSorteo}/bets`).push().set({
                uid: hl_uid, name: document.getElementById('user-display-name').innerText, phone: "N/A", date: fechaSorteo, status: "pending", payment_method: "BONO", animals: window.currentBingoCard ||
            });
...
// Inicializador retrasado para asegurar que Firebase cargue primero
setTimeout(() => {
