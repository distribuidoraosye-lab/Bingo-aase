...
function addDaysNatively(dateStr, daysToAdd) {
    ...
    // Forzamos creación en UTC para evitar desfases horarrios del dispositivo
...
function initRachaModule() {
    ...
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            // Escuchamos la fecha administrativa de francisca.js
...
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // Validar si la racha se rompió (si no es hoy y no fue ayer)
        ...
        // Si jugó hoy y no se ha registrado el día
        if (diaSalvado && last_active !== currentMasterDate) {
            ...
            // Actualizar el nivel mínimo si hoy jugó más o menos
...
// 5. RENDERIZADO VISUAL (Aquí es donde se "conectan" los candados)
    ...
    rulesBox.innerHTML = `
        <div style="width: 100%; background-color: #111827; border: 1px solid #374151; border-radius: 8px; padding: 10px; font-size: 10px;">
            ...
            <div style="display: flex; flex-direction: column; gap: 4px;">
                ${renderPremioRow(1, "1-2 Cartones", "$10", activeRow)}
                ${renderPremioRow(2, "3-5 Cartones", "$30", activeRow)}
                ${renderPremioRow(3, "6+ Cartones", "$60", activeRow)}
            </div>
...
function renderPremioRow(lvl, texto, monto, activeRow) {
    ...
    return `
        <div style="display: flex; justify-content: space-between; padding: 5px 8px; border-radius: 4px; ${style}">
