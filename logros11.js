...
let bingoRef = null;
let torneoRef = null;
let syncTimer = null; // Variable para el Amortiguador
...
function injectRachaUI() {
    ...
    // SISTEMA DE AUTO-OCULTADO: Se conecta a tu francisca.js de forma segura
    ...
    if (typeof originalSwitchMode === 'function' && !window.switchModePatched) {
        window.switchMode = function(t) {
            ...
            if (rachaBox) {
                if (t === 'bingo' || t === 'animalitos') {
                    rachaBox.style.display = 'none';
                } else {
                    rachaBox.style.display = 'block';
                }
            }
        };
...
// 2. FUNCIÓN DE FECHAS (Protección Total contra años erróneos - Se mantiene la estructura segura)
...
function addDaysSafe(dateStr, daysToAdd) {
    ...
    // Extracción segura por bloques para evitar el error de 1979
...
function initRachaModule() {
    ...
    firebase.auth().onAuthStateChanged(user => {
        if (user) {
            ...
            // SISTEMA ANTI-CACHÉ: Limpia la memoria al cerrar sesión
            ...
            clearTimeout(syncTimer); // Apaga el amortiguador
            const rachaBox = document.getElementById('racha-main-box');
...
function initStreakData(uid) {
    ...
    bingoRef.on('value', s => {
        ...
        s.forEach(c => {
            let b = c.val();
            // La fecha sigue siendo el filtro sagrado
            if (normalizeDate(b.date) === cleanMaster && b.payment_method !== 'GRATIS') count++;
        });
        streakState.ticketsBingo = count;
        
        // EL AMORTIGUADOR (Debounce): Espera medio segundo para agrupar las compras simultáneas de Bingo
    ...
    torneoRef.on('value', s => {
        ...
        // Amortiguador también para el torneo para mantener sincronía
        clearTimeout(syncTimer);
        syncTimer = setTimeout(() => {
            syncAndRender(uid);
        }, 600);
...
// 4. LÓGICA DE PROCESAMIENTO (Segura y con cura de color para el Día 1)
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        if (diaSalvado && last_active !== currentMasterDate) {
            ...
            // CURA BINGO (Día 1): Permite que el color verde suba libremente
            if (dias === 1) {
            ...
            // Sube de nivel durante el mismo día
            if (nivelHoy !== nivel_pico_hoy) {
                nivel_pico_hoy = Math.max(nivel_pico_hoy, nivelHoy);
                updates.nivel_pico_hoy = nivel_pico_hoy;
                
                // CURA BINGO (Día 1): Mantiene el color sincronizado
                if (dias === 1) {
                    nivel_minimo = nivel_pico_hoy;
                    updates.nivel_minimo = nivel_minimo;
                }
