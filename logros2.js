/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V26 - TEXTOS Y FECHAS CORREGIDAS)
...
// 2. UTILIDADES DE FECHA LOCAL (Corrige el bug del mes 01)
...
function addDaysNatively(dateStr, daysToAdd) {
    ...
    if(cleanStr === "00-00-0000") return cleanStr;
    
    let p = cleanStr.split('-');
    
    // Parseo forzado a base 10 para evitar errores con los ceros
    let d = parseInt(p, 10);
    let m = parseInt(p, 10) - 1; // Los meses en JavaScript empiezan en 0
    let y = parseInt(p, 10);
    
    // Fijamos la hora a las 12 PM local para evitar desfases de husos horarios que salten al mes anterior/siguiente
...
// 5. RENDERIZADO VISUAL Y TEXTOS CORREGIDOS
