/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V32 - CORREGIDO)
   Diseño UX limpio con "Día 1", "Día 2" y auto-reparador de DB
   ============================================================ */
...
// 2. FUNCIÓN DE FECHAS CORREGIDA (Arregla el error de 1979/1915)
...
function addDaysSafe(dateStr, daysToAdd) {
    ...
    // CORRECCIÓN: Asignación explícita por índice para evitar años erróneos
    let d = parseInt(p, 10);
...
// 4. LÓGICA DE PROCESAMIENTO (Permite subir nivel por partes hoy)
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        let data = s.val() || { dias: 0, nivel_minimo: 3, last_active: "00-00-0000", fecha_inicio: "00-00-0000", last_day_level: 0 };
        let { dias, nivel_minimo, last_active, fecha_inicio, last_day_level } = data;
        let ayer = addDaysSafe(currentMasterDate, -1);
        let updates = {};

        // Si es un día nuevo, cerramos el nivel del día anterior
        if (dias > 0 && last_active !== currentMasterDate) {
            if (last_active !== ayer) {
                // RESET por inactividad
                dias = 0; nivel_minimo = 3; fecha_inicio = "00-00-0000"; last_day_level = 0;
                updates = { dias: 0, nivel_minimo: 3, last_active: "00-00-0000", fecha_inicio: "00-00-0000", last_day_level: 0 };
            } else {
                // CONTINUACIÓN: El nivel mínimo se actualiza con lo que el usuario terminó ayer
                nivel_minimo = Math.min(nivel_minimo, last_day_level || 3);
        ...
        if (diaSalvado && last_active !== currentMasterDate) {
            ...
            last_day_level = nivelHoy;
            updates.dias = dias;
            updates.last_active = last_active;
            updates.fecha_inicio = fecha_inicio;
            updates.last_day_level = last_day_level;
        } else if (diaSalvado && last_active === currentMasterDate) {
            // Actualizamos el pico de nivel de hoy para que se guarde si el usuario compra más cartones
            if (nivelHoy !== last_day_level) {
                last_day_level = nivelHoy;
                updates.last_day_level = last_day_level;
            }
        }
        ...
        // El nivel que se muestra es el mínimo entre lo acumulado de días pasados y lo que lleva hoy
