/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V33 - BLINDADO)
   Prevención de colapsos (NaN) y niveles dinámicos diarios
   ============================================================ */
...
// 2. FUNCIÓN DE FECHAS (Protección Total contra años locos)
...
// 4. LÓGICA DE PROCESAMIENTO (A prueba de fallos y sin NaN)
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // Extracción blindada para evitar que variables queden undefined
        ...
        if (dias > 0 && last_active !== currentMasterDate) {
            if (last_active !== ayer) {
                ...
                // Nuevo día continuado (Congelamos el nivel de ayer de forma segura)
        ...
        if (diaSalvado && last_active !== currentMasterDate) {
            ...
            // Sube de nivel durante el mismo día sin dañar la base
        ...
        // Ejecuta la actualización en Firebase solo si hay cambios y previene errores asíncronos
        if (Object.keys(updates).length > 0) {
            dbRacha.ref(`users/${uid}/racha_data`).update(updates).catch(err => console.error("Error actualizando racha:", err));
        }
