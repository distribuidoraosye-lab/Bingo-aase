/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V27 - MATEMÁTICA PURA)
   Blindado contra cambios de fecha en el teléfono y errores de iOS
   ============================================================ */
...
// 2. MATEMÁTICA PURA DE FECHAS (Ignora el reloj del iPhone por completo)
function normalizeDate(str) {
    if (!str) return "00-00-0000";
    return String(str).replace(/\//g, '-').trim();
}

function addDaysMath(dateStr, daysToAdd) {
...
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        let ayer = addDaysMath(currentMasterDate, -1);
        let updates = {};
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, nivel_minimo) {
    ...
    for (let i = 0; i < 7; i++) {
        let currentSlotDate = addDaysMath(startPoint, i); // Genera DD-MM-YYYY puro
