/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V28 - SEGURO Y VISIBLE)
   Resolución de fallo de renderizado y blindaje de meses en iOS
   ============================================================ */
...
// 2. FUNCIÓN DE FECHAS SEGURA (Previene colapso y saltos de iOS)
...
function addDaysSafe(dateStr, daysToAdd) {
    ...
    let d = parseInt(p, 10);
    let m = parseInt(p, 10) - 1; // JS lee los meses de 0 a 11
    let y = parseInt(p, 10);
    
    // Forzamos la creación del objeto fecha a las 12 PM (mediodía) 
    // Esto evita que iOS lo pase al día/mes anterior por su zona horaria
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, nivel_minimo) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        // Renderizamos DD/MM debajo del candado
