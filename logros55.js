/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V35 - LÓGICA PASIVA)
   Subida de nivel en tiempo real + Amortiguador + Cero Borrados
   ============================================================ */
...
// 4. LÓGICA DE NIVELES PASIVA (NUNCA BORRA SI NO HAY JUGADA)
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        if (diaSalvado) {
            // EL USUARIO JUGÓ HOY: Aquí sí actualizamos la base de datos
            if (last_active === currentMasterDate) {
                // Ya jugó hoy, solo verificamos si subió de nivel
                if (nivelHoy !== nivel_pico_hoy) {
                    nivel_pico_hoy = Math.max(nivel_pico_hoy, nivelHoy);
                    updates.nivel_pico_hoy = nivel_pico_hoy;
                    if (dias === 1) { nivel_minimo = nivel_pico_hoy; updates.nivel_minimo = nivel_minimo; }
                }
            } else if (last_active === ayer) {
                // Viene en racha del día anterior, sumamos día
                ...
                updates = { dias, last_active, fecha_inicio, nivel_pico_hoy, nivel_minimo };
            } else {
                // Perdió la racha o es primera vez. Empieza de cero.
                dias = 1;
                fecha_inicio = currentMasterDate;
                last_active = currentMasterDate;
                nivel_pico_hoy = nivelHoy;
                nivel_minimo = nivelHoy;
                updates = { dias, last_active, fecha_inicio, nivel_pico_hoy, nivel_minimo };
            }
        } else {
            // EL USUARIO NO TIENE JUGADAS HOY:
            // Regla de oro: NO TOCAR LA BASE DE DATOS PARA NADA DESTRUCTIVO.
            // Así jamás se inyectarán "00-00-0000" ni fechas de 1900 al Admin.
            if (dias > 0 && last_active === ayer) {
                let nuevoMinimo = Math.min(nivel_minimo, nivel_pico_hoy || 3);
                if (nuevoMinimo !== nivel_minimo) {
                    nivel_minimo = nuevoMinimo;
                    updates.nivel_minimo = nivel_minimo;
                }
            }
        }

        // Enviamos las actualizaciones limpias a Firebase
        if (Object.keys(updates).length > 0) {
            dbRacha.ref(`users/${uid}/racha_data`).update(updates);
        }

        // Calculamos qué mostrar en pantalla (Visualmente)
        let diasVisuales = dias;
        if (!diaSalvado && last_active !== currentMasterDate && last_active !== ayer && last_active !== "00-00-0000") {
            // Si hace días que no juega, le mostramos 0 días en pantalla, 
            // pero la BD sigue intacta hasta que haga una jugada nueva.
            diasVisuales = 0; 
        }

        let activeRow = diaSalvado ? nivelHoy : 0; 
        renderVisuals(diasVisuales, last_active, fecha_inicio, diaSalvado, nivelHoy, activeRow);
    });
}
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, activeRow) {
    ...
    if (dias > 0 && last_active !== "00-00-0000") startPoint = addDaysSafe(last_active, -(dias - 1));
    for (let i = 0; i < 7; i++) {
