/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V30 - AUTO-REPARADOR)
   Ignora DB corrupta y calcula fechas matemáticamente hacia atrás
   ============================================================ */
...
// 2. FUNCIÓN DE FECHAS (Permite sumar y restar días con exactitud)
...
// 4. LÓGICA DE PROCESAMIENTO Y AUTO-REPARADOR DE DB
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // AUTO-REPARADOR SILENCIOSO: Si la DB tiene una fecha de inicio contaminada (ej. enero), la reescribe sola calculándola hacia atrás.
        if (dias > 0 && last_active !== "00-00-0000") {
            let expectedInicio = addDaysSafe(last_active, -(dias - 1));
            if (fecha_inicio !== expectedInicio) {
                fecha_inicio = expectedInicio;
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, nivel_minimo) {
    ...
    // AQUÍ ESTÁ LA MAGIA: Calculamos el inicio dinámicamente. Ignoramos cualquier error de la DB.
    ...
    for (let i = 0; i < 7; i++) {
        let currentSlotDate = addDaysSafe(startPoint, i); // Genera DD-MM-YYYY seguro
        ...
        let labelDate = currentSlotDate.substring(0,5).replace('-', '/');
        ...
        grid.innerHTML += `
            <div style="display: flex; flex-direction: column; align-items: center; width: 14%;">
                ...
                <span style="font-size: 8px; margin-top: 6px; color: white; font-weight: bold;">${labelDate}</span>
            </div>
        `;
    }
