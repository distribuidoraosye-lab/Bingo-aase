/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V34 - CORRECCIÓN NIVELES)
   Subida de nivel en tiempo real + Amortiguador + Anti-Caché
   ============================================================ */
...
// 4. LÓGICA DE NIVELES (SUBIDA LIBRE HOY)
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // 🛡️ VACUNA ANTI-AÑOS LOCOS (Cubre 1900 a 1919) 🛡️
        // Si la base de datos tiene una fecha absurda del pasado por el bug viejo, la perdonamos y asumimos que jugó ayer.
        if (last_active.includes("191") || last_active.includes("190")) {
            last_active = ayer;
        ...
        if (diaSalvado && last_active !== currentMasterDate) {
            ...
            updates.dias = dias; updates.last_active = last_active; updates.fecha_inicio = fecha_inicio; updates.nivel_pico_hoy = nivel_pico_hoy;
