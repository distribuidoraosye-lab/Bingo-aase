/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V37 - VISUALES LIBERADOS)
   Lógica Pasiva + Grilla visual independiente de fechas
   ============================================================ */
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        if (diaSalvado) {
            ...
            if (last_active === currentMasterDate) {
                ...
                updates.dias = dias; updates.last_active = last_active; updates.fecha_inicio = fecha_inicio; updates.nivel_pico_hoy = nivel_pico_hoy; updates.nivel_minimo = nivel_minimo;
            } else {
                dias = 1;
            ...
            // EL USUARIO NO TIENE JUGADAS HOY (NO BORRA NUNCA)
        ...
        // Llamamos a pintar la pantalla enviando los días REALES de la base de datos
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, activeRow) {
    ...
    // 🎨 LÓGICA VISUAL LIBERADA: Dibujamos en base a la cantidad de días, sin importar la fecha
