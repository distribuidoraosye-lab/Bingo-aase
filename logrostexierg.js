/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V39 - BLINDADA)
   Lógica Pasiva + Formato de Fechas Estricto + Pase VIP + Niveles
   ============================================================ */
...
// 2. FECHAS BLINDADAS (NORMALIZACIÓN ESTRICTA DE CEROS)
function normalizeDate(str) {
    ...
    // Obligamos a que el día y mes tengan 2 dígitos siempre
    let d = p.padStart(2, '0');
    let m = p.padStart(2, '0');
    let y = p;
    return `${d}-${m}-${y}`;
}
...
// 4. LÓGICA DE NIVELES PASIVA + PASE VIP + NORMALIZACIÓN
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        let last_active = normalizeDate(last_active_raw.replace("_ADMIN", ""));
        ...
        if (diaSalvado) {
            if (last_active === currentMasterDate) {
                ...
                // SUBE RACHA (NATURAL O POR ADMIN)
                ...
                // REINICIO POR SEGURIDAD (Si no es hoy ni ayer ni manual)
                dias = 1;
