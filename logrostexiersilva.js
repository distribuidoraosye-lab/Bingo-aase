...
// 2. FECHAS BLINDADAS (ARREGLO DEL CERO)
function normalizeDate(str) {
    if (!str) return "00-00-0000";
    let clean = String(str).replace(/\//g, '-').trim();
    let p = clean.split('-');
    if (p.length === 3) {
        let d = p.length === 1 ? '0' + p : p;
        let m = p.length === 1 ? '0' + p : p;
        return d + '-' + m + '-' + p;
    }
    return clean;
}

// CORRECCIÓN 1: LOS CORCHETES PARA CALCULAR BIEN LOS DÍAS
