/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V38 - DEFINITIVA)
   Lógica Pasiva + Visuales Liberados + Vacuna en Compra + Niveles + PASE VIP
   ============================================================ */
...
// 2. FECHAS BLINDADAS (CON CERO OBLIGATORIO Y SIN SIMBOLOS PROBLEMÁTICOS)
...
function addDaysSafe(dateStr, daysToAdd) {
    ...
    let extraidoD = p.shift();
    let extraidoM = p.shift();
    let extraidoY = p.shift();
    
    let d = parseInt(extraidoD, 10);
    let m = parseInt(extraidoM, 10) - 1; 
    let y = parseInt(extraidoY, 10);
...
// 4. LÓGICA DE NIVELES PASIVA + VACUNA + NIVELES
function syncAndRender(uid) {
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // CORRECCIÓN 2 Y 3: LECTURA DEL PASE VIP (_ADMIN)
        ...
        // 🛡️ LA VACUNA DEFINITIVA PARA LA LÓGICA 🛡️
        if (last_active.includes("191") || last_active.includes("190") || last_active.includes("189")) {
            last_active = ayer;
            updates.last_active = ayer;
        }

        if (diaSalvado) {
            // EL USUARIO JUGÓ HOY
            if (last_active === currentMasterDate) {
                // YA HABÍA JUGADO HOY MÁS TEMPRANO (Suma cartones)
            ...
            // AQUI SE APLICA EL PASE VIP: "|| esManual"
            } else if (last_active === ayer || esManual) {
                // PRIMERA COMPRA DEL DÍA (Viene en racha O el Admin lo forzó)
                // 1. Respetamos y guardamos el nivel más bajo logrado hasta ayer
            ...
            // NO TIENE JUGADAS HOY (PASIVO: JAMÁS BORRA RACHA)
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, nivelHoy, activeRow) {
    ...
    document.getElementById('streak-bar-7days').style.width = porcentajeBarra + '%';
}
