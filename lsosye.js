/* ============================================================
   BINGO TEXIER - MÓDULO RACHA (NÚCLEO DEFINITIVO V13)
   Lectura de milisegundos, inyección de precios y anti-colapsos.
   ============================================================ */
...
// 1. CÁLCULO MILIMÉTRICO DE LA VENTANA DE HOY (Regla 8 PM)
...
// 2. ESCÁNER DE FECHAS (Adaptado al JSON: request_timestamp y date)
function isRecordFromToday(data, window) {
    if (!data) return false;
    
    // 1. Escaneo por Milisegundos (Infalible para request_timestamp)
    let ts = data.request_timestamp || data.timestamp || data.time;
    if (ts && typeof ts === 'number' && ts > 1000000000) {
        let timeMs = ts < 10000000000 ? ts * 1000 : ts; 
        return (timeMs >= window.startMs && timeMs < window.endMs);
    }
    
    // 2. Escaneo por Texto Directo
    let dateField = data.date || data.fecha;
    if (dateField && typeof dateField === 'string') {
        return dateField.includes(window.dateStr) || dateField.includes(window.altDateStr);
    }
    
    // 3. Red de seguridad profunda
    let strData = JSON.stringify(data);
    return strData.includes(window.dateStr) || strData.includes(window.altDateStr);
}

// 3. PROTECCIÓN DE UI ANTI-CONGELAMIENTOS
...
function safeDisplay(id, show) { let el = document.getElementById(id); if (el) { show ? el.classList.remove('hidden') : el.classList.add('hidden'); } }
...
// 5. MOTOR DE BÚSQUEDA (Lee el JSON a la perfección)
function initStreakMonitors(uid) {
        ...
        if(snap.exists()) {
            snap.forEach(child => {
                ...
                if (status === 'APPROVED' || status === 'APROBADO' || tipo === 'MANUAL_ADMIN' || status === '') {
                    if (isRecordFromToday(data, windowData)) {
    ...
    // LECTOR DE BINGO (Inyección automática de precio)
    ...
    const cbBin = refBin.on('value', snap => {
        ...
        if(snap.exists()) {
            snap.forEach(child => {
                ...
                if (isRecordFromToday(data, windowData) && !isGratis) {
                    let precio = parseFloat(data.precio_carton || data.amount || data.monto);
                    sumaBin += !isNaN(precio) ? precio : configTasa; // Si el JSON no tiene precio, suma los 500
                }
            });
        }
...
// 6. CEREBRO Y GUARDADO EN FIREBASE
function procesarRacha(uid, windowData) {
    try {
        let depActual = rachaState.deposito;
        let jugActual = rachaState.jugadoTotal;
        let cartonesJugados = jugActual / configTasa;

        // Regla matemática: 0.95 desbloquea el candado
        let nivelHoy = 1; // Base 1 para no violar reglas antiguas de Firebase
        ...
        dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
            ...
            // GUARDA SIN VIOLAR LA REGLA DE NIVEL MINIMO
    ...
    } catch (e) { console.error("Error Procesando Racha:", e); }
}
