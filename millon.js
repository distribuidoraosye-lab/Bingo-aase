/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V17 - BLINDADO)
   Anclaje de Tiempo Absoluto, Auto-Reparación y UI de Día 7
   ============================================================ */
...
// 2. HERRAMIENTAS MATEMÁTICAS (Calculadoras Exactas al Mediodía)
function parseDateExact(str) {
    if (!str || typeof str !== 'string' || str.length !== 10) return new Date();
    const parts = str.split('-');
    if(parts.length !== 3) return new Date();
    // Fijamos la hora al mediodía (12:00 PM) para que los husos horarios no salten de día
...
function addDaysExact(str, days) {
    let date = parseDateExact(str);
...
// 4. ACTUALIZACIÓN VISUAL Y LÓGICA DE DÍAS (Con Auto-Reparación)
function updateStreakUI(uid) {
    ...
    let ayerDateStr = addDaysExact(currentMasterDate, -1);
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // === AUTO-LIMPIADOR DE BASE DE DATOS CORRUPTA ===
        if (dias < 0 || dias > 7 || last_active.length !== 10) {
            dias = 0; nivel_minimo = 3; last_active = "00-00-0000";
            updates.dias = 0; updates.nivel_minimo = 3; updates.last_active = "00-00-0000";
            isDirty = true;
        }

        // === 1. Validar si perdió la racha (Si no jugó ayer ni hoy) ===
        ...
        if (diaSalvado && dias < 7) {
            if (last_active !== currentMasterDate) {
                ...
                // Compró más el mismo día (Revisamos si subió de medalla)
                visualDay = dias;
            ...
            // Congelado en 7 esperando premio
            visualDay = 7;
        }
...
// 5. RENDERIZADO (Diseño Impecable)
function renderVisuals(dias, last_active, diaSalvado, visualDay) {
    ...
    if (visualDay === 7 && last_active === currentMasterDate) {
        ...
        // Pinta la caja de reglas con medallas si no está
    ...
    // C. Calcular Origen de la Racha (Anclaje Infalible)
    let day1DateStr = currentMasterDate;
    if (dias > 0) {
        // Encontramos el Día 1 restando desde el último día que el usuario jugó
        day1DateStr = addDaysExact(last_active, -(dias - 1));
    }
    ...
    // Obtenemos los valores absolutos de las fechas para compararlas de forma segura
    const timeCurrentMaster = parseDateExact(currentMasterDate).getTime();

    for(let i=1; i<=7; i++) {
        let slotDateStr = addDaysExact(day1DateStr, i - 1);
        let shortDate = slotDateStr.substring(0, 5).replace('-', '/'); 
        
        let timeSlot = parseDateExact(slotDateStr).getTime();
        ...
        if (dias > 0 && timeSlot < timeCurrentMaster) {
            ...
            // REGALO FINAL (No completado aún)
            icon = '🎁'; 
