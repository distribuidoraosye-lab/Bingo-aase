...
function updateGameDate(){
    ...
    // Escuchar resultados de la fecha SELECCIONADA
...
// --- NUEVAS FUNCIONES DE PAGINACIÓN (SÓLO PARA VISUALIZACIÓN) ---
...
function renderDatePagination() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        btn.className = `flex-shrink-0 px-4 py-1 rounded-full text-[10px] font-bold transition-all border-2 ${currentDateStr === dayStr ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-500 border-gray-200'}`;
        ...
        Object.entries(data).reverse().forEach(([key, b]) => {
            ...
            card.innerHTML = `
                <div>
                    ...
                    <div class="text-[10px]">${b.status === 'WIN' ? '<b class="text-green-600">¡GANASTE!</b>' : (b.status === 'LOST' ? '<span class="text-red-400">No acertado</span>' : '<span class="text-gray-400 font-bold">Pendiente</span>')} Animal: <b>#${b.animal}</b></div>
                </div>
...
// ... EL RESTO DE TUS FUNCIONES (renderLottoGrid, selectLottery, etc.) SIGUEN IGUAL ...
