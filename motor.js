// --- DATOS Y CONFIGURACIÓN ORIGINALES ---
...
let userBalance=0, serverTimeOffset=0, currentCardPrice=0, currentDateStr="", dailyResults={}; 

window.onload=()=>{
    // MANEJO DE AUTH (Restaurado 100% igual)
...
function updateGameDate(){
    ...
    // Si no hay fecha seleccionada aún, ponemos hoy
    if(!currentDateStr) currentDateStr = todayStr;

    // Escuchamos resultados
...
// --- FUNCIONES DE NAVEGACIÓN AGREGADAS AL FINAL ---
...
function loadMyDailyBets() {
    ...
    db.ref(`bets_animalitos/${currentDateStr}/${uid}`).on('value', s => {
        ...
        list.innerHTML = '';
        if(!data) {
            list.innerHTML = `<p class="text-center text-gray-400 text-[10px] py-4">No hay jugadas para el ${currentDateStr}</p>`;
            return;
        }
        ...
        Object.entries(data).reverse().forEach(([key, b]) => {
            ...
            card.innerHTML = `
                <div>
                    <div class="font-bold text-indigo-900 text-xs">${b.lottery} <span class="text-[9px] text-gray-400 font-normal">${b.time}</span></div>
                    <div class="text-[10px] mt-1">${b.status === 'WIN' ? '<b class="text-green-600 uppercase">Ganaste</b>' : (b.status === 'LOST' ? '<span class="text-red-400">No acertado</span>' : '<span class="text-gray-400">Pendiente</span>')} - Animal: <b>#${b.animal}</b></div>
                </div>
                <div class="text-right">
                    <div class="font-bold text-gray-800 text-sm">${b.amount} Bs</div>
                    <div class="text-[9px] bg-gray-100 px-1 rounded mt-1">Res: ${res ? '#'+res : '--'}</div>
                </div>`;
...
// Aquí siguen todas tus otras funciones (placeBet, etc...) igual que las tenías.
