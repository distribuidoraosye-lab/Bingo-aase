...
window.placeBet = async () => {
    ...
    try {
        if(isTripletaMode) {
            ...
            await db.ref(`users/${auth.currentUser.uid}/balance`).transaction(c => (c||0) - cost); await db.ref(`bets_tripletas/${currentDateStr}/${auth.currentUser.uid}`).push({ lottery: selectedLotto, time, animals: Array.from(selectedLottoAnimals), amount: cost, status: 'PENDING', timestamp: firebase.database.ServerValue.TIMESTAMP }); alert("\u00A1Tripleta Jugada!");
        } else if (isDupletaMode) {
            ...
            await db.ref(`users/${auth.currentUser.uid}/balance`).transaction(c => (c||0) - cost); await db.ref(`bets_dupletas/${currentDateStr}/${auth.currentUser.uid}`).push({ lottery: "Dupleta", time, animals: Array.from(selectedLottoAnimals), amount: cost, status: 'PENDING', timestamp: firebase.database.ServerValue.TIMESTAMP }); alert("\u00A1Dupleta Jugada!");
        } else {
...
// --- LOGICA DE RESULTADOS Y ACIERTOS (VISUAL) ---
function calculateHits(ticketType, timeStr, userAnimals) {
    ...
    // DEFINIR QUÉ LOTERÍAS MOSTRAR SEGÚN EL TIPO DE TICKET
