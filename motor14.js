...
// --- SISTEMA CENTRAL: Esto se alimenta de Admin en el futuro ---
let SYSTEM_CONFIG = {
    payment_methods: {
        pago_movil: {
            bank: "Banesco (0134)",
...
// ==========================================
//  LÓGICA DE VERIFICACIÓN AUTOMÁTICA (NUEVO)
// ==========================================
window.verifyPayment = async () => {
    ...
    if(refInput.length !== 5) { alert("La referencia debe tener los últimos 5 dígitos."); return; }
    if(!amountInput || amountInput <= 0) { alert("Monto inválido."); return; }

    // UI: Mostrar "Buscando..."
    btn.disabled = true;
    btn.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i> BUSCANDO PAGO...';
    feedback.classList.remove('hidden');
    feedback.className = "mt-3 text-center p-3 rounded-lg text-sm font-bold bg-gray-100 text-gray-600";
    feedback.textContent = "Conectando con el banco...";
    btnSupport.classList.add('hidden');

    try {
        // 3. Buscar en Firebase (Carpeta: pagos_entrantes)
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => {
                ...
                // REGLAS DE COINCIDENCIA (EL CEREBRO)
                // Buscamos que el mensaje del banco contenga la referencia Y el monto
                const txt = (pago.mensaje_banco || "").toLowerCase();
                const hasRef = txt.includes(refInput); 
                const hasMonto = txt.includes(amountInput.toString().replace('.',',')); // Algunos bancos usan coma
                const hasMontoDot = txt.includes(amountInput.toString()); // Otros punto

                // Solo procesar si NO ha sido usado y coincide Ref + Monto
                if (hasRef && (hasMonto || hasMontoDot) && (!pago.status || pago.status === 'pendiente_revision')) {
                    matchFound = { key: child.key, data: pago };
                }
            });
        }

        if (matchFound) {
            // --- ÉXITO: PAGO ENCONTRADO ---
            
            // A. Marcar como usado para evitar duplicados
            ...
            feedback.innerHTML = `<i class="fas fa-check-circle text-xl block mb-1"></i> ¡PAGO APROBADO!<br>Se acreditaron ${amountInput} Bs.`;
            ...
            setTimeout(() => {
                ...
                btn.disabled = false;
                btn.innerHTML = '<i class="fas fa-search mr-2"></i> VERIFICAR AHORA';
                feedback.classList.add('hidden');
            }, 3000);

        } else {
            // --- FALLO: PAGO NO ENCONTRADO ---
            feedback.className = "mt-3 text-center p-3 rounded-lg text-sm font-bold bg-red-100 text-red-700 border border-red-300";
            feedback.innerHTML = `<i class="fas fa-times-circle text-xl block mb-1"></i> PAGO NO ENCONTRADO<br>Puede tardar unos minutos en llegar el mensaje del banco.`;
            
            // Habilitar botón de WhatsApp con texto pre-cargado
            const msg = `Hola Soporte, hice un pago y no se acreditó.%0A%0A*Ref:* ${refInput}%0A*Monto:* ${amountInput} Bs%0A*Fecha:* ${dateInput}`;
