...
// ==========================================
//  LÓGICA DE VERIFICACIÓN CON ANIMACIÓN MEJORADA (CON FIX ANTI-CRASH)
// ==========================================
    ...
    try {
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => {
                // --- FIX: TRY-CATCH DENTRO DEL BUCLE PARA SALTAR MENSAJES ROTOS ---
                try {
                    const pago = child.val();
                    const txt = (pago.mensaje_banco || "").toLowerCase();
                    
                    // LÓGICA DE COINCIDENCIA (SABUESO)
                    ...
                    if (hasRef && hasMonto && (!pago.status || pago.status === 'pendiente_revision')) {
                        matchFound = { key: child.key, data: pago };
                    }
                } catch(err) {
                    console.log("Mensaje corrupto ignorado en búsqueda");
                }
            });
        }
