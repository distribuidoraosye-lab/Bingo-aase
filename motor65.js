...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Se asegura de actualizar el modal apenas lleguen los datos, no solo al abrirlo
    db.ref('config/payment_methods').on('value', s => {
        if(s.exists()) { 
            SYSTEM_CONFIG.payment_methods = s.val(); 
            // Intentar actualizar si el modal existe
...
function renderPaymentInfo() {
    // Protección extra para evitar errores si SYSTEM_CONFIG no cargó
...
// ==========================================
//  LÓGICA BLINDADA ANTI-ERRORES DE CONEXIÓN
// ==========================================
window.verifyPayment = async () => {
    ...
    try {
        await new Promise(r => setTimeout(r, 1000)); // Pequeña espera para UX

        const snapshot = await db.ref('pagos_entrantes').once('value');
        let matchFound = null;
        let alreadyClaimedByMe = false; // Bandera para falsos negativos

        if (snapshot.exists()) {
            snapshot.forEach(child => {
                // TRY-CATCH CRÍTICO PARA EVITAR QUE UN MENSAJE ROTO DETENGA TODO
                try {
                    const raw = child.val();
                    
                    // Función interna para buscar (Rayos X)
                    const checkMatch = (data, key) => {
                        ...
                        if (data.mensaje_banco) {
                            ...
                            if (hasRef && hasMonto) {
                                // SI COINCIDE, REVISAMOS ESTADO
                                if (!data.status || data.status === 'pendiente_revision') {
                                    return { key: key, data: data, status: 'valid' };
                                }
                                // SI YA FUE RECLAMADO POR MÍ MISMO, ES UN FALSO ERROR DE CONEXIÓN
                    ...
                    if (result) {
                        ...
                        if (result.status === 'already_mine') alreadyClaimedByMe = true;
                    }

                } catch(err) {
                    console.log("Dato corrupto saltado");
                }
            });
        }

        if (matchFound) {
            // CASO 1: PAGO NUEVO ENCONTRADO
        ...
        } else if (alreadyClaimedByMe) {
            // CASO 2: YA LO HABÍA RECLAMADO (EVITAR ERROR FALSO)
            showSuccess(amountInput);
        ...
        console.error(error);
        // Si hay error de red real, mostrar botón de reintentar
        btn.disabled = false;
        btn.className = "w-full bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-3 rounded-lg shadow-lg";
        btn.innerHTML = 'Error de conexión. Toca para reintentar.';
    }
    ...
    function showSuccess(amt) {
        ...
        feedback.innerHTML = `✅ Recarga exitosa de ${amt} Bs.`;
        setTimeout(() => {
    ...
    function showError() {
        ...
        feedback.innerHTML = `⚠️ No encontrado. Verifica el SMS o la referencia.`;
...
// --- FIN CÓDIGO ---
// (Resto de funciones intactas: initTicker, init, startCountdown, etc...)
