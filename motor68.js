...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('recharge-btn').onclick=()=>{
        // Renderizar inmediatamente con lo que tengamos en memoria
        renderPaymentInfo(); 
...
function init(){
    ...
    // --- CARGA INMEDIATA DE DATOS DE PAGO ---
    db.ref('config/payment_methods').on('value', s => {
        if(s.exists()) { 
            SYSTEM_CONFIG.payment_methods = s.val(); 
            // Si el modal está abierto, actualiza visualmente
            const bankEl = document.getElementById('pm-bank');
            if(bankEl && document.getElementById('deposit-modal').style.display === 'flex') {
                renderPaymentInfo();
            }
        }
    });
...
function renderPaymentInfo() {
    // Usamos la variable global en memoria para que sea instantáneo
...
// ==========================================
//  LÓGICA BLINDADA ANTI-FRAUDE Y ANTI-ERROR
// ==========================================
window.verifyPayment = async () => {
    ...
    feedback.innerHTML = "Buscando pago en el banco...";
    ...
    try {
            ...
            snapshot.forEach(child => {
                try {
                    ...
                    if (result) {
                        ...
                        if (result.status === 'already_mine') matchFound = result; // Dejar pasar para mostrar éxito
                        if (result.status === 'fraud') duplicateAttempt = true;
                    }

                } catch(err) { console.log("Skip error"); }
            });
        }
        ...
        if (matchFound) {
            ...
            if (matchFound.status === 'already_mine') {
                // YA LO COBRASTE ANTES, SOLO MUESTRA ÉXITO SIN SUMAR DE NUEVO
                showSuccess(amountInput, "Este pago ya fue acreditado a tu cuenta.");
                return;
            }

            // --- INTENTO DE COBRO ATÓMICO ---
            // 1. Marcar como cobrado PRIMERO
            ...
            try {
                ...
                // SI FALLA SUMAR SALDO, REVERTIMOS EL PAGO PARA QUE INTENTE DE NUEVO (ROLLBACK)
                console.error("Error sumando saldo, revirtiendo...", balanceError);
                await db.ref(`pagos_entrantes/${matchFound.key}`).update({
                    status: 'pendiente_revision',
                    claimed_by: null,
                    claimed_at: null
                });
                throw new Error("Error actualizando saldo. Intenta de nuevo.");
            }

        } else if (duplicateAttempt) {
            showError("REFERENCIA YA USADA.", "Este pago ya fue procesado por otro usuario.");
        } else {
            showError("PAGO NO ENCONTRADO", "Verifica el monto y la referencia del SMS.");
        }
        ...
        btn.innerHTML = 'Error de conexión/permisos. Reintentar.';
    ...
    function showSuccess(amt, text) {
        ...
        feedback.innerHTML = `✅ ${text}<br>Monto: ${amt} Bs.`;
        setTimeout(() => {
...
// --- FIN CÓDIGO ---
// (Resto de funciones intactas: initTicker, startCountdown, etc...)
