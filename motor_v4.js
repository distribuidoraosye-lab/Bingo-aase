...
window.switchMode = function(mode) {
    ...
    if(mode === 'bingo') {
        ...
        // Recargar precio del bingo al entrar
        db.ref('config/bingo_price').once('value', s => { 
            currentCardPrice = s.val() || 0; 
        ...
        if(s.exists()) {
            ...
            s.forEach(c => {
                ...
                if(val.status === 'active') {
                    ...
                    div.innerHTML = `<div class="animal-grid">${val.card.map(n => `<div class="animal-cell">${n}</div>`).join('')}</div>`;
                    container.appendChild(div);
                }
            });
...
window.onload = function() {
    ...
    // Listener Bingo Compra
    document.getElementById('start-purchase-btn').onclick = async () => {
        if(isProcessing) return;
        isProcessing = true;
        try {
            if(!auth.currentUser) throw new Error("Inicia sesión primero");
            if(userBalance < currentCardPrice) throw new Error("Saldo insuficiente");
            
            await db.ref(`users/${auth.currentUser.uid}/balance`).transaction(c => (c||0) - currentCardPrice);
