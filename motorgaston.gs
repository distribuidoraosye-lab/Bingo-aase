...
// ==========================================
//  LÓGICA VERIFICACIÓN Y SALDO (CORREGIDA)
// ==========================================
window.verifyPayment = async () => {
    ...
    btn.innerHTML = '<i class="fas fa-circle-notch fa-spin mr-2 text-xl"></i> PROCESANDO...';
    feedback.classList.remove('hidden');
    feedback.className = "mt-3 text-center p-3 rounded-lg text-xs text-gray-500 font-mono animate-pulse";
    feedback.innerHTML = "Consultando banco...";
    ...
    try {
        ...
        // Obtener fecha de hoy (Día/Mes/Año) para validar
                ...
                try {
                    ...
                    const checkMatch = (data, key) => {
                        ...
                        if (data.mensaje_banco) {
                            ...
                            // VALIDACIÓN DE FECHA: El mensaje debe tener la fecha de hoy
                            // Buscamos que contenga (ej: "24/01") o sea muy reciente
                            // Nota: Si el formato de fecha del banco varía, esto es lo que hay que ajustar.
                            // Por ahora, validamos referencia y monto que es lo más seguro.
                            
                            if (hasRef && hasMonto) {
                                // 1. Pago Nuevo
                                if (!data.status || data.status === 'pendiente_revision') {
                                    return { key: key, data: data, status: 'valid' };
                                }
                                // 2. Pago Mío (Idempotencia)
                                if (data.status === 'claimed' && data.claimed_by === auth.currentUser.uid) {
                                    return { key: key, data: data, status: 'already_mine' };
                                }
                                // 3. Pago Usado por Otro
                                if (data.status === 'claimed' && data.claimed_by !== auth.currentUser.uid) {
                                    return { key: key, data: data, status: 'fraud' };
                                }
                            }
                        }
                        return null;
                    ...
                    if (!result && typeof raw === 'object' && raw !== null) {
                    ...
                    if (result) {
                        if (result.status === 'valid') matchFound = result;
                        if (result.status === 'already_mine') matchFound = result; 
                        if (result.status === 'fraud') duplicateAttempt = true;
                    }
                } catch(err) { console.log("Skip"); }
            });
        }
        ...
        if (matchFound) {
            ...
            if (matchFound.status === 'already_mine') {
                showSuccess(amountInput, "Este pago ya fue acreditado a tu cuenta.");
                return;
            }

            // INTENTO DE ESCRITURA EN DB (Si falla aquí, es por reglas de seguridad)
            try {
                // 1. MARCAR REFERENCIA (Si esto falla, salta al catch)
                ...
                // 2. SUMAR SALDO (Solo si el paso 1 funcionó)
                await db.ref(`users/${auth.currentUser.uid}/balance`).transaction(current => {
                    return (current || 0) + amountInput;
                });

                showSuccess(amountInput, "Recarga exitosa.");

            } catch (writeError) {
                console.error("Error escritura:", writeError);
                // Si llegamos aquí, es que alguien más ganó la carrera o hay error de red
                showError("ERROR AL PROCESAR", "No se pudo reclamar el pago. Puede que ya haya sido usado.");
            }

        } else if (duplicateAttempt) {
            showError("REFERENCIA YA USADA", "Este pago ya fue procesado.");
        } else {
            showError("PAGO NO ENCONTRADO", "Verifica la referencia y el monto.");
        }
        ...
        btn.innerHTML = 'Error de conexión. Reintentar.';
    }
    ...
    function showError(title, desc) {
        ...
        feedback.innerHTML = `⚠️ ${title}<br><span class="text-xs font-normal">${desc}</span>`;
        
        // WhatsApp actualizado
        const msg = `Soporte: Pagué y no se acreditó.%0A%0A*Ref:* ${refInput}%0A*Monto:* ${amountInput}`;
        btnSupport.href = `https://wa.me/584220153364?text=${msg}`;
        btnSupport.classList.remove('hidden');
    }
};

// ... RESTO DEL CÓDIGO (initTicker, etc) ...
