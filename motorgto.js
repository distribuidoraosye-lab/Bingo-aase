...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- CARGA INMEDIATA DE DATOS (PRIORIDAD ALTA) ---
    ...
    document.getElementById('recharge-btn').onclick=()=>{
        // Llama a renderPaymentInfo al abrir, usando los datos ya cargados
        renderPaymentInfo(); 
...
function init(){
    ...
    // Configuración duplicada aquí por seguridad para cargar datos si se recarga la página
    db.ref('config/payment_methods').on('value', s => {
...
// ==========================================
//  LÓGICA BLINDADA ANTI-FRAUDE, ANTI-ERROR Y FECHA
// ==========================================
window.verifyPayment = async () => {
    ...
    try {
        ...
        // --- VALIDACIÓN DE FECHA: SOLO PAGOS DE LAS ÚLTIMAS 24H ---
        ...
        if (snapshot.exists()) {
            snapshot.forEach(child => {
                try {
                    ...
                    // Verificar si el pago es reciente
                    const msgTime = parseFloat(child.key);
                    // Si el mensaje es viejo, lo ignoramos
        ...
        if (matchFound) {
            ...
            // --- INTENTO DE COBRO ATÓMICO ---
            try {
                // 1. INTENTAR MARCAR COMO USADO (EL CANDADO)
                // Si esto falla es porque Firebase bloqueó la escritura (Regla de Seguridad)
                ...
                // 2. SOLO SI EL PASO 1 FUNCIONA, SUMAMOS SALDO
                ...
                // Si cae aquí, es porque la regla de seguridad dijo "ACCESO DENEGADO"
                // lo que significa que el estado YA ERA 'claimed'.
                showError("REFERENCIA YA USADA", "El sistema detectó que este pago ya fue procesado.");
            }

        } else if (duplicateAttempt) {
            showError("REFERENCIA YA USADA", "Este pago ya fue procesado por otro usuario.");
        } else {
            showError("PAGO NO ENCONTRADO O VENCIDO", "Verifica monto y referencia. Solo pagos de hoy.");
        }
