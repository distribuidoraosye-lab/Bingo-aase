/* ============================================================
   BINGO TEXIER - MÓDULO VIP 7 RAYOS (7rayos.js)
   ============================================================ */

// 1. INYECCIÓN DINÁMICA DEL HTML VIP
...
if(document.readyState === 'loading') { 
    document.addEventListener('DOMContentLoaded', injectVipHTML); 
} else { 
    injectVipHTML(); 
}
...
window.openVipSection = function() {
    ...
    if(sv) { 
        ...
        if(typeof initVIPSystem === 'function' && firebase.auth().currentUser) {
            initVIPSystem();
        }
    }
};

// 3. LÓGICA DE BASE DE DATOS Y PAGOS
...
window.initVIPSystem = function() {
    ...
    db.ref('vip_config').on('value', s => {
        if (s.exists()) {
            vipConfig = s.val();
            updateVipUIConfig();
            listenToTodayTicket(db);
            listenToMyVipBet(db, uid);
            listenToVipResults(db);
        }
    });
...
firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
        setTimeout(window.initVIPSystem, 1500); 
    }
});

function listenToTodayTicket(db) {
...
function listenToMyVipBet(db, uid) {
    if (!vipConfig.fecha_activa) return;
    db.ref(`bets_vip/${vipConfig.fecha_activa}/${uid}`).on('value', s => {
...
function listenToVipResults(db) {
    if (!vipConfig.fecha_activa) return;
...
function updateVipUIConfig() {
    ...
    if (elRate) elRate.textContent = parseFloat(vipConfig.tasa).toFixed(2);
}
...
function renderVipTicketUI() {
    ...
    if (!myDailyVipBet) {
        ...
        if (sellarBtn) {
            ...
            if (isLockedTime) {
                ...
                sellarBtn.className = "w-full bg-gray-500 text-gray-300 font-black py-4 rounded-xl cursor-not-allowed text-sm flex justify-center items-center tracking-wide uppercase";
                sellarBtn.innerHTML = '<i class="fas fa-lock mr-2"></i> TIEMPO AGOTADO (7:50 AM)';
            } else {
                sellarBtn.disabled = false;
                sellarBtn.className = "w-full bg-gradient-to-r from-yellow-400 to-yellow-600 hover:from-yellow-300 hover:to-yellow-500 text-black font-black py-4 rounded-xl shadow-[0_0_20px_rgba(255,215,0,0.3)] transition transform active:scale-95 text-sm flex justify-center items-center tracking-wide uppercase";
    ...
    const buildCol = (title, imgPath, dataArray, lotteryKey) => {
        ...
        dataArray.forEach((idAnimal) => {
            ...
            // LA MAGIA ESTÁ AQUÍ: Ahora busca si el idAnimal está en cualquier parte de la lista de ganadores
            let isHit = false;
            if (vipResults && vipResults[lotteryKey]) {
                const ganadores = Object.values(vipResults[lotteryKey]);
                isHit = ganadores.includes(idAnimal);
            }
            
            let aInfo = { i: '\u2753', n: '?' }; 
            if(typeof ANIMAL_MAP_LOTTO !== 'undefined' && ANIMAL_MAP_LOTTO[idAnimal]) {
                aInfo = ANIMAL_MAP_LOTTO[idAnimal];
            }
...
window.procesarSelloVIP = async () => {
    ...
    if (typeof userBalance !== 'undefined' && userBalance < costoBs) {
        return alert(`\u274C Saldo insuficiente. Necesitas Bs ${costoBs.toFixed(2)}.`);
    }
    ...
    try {
        ...
        if(btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fas fa-stamp mr-2 text-lg"></i> SELLAR TICKET HOY ($1)`;
        }
    } finally {
...
function evaluateVipPrizes() {
    ...
    ['lotto', 'granjita', 'selva'].forEach(lotteryKey => {
        if (currentVipTicket[lotteryKey] && vipResults[lotteryKey]) {
            // AQUÍ TAMBIÉN CORREGIMOS LA LÓGICA DE BÚSQUEDA
            const ganadores = Object.values(vipResults[lotteryKey]);
            currentVipTicket[lotteryKey].forEach((idAnimal) => {
                if (ganadores.includes(idAnimal)) hits++;
            });
        }
    });
