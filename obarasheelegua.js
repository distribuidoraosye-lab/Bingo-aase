...
function init(){
    ...
    // --- LEER CONFIGURACIÓN DEL TORNEO EXPRESS AQUÍ (CON PERMISOS) ---
    db.ref('config/torneo_express').on('value', s => {
        if(s.exists()) {
            torneoConfig = s.val();
            // Asegurarse de que exista el porcentaje, por defecto 100
            ...
            // Actualizar el cálculo del pozo si cambia el porcentaje o configuración
            listenTorneoPot();
        }
    });
