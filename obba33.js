/* ============================================================
   ...
   Versión: 2.1 (Emojis Integrados, Scroll Arreglado, Reglas Claras)
   ============================================================ */
...
// --- CONSTANTES Y MAPA DE ANIMALITOS INTEGRADO (Para no depender de otros archivos) ---
...
function inyectarInterfazJugadasHora() {
    setTimeout(() => {
        ...
        // B. Inyectar Modales con SCROLL CORREGIDO (pb-28)
            ...
            const modalHtml = `
                <div id="modal-jugadas-hora" class="hidden fixed inset-0 bg-black/90 z-[999999] flex items-center justify-center">
                    <div class="bg-gray-100 w-full max-w-md h-[90vh] md:h-[85vh] rounded-2xl flex flex-col relative overflow-hidden shadow-2xl">
                        ...
                        <div class="bg-green-50 p-3 border-b border-green-200 text-center z-10 shadow-inner">
                        ...
                        <!-- AQUI ESTÁ LA CORRECCIÓN DEL SCROLL (pb-28) -->
                        <div id="jh-horas-list" class="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar bg-gray-100 pb-28">
...
window.abrirPanelJugadasHora = () => {
    // Ya NO bloqueamos si tiene 0, para que pueda leer las reglas y se anime a jugar
...
function renderizarListaHorasJH() {
    ...
    HORAS_TORNEO_JH.forEach(bloque => {
        ...
        // CORRECCIÓN: Si no tiene cartones, mostramos 1 slot con candado para que vea de qué trata
        ...
        for (let i = 0; i < iteraciones; i++) {
            ...
            if (noTieneCartones) {
                ...
                // Diseño normal de usuario con cartones
...
window.guardarJugadasHora = async () => {
    // Validar que tenga cartones antes de intentar guardar en la DB
