/* ============================================================
   ...
   Independiente y Blindado - Basado en el patrón del Módulo de Socios
   ============================================================ */
...
// Calculador de fecha autónomo (Idéntico a tu lógica de saltar a las 6PM)
...
function inyectarInterfazJugadasHora() {
    // Retraso intencional de 1.5s para asegurar que tu HTML base ya existe
    setTimeout(() => {
        // A. Inyectar Banner
        ...
        if (!document.getElementById('modal-jugadas-hora')) {
            const modalHtml = `
                ...
                <div id="modal-selector-animal-jh" class="hidden fixed inset-0 bg-black/80 z-[9999999] flex items-end justify-center sm:items-center">
...
function escucharDatosJugadasHora(user) {
    ...
    // Escuchar cantidad de tickets del Torneo
    ...
    // Escuchar las selecciones por hora
...
// --- CONTROLADORES EXPORTADOS A WINDOW (Para los OnClick) ---
...
function renderizarListaHorasJH() {
    ...
    HORAS_TORNEO_JH.forEach(bloque => {
        ...
        for (let i = 0; i < misCartonesCountJH; i++) {
            const pickNum = picksUsuario[i]; 
            
            if (pickNum) {
                // Caemos atrás de forma segura si ANIMAL_MAP_LOTTO no existe localmente
                const animalInfo = (window.ANIMAL_MAP_LOTTO && window.ANIMAL_MAP_LOTTO[pickNum]) ? window.ANIMAL_MAP_LOTTO[pickNum] : {i:'❓', n:'?'};
                slotsHtml += `
        ...
        // Evitar error si LOTTO_IMGS no ha cargado
        const imgSrc = (window.LOTTO_IMGS && window.LOTTO_IMGS['Lotto Activo']) ? window.LOTTO_IMGS['Lotto Activo'] : 'https://bingotexier.com/archivos/imagenes/b619cd4a138baf5860c9c2250f9fe9d78ca2abd3.png';
...
window.abrirSelectorAnimalJH = (horaStr, slotIndex, estaBloqueada) => {
    ...
    keys.forEach(k => {
        const a = (window.ANIMAL_MAP_LOTTO && window.ANIMAL_MAP_LOTTO[k]) ? window.ANIMAL_MAP_LOTTO[k] : {n:`Num ${k}`, i:"❓"};
...
window.cerrarSelectorAnimalJH = () => {
    document.getElementById('modal-selector-animal-jh').classList.add('hidden');
    slotActivoEdicionJH = null;
};
...
window.seleccionarAnimalParaSlotJH = (animalNum) => {
    ...
    jugadasLocalesJH[hora][index] = animalNum;
