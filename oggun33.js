...
window.abrirAnuncio = (adId) => {
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        hostedVideoPlayer.addEventListener('pause', () => {
            // Evitamos que muestre "PAUSADO" si en realidad ya se terminó el video
        ...
        // Seguro anti-errores por si el navegador no avisa bien cuando sale del buffering
        hostedVideoPlayer.addEventListener('timeupdate', () => {
            if (!hostedVideoPlayer.paused && hostedVideoPlayer.readyState >= 3) {
                if (!overlayPlay.classList.contains('hidden')) {
                    overlayPlay.classList.add('hidden');
                    txtPausa.classList.add('hidden');
                    iniciarTemporizadorAnuncio();
                }
...
// --- NÚCLEO ANTI-TRAMPAS REFORZADO DEL CONTADOR ---
function iniciarTemporizadorAnuncio() {
    ...
    timerAnuncio = setInterval(() => {
        // DOBLE VERIFICACIÓN FÍSICA: Si el video está activo, pero en pausa o cargando, NO descuenta.
        if (anuncioActivo) {
            if (anuncioActivo.type === 'hosted_video' && hostedVideoPlayer) {
                if (hostedVideoPlayer.paused || hostedVideoPlayer.readyState < 3) {
                    return; // Aborta este segundo, el contador no se mueve
                }
            } else if (anuncioActivo.type === 'youtube' && ytAdPlayer) {
                if (typeof ytAdPlayer.getPlayerState === 'function' && ytAdPlayer.getPlayerState() !== YT.PlayerState.PLAYING) {
                    return; // Aborta este segundo
                }
            }
        }
...
// --- ANTI TRAMPA INTELIGENTE (Cambio de Pestaña) ---
function configurarAntiTrampas() {
    document.addEventListener("visibilitychange", () => {
        ...
        if (modal && modal.style.display === 'flex' && document.hidden && !esMisionSocial) {
            ...
            if (hostedVideoPlayer) {
                hostedVideoPlayer.pause(); // Poner pausa automática al video si cambian de pestaña
            }
