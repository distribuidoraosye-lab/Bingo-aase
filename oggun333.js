...
let lastVideoTime = -1; // NUEVA VARIABLE: Rastreador físico de tiempo
...
window.abrirAnuncio = (adId) => {
    ...
    lastVideoTime = -1; // Reset de seguridad
    ...
    // Limpieza estricta de reproductores y temporizadores
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        container.innerHTML = `
            <div class="w-full h-full flex flex-col items-center justify-start overflow-y-auto pt-4 pb-12">
                ...
                <div class="w-full max-w-sm max-h-[70vh] aspect-[9/16] bg-black rounded-xl overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.8)] relative flex justify-center items-center">
                    <video id="html5-hosted-video" src="${anuncioActivo.media_url}" class="w-full h-full object-contain" playsinline preload="metadata" controlsList="nodownload noplaybackrate"></video>
                </div>
            </div>
        `;
        ...
        // Ya no detenemos el reloj en 'waiting' o 'pause', el guardián de abajo se encarga de todo
...
function iniciarTemporizadorAnuncio() {
    ...
    timerAnuncio = setInterval(() => {
        // VERIFICACIÓN ESTRICTA: El contador es un guardián que analiza el reproductor cada segundo
        if (anuncioActivo) {
            
            if (anuncioActivo.type === 'hosted_video' && hostedVideoPlayer) {
                let currentTime = hostedVideoPlayer.currentTime;
                
                // Si está pausado, o no ha empezado (0), o el tiempo NO ha avanzado (buffering)
                if (hostedVideoPlayer.paused || currentTime === 0 || currentTime === lastVideoTime) {
                    
                    // Si el video está intentando rodar pero se quedó congelado (Buffering real)
                    ...
                    return; // ⛔ ABORTA ESTE SEGUNDO, NO DESCUENTA NADA
                }
                
                // Si llegó aquí, el video SI avanzó físicamente. Actualizamos memoria.
                lastVideoTime = currentTime;
                
                // Ocultamos el cartel de carga si estaba visible
                const overlayPlay = document.getElementById('overlay-play-anuncio');
                const txtPausa = document.getElementById('txt-pausa-advertencia');
        ...
        // 👇 SOLO SI PASÓ LAS PRUEBAS INFALIBLES DE ARRIBA, EL RELOJ DESCUENTA
        tiempoRestante--;
...
function configurarAntiTrampas() {
    document.addEventListener("visibilitychange", () => {
        ...
        if (modal && modal.style.display === 'flex' && document.hidden && !esMisionSocial) {
            // No detenemos el temporizador, dejamos que el guardián haga su trabajo
            const overlayPlay = document.getElementById('overlay-play-anuncio');
            const txtPausa = document.getElementById('txt-pausa-advertencia');
