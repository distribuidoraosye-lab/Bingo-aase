...
let lastVideoTime = -1; // Rastreador físico de tiempo
...
window.abrirAnuncio = (adId) => {
    ...
    // Restaurar estilo normal del botón por defecto
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        // preload="auto" para acelerar carga
        container.innerHTML = `
            <div class="w-full h-full flex flex-col items-center justify-start overflow-y-auto pt-4 pb-12">
                <p class="text-yellow-400 font-black mb-3 animate-pulse text-sm text-center uppercase drop-shadow-md">
        ...
        // NÚCLEO ANTI-SILENCIO: Fuerza el volumen y evita que lo muteen
        ...
        btnForzarPlay.onclick = () => {
            if (hostedVideoPlayer) {
                // UI: Transformar el botón en un spinner bloqueado instantáneamente
                ...
                hostedVideoPlayer.play().catch(e => {
                    ...
                    // Si falla por permisos del navegador, regresar al botón original
                    btnForzarPlay.disabled = false;
                    btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
                    iconoPlay.className = "fas fa-play mr-3 text-2xl";
        ...
        hostedVideoPlayer.addEventListener('playing', () => {
            ...
            // Restaurar botón por detrás para el futuro
            btnForzarPlay.disabled = false;
            btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
            iconoPlay.className = "fas fa-play mr-3 text-2xl";
...
function iniciarTemporizadorAnuncio() {
    ...
    lastVideoTime = -1; // Reseteamos la memoria temporal
    ...
    timerAnuncio = setInterval(() => {
        ...
        if (anuncioActivo) {
            ...
            if (anuncioActivo.type === 'hosted_video' && hostedVideoPlayer) {
                ...
                // Si está pausado, no ha empezado o no avanza el tiempo (buffering)
                if (hostedVideoPlayer.paused || currentTime === 0 || currentTime === lastVideoTime) {
                    
                    // Si intenta avanzar pero está trabado por internet lento
                ...
                // Si llegó aquí, el video avanza
                lastVideoTime = currentTime;
                
                // Quitar cartel y restaurar botón oculto si quedó atrapado en buffering visual
                const overlayPlay = document.getElementById('overlay-play-anuncio');
                const txtPausa = document.getElementById('txt-pausa-advertencia');
        ...
        // RELOJ DESCUENTA SI TODO ESTÁ EN REPRODUCCIÓN REAL
        tiempoRestante--;
