/* ============================================================
   BINGO TEXIER - FRONTEND BINGO GRATIS (EMBUDO)
   Archivo: bingo_gratis_front.js
   Funciones: Inyección de UI, Control de Anuncios, Selección de Cartón
   ============================================================ */

document.addEventListener('DOMContentLoaded', () => {
    setTimeout(iniciarMotorGratis, 1500); // Esperamos que francisca.js cargue auth y db
});
...
// Mapa local para no depender del archivo principal si falla
...
function iniciarMotorGratis() {
    ...
    console.log("Iniciando Embudo Bingo Gratis...");
...
function inyectarBotonesYVistas() {
    // 1. INYECTAR BOTÓN EN EL MENÚ PRINCIPAL
    ...
    if (gameSelector && !document.getElementById('btn-bingo-gratis-menu')) {
        ...
        btnGratis.innerHTML = `
            <span class="absolute top-0 right-0 bg-yellow-400 text-red-900 text-[10px] font-black px-3 py-1 rounded-bl-lg animate-pulse">GANA $100 HOY</span>
            <div class="flex items-center justify-center gap-3">
                ...
                <div class="text-left">
                    ...
                    <span class="text-xs font-bold text-green-200">Gana cartones viendo anuncios</span>
                </div>
            </div>
        `;
...
function escucharControlTorneo() {
    db.ref('config/bingo_gratis/show_torneo').on('value', snap => {
        ...
        // Buscamos el botón original del torneo (que llama a switchMode('animalitos'))
...
function escucharAnunciosYCreditos() {
    ...
    // Escuchar créditos gratuitos del usuario
    ...
    db.ref('config/bingo_gratis/ads').on('value', snap => {
        ...
        if(!snap.exists()) {
            container.innerHTML = '<p class="text-center text-xs text-gray-400 font-bold py-4">No hay misiones disponibles por ahora.</p>';
            return;
        }
        ...
        snap.forEach(child => {
            const ad = child.val();
            if(ad.active) {
                adsDisponibles.push({ id: child.key, ...ad });
            }
        });
        ...
        adsDisponibles.forEach(ad => {
            ...
            div.innerHTML = `
                <div class="flex items-center gap-3">
                    <i class="fab ${icon} text-2xl"></i>
                    <div>
                        <h4 class="font-bold text-gray-800 text-sm leading-tight">${ad.title}</h4>
                        <p class="text-[10px] text-green-600 font-black uppercase tracking-widest mt-0.5">+${ad.reward} CARTÓN GRATIS</p>
                    </div>
                </div>
                <button onclick="abrirAnuncio('${ad.id}')" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg text-xs shadow transition active:scale-95">
                    VER
                </button>
            `;
...
function abrirAnuncio(adId) {
    ...
    const timerTxt = document.getElementById('timer-anuncio-txt');
    ...
    if(anuncioActivo.type === 'popup') {
        container.innerHTML = `
            <a href="${anuncioActivo.target_link}" target="_blank" class="w-full h-full flex flex-col justify-center items-center">
                <img src="${anuncioActivo.media_url}" class="max-w-full max-h-[80vh] object-contain">
                <p class="text-white mt-4 font-bold bg-blue-600 px-4 py-2 rounded-full animate-bounce"><i class="fas fa-external-link-alt mr-2"></i> Toca la imagen para visitar</p>
            </a>
        `;
    } else {
        // Iframe para YouTube o TikTok (Forzando autoplay y controles ocultos si es YT)
        let embedUrl = anuncioActivo.media_url;
        if(anuncioActivo.type === 'youtube' && !embedUrl.includes('embed')) {
            // Convertir link normal a embed básico
        ...
        container.innerHTML = `<iframe src="${embedUrl}" class="w-full h-full border-0 pointer-events-none" allow="autoplay; encrypted-media" allowfullscreen></iframe>`;
    }
...
window.reclamarRecompensaAnuncio = async () => {
    ...
    try {
        const uid = auth.currentUser.uid;
        // Sumar créditos
        await db.ref(`users/${uid}/creditos_bingo_gratis`).transaction(c => (c || 0) + anuncioActivo.reward);
...
function renderizarGridAnimalesGratis() {
    ...
    for (let i = 1; i <= 25; i++) {
        ...
        // Estilos clónicos pero en verde
...
window.guardarCartonGratuito = async () => {
    ...
    try {
        ...
        await db.ref(`users/${uid}/creditos_bingo_gratis`).transaction(c => {
            if ((c || 0) <= 0) return; // Abortar si no tiene saldo
            return c - 1;
        });
        ...
        // Éxito visual
        document.getElementById('area-armado-carton').innerHTML = `
            <div class="text-center py-6">
                <i class="fas fa-check-circle text-6xl text-emerald-500 mb-4 animate-bounce"></i>
                <h3 class="text-2xl font-black text-gray-800 uppercase">\u00A1Cartón Registrado!</h3>
                <p class="text-gray-500 font-bold mb-6">ID: #${randomId}</p>
                <button onclick="location.reload()" class="bg-gray-800 hover:bg-black text-white font-bold py-3 px-8 rounded-xl shadow-lg transition">VOLVER AL INICIO</button>
            </div>
        `;
