...
window.abrirAnuncio = (adId) => {
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        document.getElementById('txt-btn-forzar-play').innerText = "HABILITAR VIDEO Y EMPEZAR";
        const ttId = parseTikTokId(anuncioActivo.media_url);
        
        // Iframe Vertical (9:16)
        container.innerHTML = `
            <iframe src="https://www.tiktok.com/embed/v2/${ttId}" class="w-full max-w-sm aspect-[9/16] rounded-xl shadow-2xl border-0 bg-black" allow="fullscreen"></iframe>
            <div id="bloqueador-transparente" class="absolute inset-0 z-20 bg-transparent"></div>
        `;
        
        btnForzarPlay.onclick = () => {
            overlayPlay.classList.add('hidden');
            const bloqueador = document.getElementById('bloqueador-transparente');
            if(bloqueador) bloqueador.classList.add('hidden'); // Quita el bloqueador para dejarle dar play nativo
            iniciarTemporizadorAnuncio();
        };
    ...
    try {
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                // RESETEO SUAVE DE INTERFAZ (Vuelve al inicio de la pestaña gratis)
                seleccionCartonGratis.clear();
                document.getElementById('area-armado-carton').classList.add('hidden');
