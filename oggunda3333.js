    ...
    if(!document.getElementById('modal-anuncio-estricto')) {
        const modalHtml = `
            <div id="modal-anuncio-estricto" class="fixed inset-0 bg-black z-[99999] hidden flex-col justify-between">
                ...
                <div class="flex-1 relative flex items-center justify-center bg-black/95 p-4">
                    <div id="contenedor-media-anuncio" class="w-full h-full flex flex-col justify-center items-center relative"></div>
                    
                    <!-- Botón Flotante Obligatorio -->
                    <div id="overlay-play-anuncio" class="absolute inset-0 z-40 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center hidden">
                        <button id="btn-forzar-play" class="bg-red-600 hover:bg-red-500 text-white font-black py-4 px-8 rounded-full shadow-[0_0_20px_rgba(220,38,38,0.8)] animate-pulse text-sm sm:text-lg tracking-widest uppercase flex items-center transition transform active:scale-95">
                            <i class="fas fa-play mr-3 text-2xl"></i> <span id="txt-btn-forzar-play">DALE PLAY PARA INICIAR</span>
                        </button>
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        container.innerHTML = `
            <div class="w-full max-w-3xl aspect-video bg-black shadow-2xl rounded-lg overflow-hidden relative">
                <!-- Bloqueo total de clics en YT operado solo por API -->
        ...
        document.getElementById('txt-btn-forzar-play').innerText = "HABILITAR REPRODUCTOR";
        const ttId = parseTikTokId(anuncioActivo.media_url);
        
        // Iframe Vertical (9:16) con aviso llamativo integrado, sin bloqueador transparente
        container.innerHTML = `
            <div class="w-full max-w-sm flex flex-col items-center">
                <div id="aviso-play-tiktok" class="hidden mb-3 bg-yellow-400 text-black font-black px-4 py-3 rounded-lg shadow-[0_0_15px_rgba(250,204,21,0.5)] animate-bounce text-center uppercase tracking-wide text-xs w-full">
                    <i class="fas fa-hand-pointer mr-1 text-red-600 text-lg"></i> \u00A1DALE PLAY AL VIDEO DE TIKTOK AHORA!
                </div>
                <div class="relative w-full aspect-[9/16] rounded-xl overflow-hidden shadow-2xl bg-black border border-gray-700">
                    <iframe src="https://www.tiktok.com/embed/v2/${ttId}" class="absolute inset-0 w-full h-full border-0" allow="fullscreen"></iframe>
                </div>
            </div>
        `;
        
        btnForzarPlay.onclick = () => {
            overlayPlay.classList.add('hidden'); // Oculta el fondo oscuro principal
            const aviso = document.getElementById('aviso-play-tiktok');
            if(aviso) aviso.classList.remove('hidden'); // Muestra el mensaje amarillo rebotando
            iniciarTemporizadorAnuncio(); // Empieza el contador de 15s
        };

    } else if (anuncioActivo.type === 'popup') {
        document.getElementById('txt-btn-forzar-play').innerText = "ABRIR ENLACE Y EMPEZAR";
        
        container.innerHTML = `<img src="${anuncioActivo.media_url}" class="max-w-full max-h-[80vh] object-contain rounded-xl shadow-2xl relative z-10">`;
        
        btnForzarPlay.onclick = () => {
            window.open(anuncioActivo.target_link, '_blank');
            overlayPlay.classList.add('hidden');
            iniciarTemporizadorAnuncio();
        };
    }
...
window.guardarCartonGratuito = async () => {
    ...
    try {
        ...
        // CONFIRMACIÓN LIMPIA SIN REDIRECCIÓN
