...
window.abrirAnuncio = (adId) => {
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        // Widget OFICIAL de TikTok con aviso simplificado
        container.innerHTML = `
            <div class="w-full max-w-sm flex flex-col items-center bg-white rounded-xl overflow-hidden relative h-[70vh] overflow-y-auto shadow-2xl">
                <div id="aviso-play-tiktok" class="hidden w-full bg-yellow-400 text-black font-black px-4 py-3 text-center uppercase tracking-wide text-xs z-50 sticky top-0 shadow-md animate-bounce">
                    \u00A1DALE PLAY AL VIDEO DE TIKTOK!
                </div>
                <blockquote class="tiktok-embed" cite="${anuncioActivo.media_url}" data-video-id="${ttId}" style="max-width: 605px;min-width: 325px; margin: 0;">
                    <section></section>
                </blockquote>
            </div>
        `;
        
        // Inyectamos el script oficial de TikTok
        const oldScript = document.getElementById('tt-embed-script');
        if(oldScript) oldScript.remove();
        const ttScript = document.createElement('script');
        ttScript.id = 'tt-embed-script';
