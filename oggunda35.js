        ...
        const modalHtml = `
            <div id="modal-anuncio-estricto" class="fixed inset-0 bg-black z-[99999] hidden flex-col justify-between">
                ...
                <div class="flex-1 relative flex items-center justify-center bg-black/95 p-4">
                    <div id="contenedor-media-anuncio" class="w-full flex justify-center items-center relative"></div>
...
function renderizarListaMisiones() {
    ...
    adsDisponibles.forEach(ad => {
        // Iconografía dinámica según el tipo de misión
        let icon = 'fa-play-circle text-blue-500';
        if(ad.type === 'tiktok') icon = 'fa-tiktok text-black';
        else if(ad.type === 'youtube') icon = 'fa-youtube text-red-600';
...
window.abrirAnuncio = (adId) => {
    ...
    if (anuncioActivo.type === 'youtube') {
        ...
        overlayPlay.classList.add('hidden'); // Quitar el bloqueador para TikTok
        const ttId = parseTikTokId(anuncioActivo.media_url);
        
        // Iframe nativo puro para TikTok (Estilo TakeApp)
        container.innerHTML = `
            <div class="w-full flex flex-col items-center">
                <p class="text-yellow-400 font-black mb-2 animate-pulse text-xs text-center uppercase">\u25BC Reproduce el video mientras esperas \u25BC</p>
                <div class="w-full max-w-[325px] aspect-[9/16] rounded-xl overflow-hidden shadow-2xl bg-black">
                    <iframe src="https://www.tiktok.com/embed/v2/${ttId}" class="w-full h-full border-0" allow="fullscreen"></iframe>
                </div>
            </div>
        `;
        
        iniciarTemporizadorAnuncio(); // Arranca directo

    } else if (socialMissionTypes.includes(anuncioActivo.type)) {
        // Lógica unificada para todas las redes sociales y popups
        ...
        // Si hay imagen de banner, la muestra. Si no, muestra el icono gigante.
        let mediaHtml = anuncioActivo.media_url 
            ...
            : `<i class="${iconoPlay.className.replace('mr-3 text-2xl', 'text-8xl text-white mb-6')}"></i>`;
...
function configurarAntiTrampas() {
    document.addEventListener("visibilitychange", () => {
        ...
        // Si es una misión social (WhatsApp, Insta, etc), DEBEN irse a otra pestaña, no se les castiga.
        const socialMissionTypes = ['popup', 'whatsapp', 'instagram', 'facebook', 'telegram'];
        ...
        if (modal && modal.style.display === 'flex' && document.hidden && !esMisionSocial) {
            // Solo castigar si estaban viendo YouTube o TikTok
            detenerTemporizadorAnuncio();
