...
let ytAdPlayer = null;
let hostedVideoPlayer = null; 
let lastVideoTime = -1; 

// Variables secretas para validación de tiempo anti-consola
let _misionIniciadaEn = 0;
let _tiempoMinimoRequerido = 0;
let _misionValidada = false;
            ...
            <div id="modal-anuncio-estricto" class="fixed inset-0 bg-black z-[99999] hidden flex-col justify-between">
                ...
                <div class="flex-1 relative flex items-center justify-center bg-black/95 p-4 overflow-hidden">
                    ...
                    <div id="overlay-play-anuncio" class="absolute inset-0 z-40 bg-black/80 backdrop-blur-sm flex flex-col items-center justify-center hidden">
                        ...
                        <p id="txt-pausa-advertencia" class="text-yellow-400 font-bold mt-4 text-sm hidden uppercase text-center px-4">Contador Pausado - Reproduce para continuar</p>
                    </div>
                </div>
...
window.abrirAnuncio = (adId) => {
    ...
    // Reseteamos variables anti-trampas
    lastVideoTime = -1; 
    _misionIniciadaEn = 0;
    _tiempoMinimoRequerido = 0;
    _misionValidada = false;
    ...
    const txtBtnPlay = document.getElementById('txt-btn-forzar-play');
    ...
    btnForzarPlay.disabled = false;
    btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
    iconoPlay.className = "fas fa-play mr-3 text-2xl"; 
    txtBtnPlay.innerText = "DALE PLAY AL VIDEO";
    ...
    if (anuncioActivo.type === 'youtube') {
                    ...
                    events: {
                        'onStateChange': (event) => {
                            if (event.data === YT.PlayerState.PLAYING) {
                                ...
                                txtPausa.classList.add('hidden');
                                if(!timerAnuncio) iniciarTemporizadorAnuncio();
                            } else if (event.data === YT.PlayerState.PAUSED || event.data === YT.PlayerState.ENDED) {
                                overlayPlay.classList.remove('hidden');
                                txtPausa.innerText = "CONTADOR PAUSADO - REPRODUCE PARA CONTINUAR";
        ...
        container.innerHTML = `
            <div class="w-full h-full flex flex-col items-center justify-start overflow-y-auto pt-4 pb-12">
                ...
                <div class="w-full max-w-sm max-h-[70vh] aspect-[9/16] bg-black rounded-xl overflow-hidden shadow-[0_0_20px_rgba(0,0,0,0.8)] relative flex justify-center items-center">
                    <video id="html5-hosted-video" src="${anuncioActivo.media_url}" class="w-full h-full object-contain" playsinline preload="auto" controlsList="nodownload noplaybackrate"></video>
                </div>
            </div>
        `;
        ...
        hostedVideoPlayer.volume = 1.0;
        hostedVideoPlayer.muted = false;
        hostedVideoPlayer.addEventListener('volumechange', () => {
            if (hostedVideoPlayer.muted || hostedVideoPlayer.volume < 1) {
                hostedVideoPlayer.muted = false;
                hostedVideoPlayer.volume = 1.0;
            }
        });
        ...
        btnForzarPlay.onclick = () => {
            if (hostedVideoPlayer) {
                btnForzarPlay.disabled = true;
                btnForzarPlay.classList.add('cursor-not-allowed', 'opacity-80');
                iconoPlay.className = "fas fa-spinner fa-spin mr-3 text-2xl";
                txtBtnPlay.innerText = "CONECTANDO...";

                hostedVideoPlayer.play().catch(e => {
                    btnForzarPlay.disabled = false;
                    btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
                    iconoPlay.className = "fas fa-play mr-3 text-2xl";
                    txtBtnPlay.innerText = "REINTENTAR";
                });
                hostedVideoPlayer.controls = true; 
            }
        };

        hostedVideoPlayer.addEventListener('playing', () => {
            overlayPlay.classList.add('hidden');
            txtPausa.classList.add('hidden');
            ...
            btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
            iconoPlay.className = "fas fa-play mr-3 text-2xl";
            txtBtnPlay.innerText = "CONTINUAR VIDEO";
            
            if(!timerAnuncio) iniciarTemporizadorAnuncio();
        });

        hostedVideoPlayer.addEventListener('waiting', () => {
            overlayPlay.classList.remove('hidden');
            txtPausa.innerText = "EL VIDEO EST\u00C1 CARGANDO...";
            txtPausa.classList.remove('hidden');
            
            btnForzarPlay.disabled = true;
            btnForzarPlay.classList.add('cursor-not-allowed', 'opacity-80');
            iconoPlay.className = "fas fa-spinner fa-spin mr-3 text-2xl";
            txtBtnPlay.innerText = "ESPERANDO CONEXIÓN...";
        });

        hostedVideoPlayer.addEventListener('pause', () => {
            if (!hostedVideoPlayer.ended) {
                if (hostedVideoPlayer.readyState >= 3) {
                    overlayPlay.classList.remove('hidden');
                    txtPausa.innerText = "CONTADOR PAUSADO - REPRODUCE PARA CONTINUAR";
                    txtPausa.classList.remove('hidden');
                    ...
                    btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
                    iconoPlay.className = "fas fa-play mr-3 text-2xl";
                    txtBtnPlay.innerText = "REANUDAR VIDEO";
                }
            }
        });
        ...
        hostedVideoPlayer.addEventListener('ended', () => {
            overlayPlay.classList.remove('hidden');
            txtPausa.innerText = "\u00A1VIDEO TERMINADO!";
            txtPausa.classList.remove('hidden');
            
            btnForzarPlay.disabled = false;
            btnForzarPlay.classList.remove('cursor-not-allowed', 'opacity-80');
            iconoPlay.className = "fas fa-redo mr-3 text-2xl";
            txtBtnPlay.innerText = "REPETIR VIDEO";
        });
...
// =========================================================
// NÚCLEO ANTI-TRAMPAS FÍSICO (EL PERRO GUARDIÁN)
// =========================================================
function iniciarTemporizadorAnuncio() {
    ...
    // CAPA ANTI-CONSOLA: Guardamos la hora de inicio en servidor
    _misionIniciadaEn = Date.now();
    _tiempoMinimoRequerido = (anuncioActivo.timer || 15) * 1000;
    _misionValidada = false;

    timerAnuncio = setInterval(() => {
        if (anuncioActivo) {
            if (anuncioActivo.type === 'hosted_video' && hostedVideoPlayer) {
                let currentTime = hostedVideoPlayer.currentTime;
                
                if (hostedVideoPlayer.paused || currentTime === 0 || currentTime === lastVideoTime) {
                    if (currentTime === lastVideoTime && !hostedVideoPlayer.paused && currentTime > 0) {
                        const overlayPlay = document.getElementById('overlay-play-anuncio');
                        const txtPausa = document.getElementById('txt-pausa-advertencia');
                        const btnForzarPlay = document.getElementById('btn-forzar-play');
                        const iconoPlay = document.getElementById('icono-forzar-play');
                        ...
                        if(txtPausa) {
                            txtPausa.innerText = "CARGANDO VIDEO - CONTADOR PAUSADO";
                            txtPausa.classList.remove('hidden');
                        }
                        if(btnForzarPlay) {
                            btnForzarPlay.disabled = true;
                ...
                const overlayPlay = document.getElementById('overlay-play-anuncio');
                const txtPausa = document.getElementById('txt-pausa-advertencia');
                if (overlayPlay && !overlayPlay.classList.contains('hidden')) {
                    overlayPlay.classList.add('hidden');
                    txtPausa.classList.add('hidden');
            ...
            else if (anuncioActivo.type === 'youtube' && ytAdPlayer && typeof ytAdPlayer.getCurrentTime === 'function') {
                let currentYtTime = ytAdPlayer.getCurrentTime();
                
                if (ytAdPlayer.getPlayerState() !== YT.PlayerState.PLAYING || currentYtTime === 0 || currentYtTime === lastVideoTime) {
                    return; 
                }
                lastVideoTime = currentYtTime;
            }
        }
        ...
        if(tiempoRestante <= 0) {
            clearInterval(timerAnuncio);
            timerAnuncio = null;
            _misionValidada = true; // El contador del sistema certifica que terminó
...
// --- FUNCIÓN DE LIMPIEZA PARA REUTILIZAR ---
function cerrarYLimpiarAnuncio() {
    ...
    const contenedor = document.getElementById('contenedor-media-anuncio');
    if(contenedor) contenedor.innerHTML = '';
}

window.reclamarRecompensaAnuncio = async () => {
    // CAPA ANTI-CONSOLA: Validar tiempo físico transcurrido
    const tiempoFisicoTranscurrido = Date.now() - _misionIniciadaEn;
    if (!_misionValidada || tiempoFisicoTranscurrido < _tiempoMinimoRequerido) {
        alert("\u26D4 Trampa detectada: Alteraci\u00F3n de tiempo. Misi\u00F3n cancelada.");
        cerrarYLimpiarAnuncio();
        return;
    }
    ...
    try {
        const uid = auth.currentUser.uid;
        const adId = anuncioActivo.id;
        const reward = anuncioActivo.reward;

        // DOBLE CANDADO 1: Transacción para la misión (Bloquea las múltiples pestañas)
        const txMision = await db.ref(`users/${uid}/misiones_completadas/${adId}`).transaction((estadoActual) => {
            if (estadoActual === true) return; // Aborta si ya estaba completada
            return true; 
        });

        if (!txMision.committed) {
            alert("\u26D4 Trampa detectada: Ya hab\u00EDas reclamado la recompensa de esta misi\u00F3n.");
            cerrarYLimpiarAnuncio(); 
            return;
        }

        // DOBLE CANDADO 2: Transacción para el saldo en el servidor
        await db.ref(`users/${uid}/creditos_bingo_gratis`).transaction(saldoServidor => {
            return (saldoServidor || 0) + reward;
        });
        
        cerrarYLimpiarAnuncio();
        
        setTimeout(() => {
            const area = document.getElementById('area-canje-creditos');
...
function configurarAntiTrampas() {
    document.addEventListener("visibilitychange", () => {
        ...
        if (modal && modal.style.display === 'flex' && document.hidden && !esMisionSocial) {
            ...
            if (ytAdPlayer && typeof ytAdPlayer.pauseVideo === 'function') {
                ytAdPlayer.pauseVideo();
            }
            if (hostedVideoPlayer) {
                hostedVideoPlayer.pause(); 
            }
...
window.guardarCartonGratuito = async () => {
    ...
    try {
        ...
        // DOBLE CANDADO 3: Transacción al cobrar el cartón para evitar que gasten el mismo crédito dos veces
        const txSaldo = await db.ref(`users/${uid}/creditos_bingo_gratis`).transaction(c => {
            if ((c || 0) <= 0) return; // Si tiene 0 o menos, aborta
            return c - 1; // Si tiene, descuenta 1
        });

        if (!txSaldo.committed) {
            alert("\u26D4 Saldo insuficiente. No tienes cartones libres o ya fueron utilizados.");
            seleccionCartonGratis.clear();
            document.getElementById('area-armado-carton').classList.add('hidden');
