...
// 2. TRADUCTOR UNIVERSAL DE FECHAS (Corrección de error de lectura)
function normalizeDate(str) {
    if (!str) return "00-00-0000";
    // Extraemos solo la parte de la fecha (DD-MM-YYYY) ignorando horas que pueda traer el Bingo
    let cleanStr = String(str).replace(/\//g, '-').trim().split(' ');
    return cleanStr;
}
...
function addDaysNatively(dateStr, daysToAdd) {
    ...
    // Corrección crítica: Mapeo correcto del arreglo p = Año, p = Mes, p = Día
...
function initStreakMonitor(uid) {
    dbRacha.ref('draw_status/date').on('value', snap => {
        ...
        // BINGO: Compara usando el Traductor Universal (Ahora limpio de horas)
...
function renderVisuals(dias, last_active, fecha_inicio, diaSalvado, visualDay, nivelAlcanzadoHoy, nivel_minimo) {
    ...
    if (visualDay === 7 && last_active === currentMasterDate) {
        ...
        // Mensaje dinámico si el día ya fue asegurado
