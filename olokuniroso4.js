/* ============================================================
   ...
   Descripci\u00F3n: M\u00F3dulo inyectable, independiente y seguro.
...
let corralAllPlayers = {}; // Variable global para almacenar a todos los participantes

// F\u00F3rmula para ocultar el tel\u00E9fono por privacidad
function ocultarTelefono(tel) {
    if (!tel) return "Sin N\u00FAmero";
    let str = String(tel).trim();
    if (str.length < 7) return str;
    let inicio = str.substring(0, 4);
    let fin = str.substring(str.length - 3);
    return `${inicio}***${fin}`;
}

// 1. INYECTAR EL HTML DEL JUEGO Y EL BOT\u00D3N EN EL MEN\u00DA
function injectCorralApp() {
    ...
    if(containerWrapper && !document.getElementById('section-corral')) {
        ...
        <div class="bg-gray-900 border-2 border-orange-600 shadow-2xl rounded-xl overflow-hidden relative flex flex-col mb-6">
            ...
            <div id="corral-game-area" class="p-6 flex-1 min-h-[300px] flex flex-col justify-start items-center relative">
                </div>
...
function initCorralSystem() {
    ...
    // Escuchar a todos los jugadores para mostrarlos en vivo
    db.ref('corral_game/players').on('value', s => {
        corralAllPlayers = s.exists() ? s.val() : {};
        renderCorralUI();
    });
...
function renderCorralUI() {
    ...
    if (!corralPlayer) {
        ...
        `;
    } else if (corralPlayer.status === 'vivo' && corralPlayer.corral === 0) {
        mainContent = `
            <div class="text-center w-full animate-bounce-in my-4">
        ...
        mainContent = `
            <div class="text-center w-full animate-pulse-fast my-4">
                ...
                <div class="bg-black/50 border border-gray-700 p-4 rounded-xl">
                    ...
                    <p class="text-[10px] text-gray-500 uppercase tracking-widest">Atento al grupo de WhatsApp.<br>Si el animalito sorteado est\u00E1 en tu corral, avanzas.</p>
                </div>
            </div>
        `;
    } else if (corralPlayer.status === 'muerto') {
        if (corralConfig.recompra_activa) {
            mainContent = `
                <div class="text-center w-full animate-bounce-in my-4">
                    ...
                    <div class="bg-red-900/20 border border-red-800 p-4 rounded-xl mb-6">
                        ...
                        <p class="text-gray-300 text-[10px]">A\u00FAn puedes volver pagando la recompra.</p>
                    </div>
            ...
            mainContent = `
                <div class="text-center w-full opacity-80 grayscale my-4">
                    ...
                    <div class="bg-black border border-gray-800 p-3 rounded-lg inline-block">
                        <span class="text-[10px] text-red-600 font-bold uppercase tracking-widest"><i class="fas fa-ban mr-1"></i> FASE DE MUERTE S\u00DABITA</span>
                    </div>
                </div>
            `;
        }
    ...
    // LISTA DE PARTICIPANTES (Aparece abajo del estado del jugador)
    let playersHtml = `
        <div class="mt-4 w-full border-t border-gray-700 pt-4">
            <h4 class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-3 text-center"><i class="fas fa-users mr-1"></i> Jugadores Activos</h4>
            <div class="max-h-48 overflow-y-auto custom-scrollbar space-y-2 pr-1 w-full">`;
            
    // Ordenar participantes: los vivos primero
    let sortedPlayers = Object.values(corralAllPlayers).sort((a,b) => {
        if(a.status === 'vivo' && b.status !== 'vivo') return -1;
        if(b.status === 'vivo' && a.status !== 'vivo') return 1;
        return 0;
    });

    if (sortedPlayers.length === 0) {
        playersHtml += `<p class="text-xs text-gray-500 text-center italic">A\u00FAn no hay participantes en la sala.</p>`;
    } else {
        sortedPlayers.forEach(p => {
            let isVivo = p.status === 'vivo';
            let corralName = "Esperando...";
            let colorClass = "text-gray-500";
            let iconClass = "fa-clock";
            
            if(isVivo) {
                if(p.corral === 1) { corralName = "CORRAL 1"; colorClass = "text-orange-400"; iconClass = "fa-horse-head"; }
                else if(p.corral === 2) { corralName = "CORRAL 2"; colorClass = "text-blue-400"; iconClass = "fa-crow"; }
                else { corralName = "ELIGIENDO"; colorClass = "text-gray-400"; iconClass = "fa-spinner fa-spin"; }
            } else {
                corralName = "ELIMINADO"; colorClass = "text-red-500"; iconClass = "fa-skull";
            }

            let opacity = isVivo ? "opacity-100" : "opacity-50 grayscale";

            playersHtml += `
                <div class="flex justify-between items-center bg-gray-800/80 p-2 rounded-lg border border-gray-700 text-xs shadow-sm w-full ${opacity}">
                    <div class="flex flex-col text-left">
                        <span class="text-white font-bold tracking-wide">${p.name || 'Jugador'}</span>
                        <span class="text-gray-400 text-[10px] font-mono tracking-widest">${ocultarTelefono(p.phone)}</span>
                    </div>
                    <div class="${colorClass} font-black text-[10px] uppercase tracking-widest flex items-center bg-black/40 px-2 py-1 rounded">
                        <i class="fas ${iconClass} mr-1"></i> ${corralName}
                    </div>
                </div>
            `;
        });
    ...
    // Renderizamos el contenido principal + la lista de jugadores
    area.innerHTML = mainContent + playersHtml;
}
...
window.recomprarCorral = async () => {
    ...
    if(!confirm(`\u00BFRevivir por Bs ${precioRec.toFixed(2)}?`)) return;
...
function mostrarAlertaResultado(resultado) {
    ...
    if (fueGanador) {
        ...
        text.innerHTML = `Sali\u00F3 el <b>${resultado.animal} - ${resultado.nombre}</b>.<br>Prep\u00E1rate para elegir de nuevo.`;
    } else {
