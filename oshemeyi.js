/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V13 - ANTI-FALLOS)
   Motor Matemático Absoluto (Adiós problemas de zona horaria)
   ============================================================ */
    ...
    container.innerHTML = `
        <div id="racha-main-box" class="bg-gray-900 rounded-2xl p-1 shadow-2xl border-2 border-yellow-500 relative overflow-hidden mt-2 mb-6">
            ...
            <div class="bg-gray-800 p-4 border-t border-gray-700 flex flex-col items-center">
                ...
                <div class="bg-black/50 p-3 rounded-lg border border-gray-600 shadow-inner w-full max-w-xs text-center mb-4 relative">
                    ...
                    <div class="flex justify-center items-end gap-2">
                        ...
                        <span id="today-played-label" class="text-sm font-bold text-gray-500 pb-1">Para Hoy</span>
                    </div>
                    <p id="cycle-notice" class="text-[8px] text-green-400 font-bold mt-2 uppercase hidden">✓ Los tickets de hoy ya fueron procesados.</p>
                </div>
...
// 2. MOTOR MATEMÁTICO (Sin Zonas Horarias, Sin Errores)
function getMasterDateStr() {
    const now = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
    if(now.getHours() >= 20) now.setDate(now.getDate() + 1); 
    return `${String(now.getDate()).padStart(2,'0')}-${String(now.getMonth()+1).padStart(2,'0')}-${now.getFullYear()}`;
}

// Verifica el ticket de Bingo basado PÚRAMENTE en milisegundos de tiempo absoluto
function isBingoTicketValid(pushKey) {
    if (!pushKey || pushKey.length < 8) return false;
    const PUSH_CHARS = '-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz';
    let ticketMs = 0;
    for (let i = 0; i < 8; i++) {
        const n = PUSH_CHARS.indexOf(pushKey.charAt(i));
        if (n === -1) return false;
        ticketMs = ticketMs * 64 + n;
    }

    // Calcular el ciclo activo basado en Caracas (UTC-4)
    const CARACAS_OFFSET = -4 * 3600000;
    const caracasTicketMs = ticketMs + CARACAS_OFFSET;
    
    const nowMs = Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0);
    const caracasNowMs = nowMs + CARACAS_OFFSET;

    const dayLength = 24 * 3600000;
    const caracasStartOfDay = Math.floor(caracasNowMs / dayLength) * dayLength;
    const cutoffToday = caracasStartOfDay + (20 * 3600000); // Corte a las 20:00 (8 PM)

    let startCycle, endCycle;
    if (caracasNowMs >= cutoffToday) {
        startCycle = cutoffToday;
        endCycle = cutoffToday + dayLength;
    } else {
        startCycle = cutoffToday - dayLength;
        endCycle = cutoffToday;
    }

    return caracasTicketMs >= startCycle && caracasTicketMs < endCycle;
}
...
// 3. MONITORES DIRECTOS
function initStreakMonitor(uid) {
    
    // A. BINGO - Filtrado por Matemática Absoluta
    dbRacha.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        let countB = 0;
        if (s.exists()) {
            s.forEach(c => {
                if (c.val().payment_method !== 'GRATIS' && isBingoTicketValid(c.key)) {
                    countB++;
                }
            });
        }
    ...
    // B. TORNEO - Filtrado Directo por Carpeta (Blindado)
    const listenTorneo = () => {
        const now = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
        if (now.getHours() >= 18) now.setDate(now.getDate() + 1);
        const tDate = `${String(now.getDate()).padStart(2,'0')}-${String(now.getMonth()+1).padStart(2,'0')}-${now.getFullYear()}`;
        
        dbRacha.ref(`bets_torneo_express/${tDate}/${uid}`).on('value', s => {
    ...
    listenTorneo();
    // Re-evaluar carpeta de torneo cada 60 seg (por si dan las 6 PM mientras el usuario mira la pantalla)
    setInterval(listenTorneo, 60000); 
}

// 4. ACTUALIZACIÓN VISUAL Y GUARDADO
function updateStreakUI(uid) {
    ...
    const masterDate = getMasterDateStr();
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // Textos Inteligentes
        document.getElementById('mision-title-text').textContent = isTomorrow ? "SORTEO DE MAÑANA:" : "SORTEO DE HOY:";
        document.getElementById('today-played-label').textContent = isTomorrow ? "Para Mañana" : "Para Hoy";
        const noticeEl = document.getElementById('cycle-notice');
        if (isTomorrow) { noticeEl.classList.remove('hidden'); } else { noticeEl.classList.add('hidden'); }
        ...
        if (diaSalvado) {
            ...
            if (data.last_update !== masterDate) {
                ...
                updates.dias = visualDay; updates.nivel_minimo = minLvl;
                updates.last_update = masterDate; updates.last_active = masterDate;
                isDirty = true;
        ...
        const ticketsDisplay = document.getElementById('today-played-tickets');
        if (ticketsDisplay) ticketsDisplay.textContent = totalJugado;
...
function renderStreakDaysGrid(diasCompletados, diaSalvado) {
    ...
    for(let i=1; i<=7; i++) {
        let icon = '🔒'; let color = 'bg-gray-800 border-gray-700 text-gray-600'; let subText = 'Día ' + i;
        if (i <= diasCompletados) {
            ...
            if (diaSalvado) { icon = '✅'; color = 'bg-green-500 border-white text-white shadow-[0_0_15px_rgba(34,197,94,0.8)] scale-110'; subText = 'Asegurado'; } 
            else { icon = '🎯'; color = 'bg-orange-500 border-white text-white shadow-[0_0_15px_rgba(249,115,22,0.8)] scale-110 animate-pulse'; subText = 'Hoy'; }
        } else if (i === 7) { icon = '🎁'; color = 'bg-gray-800 border-gray-700 text-gray-500 opacity-50'; }

        grid.innerHTML += `
            <div class="flex flex-col items-center">
                <div class="w-8 h-8 rounded-full border-2 ${color} flex items-center justify-center text-xs transition-all duration-500">${icon}</div>
