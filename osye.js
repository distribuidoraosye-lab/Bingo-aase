/* ============================================================
   BINGO TEXIER - HORA LOCA (VERSIÓN BLINDADA Y DESACOPLADA)
   - Corrección crítica: Evita listeners duplicados en Firebase.
   - Forzado de ocultamiento estricto si el estatus ya no es 'pending'.
   - Sincronización de fecha con francisca.js
   ============================================================ */
...
let hl_listenersActivos = false; // EL SEGURO CONTRA BUCLES
...
// 2. ARRANQUE E INYECCIÓN DE LISTENERS (ESTRUCTURA PLANA Y ÚNICA)
firebase.auth().onAuthStateChanged(user => {
    if (user) {
        dbHL = firebase.database();
        hl_uid = user.uid;
        
        // Evitar que el código se ejecute múltiples veces
        if (hl_listenersActivos) return;
        hl_listenersActivos = true;

        // Validar si es usuario nuevo una sola vez
        dbHL.ref(`users/${hl_uid}/welcome_bonus_status`).once('value', snapBono => {
            ...
            // Generar fecha exacta usando el reloj del servidor (mismo de francisca.js)
            const d_hl = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
            const hoy = `${String(d_hl.getDate()).padStart(2,'0')}-${String(d_hl.getMonth()+1).padStart(2,'0')}-${d_hl.getFullYear()}`;
            ...
            // C. Escuchar Saldo Real (El vigilante de compras)
            dbHL.ref(`users/${hl_uid}/balance`).on('value', snapBalance => {
                ...
                if (hl_estadoReto === 'locked' && saldoActual < hl_balanceInicial) {
                    hl_estadoReto = 'claiming'; // Bloqueo temporal de seguridad
                    liberarPremioFinal();
                }
                if (hl_estadoReto === 'locked' && saldoActual > hl_balanceInicial) {
            ...
            // D. Escuchar Cartones Gratis para actualizar el contador
        ...
        hl_listenersActivos = false; // Resetear bandera al cerrar sesión
    }
});

// 3. CONTROL DE INTERFAZ REACTIVO (BLINDADO)
function actualizarInterfazVisual() {
    ...
    // Si NO está pendiente, o la hora loca está apagada, la ruleta se oculta FORZOSAMENTE
    if (hl_estadoReto !== 'pending' || !admin_HoraLocaActiva) {
        if(modalRuleta) modalRuleta.style.display = 'none';
        window.ruletaAutomaticaMostrada = true; // Previene que intente mostrarse de nuevo
    }
...
// 5. LIBERACIÓN DEL PREMIO (TRANSACCIÓN ATÓMICA DE FIREBASE)
async function liberarPremioFinal() {
    ...
    const result = await statusRef.transaction(currentStatus => {
        ...
        return undefined; // Si no es 'locked', la transacción se cancela (evita doble entrega)
    });
