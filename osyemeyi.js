/* ============================================================
   BINGO TEXIER - MÓDULO RACHA (NÚCLEO REESCRITO - V10)
   Reglas: Cero Amnesia, Lector Universal, Tolerancia Decimal
   ============================================================ */

const dbRacha = firebase.database();
let configTasa = 500; // Valor seguro por defecto

// MEMORIA DE SESIÓN (No se borra al comprar cartones)
let streakState = {
    todayDeposit: 0,
    jugadoBingo: 0,
    jugadoTorneo: 0,
...
// 0. CARGA DE TASA EN TIEMPO REAL
...
// TRADUCTOR UNIVERSAL DE FECHAS (El que evita que se ponga en 0)
function getFormattedDateStr(timeVal) {
    if (!timeVal) return null;
    let d;
    
    if (typeof timeVal === 'string') {
        if (timeVal.includes('-') && timeVal.split('-').length === 2) {
            let parts = timeVal.split(' ').split('-');
            d = new Date(`${parts}/${parts}/${parts}`);
        } else if (timeVal.includes('/') && timeVal.split('/').length === 2) {
            let parts = timeVal.split(' ').split('/');
            d = new Date(`${parts}/${parts}/${parts}`);
        } else {
            d = new Date(timeVal);
...
function getSystemDate() {
    return getFormattedDateStr(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
}

// 1. INYECTOR DE INTERFAZ (Se mantiene tu diseño visual premium)
        ...
        <div id="modal-rules-racha" class="hidden fixed inset-0 bg-black/95 z- flex items-center justify-center p-4">
            <div class="bg-gray-900 border-2 border-yellow-500 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
                ...
                <div class="space-y-3 text-xs text-gray-300">
                    <p><b class="text-white uppercase">¿CÓMO GANAR?</b><br>El premio final se basa en el nivel del día que MENOS hayas apostado.</p>
                    <div class="bg-gray-800 p-3 rounded-lg border border-gray-700 shadow-inner">
                        <p class="mb-2"><b class="text-orange-400">🥉 Bronce:</b> Juega 1 cartón diario x 7 días ➔ <b class="text-green-400 font-black">Cobras $10</b></p>
                        <p class="mb-2"><b class="text-gray-300">🥈 Plata:</b> Juega 3 cartones diarios x 7 días ➔ <b class="text-green-400 font-black">Cobras $30</b></p>
                        <p><b class="text-yellow-400">🥇 Oro:</b> Juega 6 cartones diarios x 7 días ➔ <b class="text-green-400 font-black">Cobras $60</b></p>
                    </div>
                    <p><b class="text-red-400 uppercase">⚠️ REGLA DE ORO:</b><br>Si depositas, debes jugar ese saldo antes de retirar. Si retiras sin jugar lo depositado, pierdes la racha y vuelves al Día 0.</p>
                </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        ...
        firebase.auth().onAuthStateChanged(user => {
            if (user && currentUserActive === user.uid) return; 
            
            currentUserActive = user ? user.uid : null;
...
// 2. EL MOTOR DE BÚSQUEDA (Indestructible)
async function initStreakMonitor(uid) {
    const todayTargetStr = getSystemDate();

    // VIGILANTE DE DEPÓSITOS
    ...
    const cbDep = refDep.on('value', s => {
        ...
        if (s.exists()) {
            s.forEach(ref => {
                const data = ref.val();
                let recordDate = getFormattedDateStr(data.date || data.timestamp || data.fecha || data.time);
                if (recordDate === todayTargetStr) {
        ...
        updateStreakUI(uid, todayTargetStr);
    });
    ...
    const cbBin = refBin.on('value', s => {
        ...
        if (s.exists()) {
            s.forEach(c => {
                const val = c.val();
                let recordDate = getFormattedDateStr(val.date || val.timestamp || val.time);
                if(recordDate === todayTargetStr && val.payment_method !== 'GRATIS') {
    ...
    // VIGILANTE DE TORNEO
    const refTor = dbRacha.ref(`bets_torneo_express/${todayTargetStr}/${uid}`);
...
// 3. ACTUALIZACIÓN LÓGICA Y GUARDADO
function updateStreakUI(uid, todayTargetStr) {
    ...
    // TOLERANCIA DECIMAL (0.90 evita bloqueos matemáticos)
    let nivelAlcanzadoHoy = 0;
    if (usdJugado >= 5.90) { nivelAlcanzadoHoy = 3; }
    else if (usdJugado >= 2.90) { nivelAlcanzadoHoy = 2; }
    else if (usdJugado >= 0.90) { nivelAlcanzadoHoy = 1; }

    let diaSalvado = (usdJugado >= 0.90);
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        let isPenalized = (data.penalizado_hoy === todayTargetStr);
        let isDirty = false;
        let updates = {};
        ...
        // Control de pérdida de racha por inactividad
        if (!isPenalized && data.dias > 0 && data.last_active !== todayTargetStr && data.last_active !== ayerStr) {
            data.dias = 0;
            data.nivel_minimo = 3;
            updates.dias = 0; updates.nivel_minimo = 3; updates.last_active = todayTargetStr;
            isDirty = true;
        }
        ...
        if (!isPenalized && diaSalvado) {
            ...
            if (data.last_update !== todayTargetStr) {
                ...
                updates.last_update = todayTargetStr;
                updates.last_active = todayTargetStr;
                isDirty = true;
                data.dias = visualDay; 
                data.nivel_minimo = minLvl;
            } else if (data.last_update === todayTargetStr) {
        ...
        } else if (!isPenalized && depVal > 0) {
            if (data.last_active !== todayTargetStr) {
                updates.last_active = todayTargetStr;
                isDirty = true;
            }
        }
        ...
        if (depVal > 0 && playVal < (depVal * 0.98)) { // 2% de margen de tolerancia
        ...
        renderStreakDaysGrid(data.dias, isPenalized, diaSalvado, depVal);
    });
}

function renderStreakDaysGrid(diasCompletados, isPenalized, diaSalvado, depVal) {
...
function protectWithdrawals(uid) {
    ...
    form.onsubmit = async (e) => {
        ...
        if(dep > 0 && play < (dep * 0.98)) { // Margen matemático
