/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V9.1 - Lógica por Cartones)
...
// 2. Controladores de Fecha y Hora (Cortes independientes)
...
// Extrae la hora exacta de la compra directamente del ID de Firebase
...
// Calcula a qué día de racha pertenece esa hora
function getCustomDateStrForTimestamp(ts, offsetHours) {
    const d = new Date(ts);
    if(d.getHours() >= offsetHours) d.setDate(d.getDate() + 1);
    return `${String(d.getDate()).padStart(2,'0')}-${String(d.getMonth()+1).padStart(2,'0')}-${d.getFullYear()}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ocultar racha al entrar a un juego
    setTimeout(() => {
...
// 3. Monitor Inteligente (Independiente del Panel Admin)
async function initStreakMonitor(uid) {
    ...
    dbRacha.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        let countB = 0;
        const rachaDate = getCustomDateStr(20); 
        
        if (s.exists()) {
            s.forEach(c => {
                const val = c.val();
                if (val.payment_method === 'GRATIS') return; 
                
                // Magia: Ignoramos la fecha del Admin y usamos la hora real de la compra
                const realTimestamp = getTimestampFromPushKey(c.key);
                const ticketDate = getCustomDateStrForTimestamp(realTimestamp, 20);
                
                if (ticketDate === rachaDate) countB++;
            });
        }
