/* ============================================================
   ...
   VERSIÓN ESTABLE - DISEÑO PREMIUM RESTAURADO Y LÓGICA ACTIVA
   ============================================================ */
...
// 1. Interfaz Premium (Tamaño ideal y textos claros)
    ...
    container.innerHTML = `
        <div class="bg-gray-900 rounded-2xl p-1 shadow-2xl border-2 border-yellow-500 relative overflow-hidden mt-2 mb-6">
            ...
            <div class="bg-gradient-to-r from-red-700 via-orange-600 to-red-700 p-4 text-center border-b border-gray-800 shadow-inner">
                <h3 id="projected-prize-text" class="font-black text-white text-lg sm:text-xl tracking-tighter uppercase animate-pulse drop-shadow-lg leading-tight">
                    🔥 ¡GANA HASTA $60 GRATIS SI JUEGAS LOS 7 DÍAS! 🔥
                </h3>
            </div>

            <div class="p-5 bg-black/80">
                <div class="w-full bg-gray-800 rounded-full h-5 p-0.5 border border-gray-700 shadow-inner relative mb-6">
                    ...
                    <div class="absolute inset-0 flex justify-between px-3 items-center text-[11px] font-black text-white/50 pointer-events-none">
            ...
            <div class="bg-gray-800 p-4 border-t border-gray-700">
                <div class="flex justify-between items-center mb-3">
                    <span class="text-xs text-gray-400 font-bold uppercase tracking-widest">Estado de Hoy:</span>
                    <span id="today-level-badge" class="text-xs bg-gray-700 px-2 py-1 rounded text-white font-black shadow-sm">🎯 ESPERANDO...</span>
                </div>
                
                <div class="grid grid-cols-2 gap-3 text-center">
                    <div class="bg-black/50 p-3 rounded-lg border border-gray-600 shadow-inner">
                        <p class="text-[10px] text-gray-500 uppercase font-bold mb-1">Depositado Hoy</p>
                        <p id="today-deposit-text" class="text-base font-black text-white">Bs 0.00</p>
                    </div>
                    <div class="bg-black/50 p-3 rounded-lg border border-gray-600 shadow-inner">
                        <p class="text-[10px] text-gray-500 uppercase font-bold mb-1">Jugado Hoy</p>
                        <p id="today-played-text" class="text-base font-black text-orange-400">Bs 0.00</p>
                    </div>
                </div>
                <p id="rollover-warning" class="text-xs text-red-400 text-center mt-3 font-black uppercase tracking-widest hidden animate-pulse">
                    ⚠️ ¡ALERTA! Juega todo lo depositado hoy para asegurar el día.
                </p>
            </div>

            <div class="bg-black/90 p-3 text-center">
                <button onclick="showRachaRules()" class="text-xs text-yellow-500 font-black uppercase tracking-widest hover:underline opacity-90 hover:opacity-100 transition">
        ...
        <div id="modal-rules-racha" class="hidden fixed inset-0 bg-black/95 z- flex items-center justify-center p-4">
            <div class="bg-gray-900 border-2 border-yellow-500 rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
                ...
                <div class="space-y-3 text-xs text-gray-300">
                    <p><b class="text-white uppercase">¿CÓMO GANAR?</b><br>Juega 7 días seguidos cumpliendo el mínimo diario. El premio final se basa en el nivel del día que MENOS hayas jugado en la semana.</p>
                    ...
                    <p><b class="text-red-400 uppercase">⚠️ REGLA ESTRICTA (ANTI-TRAMPA):</b><br>Todo lo que deposites en el día <b>DEBES JUGARLO</b> (en Bingo o Torneo). Si depositas y luego retiras el dinero sin haberlo apostado, tu racha se penaliza y vuelves al Día 0.</p>
                </div>
...
function updateStreakUI(uid, dateStr) {
    ...
    // CÁLCULOS ESTRICTOS BASADOS EN JUEGO
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        if (isPenalized) {
            ...
            titleEl.className = "font-black text-red-500 text-lg sm:text-xl tracking-wider uppercase";
            document.getElementById('streak-bar-7days').className = "bg-red-600 h-full rounded-full transition-all duration-500";
        } else {
            ...
            if (diaSalvado) {
                if (nivelAlcanzadoHoy === 3) {
                    ...
                    titleEl.innerHTML = `🚀 ¡JUEGA MÁS HOY Y ASEGURA $60! 🚀`;
                } else {
                    badgeEl.innerHTML = "<span class='text-orange-400'>🥉 DÍA BRONCE ASEGURADO</span>";
                    titleEl.innerHTML = `🔥 ¡AUMENTA TU JUEGO PARA LLEVARTE $60! 🔥`;
                }
...
function renderStreakDaysGrid(diasCompletados, isPenalized, diaSalvado, usdDepositado) {
    ...
    for(let i=1; i<=7; i++) {
        let icon = '🔒';
        let cssText = 'text-gray-600 opacity-50 text-sm';
        let subText = 'Día ' + i;

        if (i <= diasCompletados) {
            icon = '✅'; cssText = 'text-green-500 text-lg drop-shadow-md'; subText = 'Listo';
        } else if (i === diasCompletados + 1) {
            if (isPenalized) {
                icon = '🚫'; cssText = 'text-red-500 text-lg animate-pulse'; subText = 'Perdido';
            } else if (diaSalvado) {
                icon = '✅'; cssText = 'text-green-400 text-lg drop-shadow-md'; subText = 'Listo';
            } else if (usdDepositado > 0) {
                icon = '⏳'; cssText = 'text-yellow-400 text-lg animate-pulse'; subText = 'Juega';
            } else {
                icon = '🎯'; cssText = 'text-orange-500 text-lg opacity-80'; subText = 'Hoy';
            }
        } else if (i === 7) {
            icon = '🎁'; cssText = 'text-gray-400 opacity-50 text-sm'; 
        }

        grid.innerHTML += `
            <div class="flex flex-col items-center justify-center w-10">
                <div class="transition-all duration-300 ${cssText}">${icon}</div>
                <span class="text-[9px] font-black mt-1 uppercase ${i <= diasCompletados + 1 && !isPenalized ? 'text-gray-200' : 'text-gray-600'} text-center leading-tight">${subText}</span>
            </div>
        `;
    }
}
