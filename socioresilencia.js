...
function iniciarPanelSocio(user) {
    // 1. Mostrar su número como código extrayéndolo directamente de la base de datos
    firebase.database().ref(`users/${user.uid}/phone`).once('value').then(snapshot => {
        const telDisplay = document.getElementById('mi-codigo-socio');
        if (telDisplay && snapshot.exists()) {
            telDisplay.textContent = snapshot.val(); // Trae el número exacto y limpio
        }
    }).catch(e => console.error("Error cargando teléfono: ", e));

    // 2. Escuchar el saldo de sus comisiones en tiempo real
    ...
    // 3. INYECTAR LA INTERFAZ DE AMIGOS VINCULADOS DESDE JS (Para no tocar el HTML)
    const codigoSocioContainer = document.getElementById('mi-codigo-socio');
    if (codigoSocioContainer && !document.getElementById('contenedor-mis-referidos')) {
        // Encontramos la caja donde está el código y agregamos la nueva sección justo debajo
        const padreCaja = codigoSocioContainer.parentElement;
        const htmlListaAmigos = `
            <div id="contenedor-mis-referidos" class="bg-white p-3 rounded-lg border border-gray-200 mt-3 text-left shadow-inner max-h-32 overflow-y-auto custom-scrollbar">
                <p class="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2"><i class="fas fa-users text-indigo-400 mr-1"></i> Tus Amigos Vinculados</p>
                <ul id="lista-mis-referidos" class="space-y-1">
                    <li class="text-[10px] text-gray-400 italic text-center py-2">Cargando...</li>
                </ul>
            </div>
        `;
        padreCaja.insertAdjacentHTML('afterend', htmlListaAmigos);
    }

    // 4. ESCUCHAR LA BASE DE DATOS Y LLENAR LA LISTA DE AMIGOS
    firebase.database().ref('users').orderByChild('referido_por').equalTo(user.uid).on('value', snapshot => {
        const listaUI = document.getElementById('lista-mis-referidos');
        if (!listaUI) return;
        
        listaUI.innerHTML = '';
        
        if (!snapshot.exists()) {
            listaUI.innerHTML = '<li class="text-[10px] text-gray-400 italic text-center py-2">Aún no tienes invitados.<br>¡Comparte tu número!</li>';
            return;
        }

        snapshot.forEach(child => {
            const amigo = child.val();
            // Sacar solo el primer nombre
            const nombreStr = amigo.name ? amigo.name.split(' ') : 'Amigo';
            // Ocultar el centro del teléfono por privacidad (Ej: 0412***4567)
            let tlfMask = '***';
            if (amigo.phone && amigo.phone.length >= 10) {
                tlfMask = amigo.phone.substring(0,4) + '***' + amigo.phone.substring(amigo.phone.length - 4);
            }
            
            listaUI.innerHTML += `
                <li class="flex justify-between items-center bg-gray-50 p-1.5 rounded border border-gray-100 mb-1">
                    <span class="text-[11px] font-bold text-gray-700"><i class="fas fa-user-circle text-indigo-300 mr-1"></i> ${nombreStr}</span>
                    <span class="text-[10px] text-gray-400 font-mono">${tlfMask}</span>
                </li>
            `;
        });
    });
