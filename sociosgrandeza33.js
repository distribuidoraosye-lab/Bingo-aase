...
function iniciarPanelSocio(user) {
    // Mostrar su número como código (Extrayendo estrictamente solo los números)
    const telDisplay = document.getElementById('mi-codigo-socio');
    if (telDisplay) {
        let numeroLimpio = user.email.split('@').replace(/[^0-9]/g, '');
        telDisplay.textContent = numeroLimpio;
    }
