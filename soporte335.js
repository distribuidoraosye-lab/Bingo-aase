...
// 4. EL SECUESTRADOR SUPREMO DE WHATSAPP (Atrapa Error 5, Retiros viejos y nuevos)
function interceptarWhatsApp() {
    // 4.1 Atrapar enlaces normales y el botón manual
    ...
    // 4.2 Atrapar botones generados dinámicamente (Ej. SweetAlert de Retiros con contador)
    ...
    botonesDinamicos.forEach(btn => {
        ...
        if (!btn.dataset.secuestrado && (onclk.includes('wa.me') || onclk.includes('api.whatsapp') || (htmlLower.includes('whatsapp') && (htmlLower.includes('retiro') || htmlLower.includes('soporte') || htmlLower.includes('contactar'))))) {
            ...
            // Si es un enlace dentro del modal, anular href
            if(btn.tagName === 'A') {
...
// 4.3 Atrapar redirecciones de sistema invisibles (Interceptar window.open)
if (!window.whatsappIntercepted) {
    ...
    window.open = function(url, target, features) {
        if (url && (url.includes('wa.me') || url.includes('api.whatsapp.com') || url.includes('whatsapp.com'))) {
            abrirChatSoporte();
            return null; // Evita que se abra la pestaña
        }
...
window.addEventListener('load', () => {
    ...
    // El Ojo de Halcón: MutationObserver (Atrapa los modales del contador de retiros en cuanto se pintan en pantalla)
