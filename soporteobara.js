...
// 3. LÓGICA DE CHAT Y FIREBASE (Con control estricto de caché y reinicio de botones)
let historialCargado = false;
let isFirstLoad = true;
let autoRespuestaEnviada = false;
let currentChatListenerUid = null; 

// FUNCIÓN NUEVA: Inyecta los botones al final del chat
function inyectarBotonesPreseleccion() {
    const area = document.getElementById('chat-mensajes-area');
    if (!area) return;
    
    // Si ya existen, los borramos para que no se dupliquen
    const existing = document.getElementById('opciones-preseleccion');
    if (existing) existing.remove();

    area.insertAdjacentHTML('beforeend', `
        <div id="opciones-preseleccion" class="flex flex-col gap-2 mt-2 pt-2 mb-2 w-11/12 transition-all">
            <button onclick="seleccionarOpcionChat('💳 Reportar un pago manual')" class="bg-white border border-blue-300 text-blue-700 text-xs font-bold p-2 rounded-lg text-left hover:bg-blue-50 transition shadow-md">💳 Reportar un pago manual</button>
            <button onclick="seleccionarOpcionChat('💸 Duda sobre retiros')" class="bg-white border border-blue-300 text-blue-700 text-xs font-bold p-2 rounded-lg text-left hover:bg-blue-50 transition shadow-md">💸 Duda sobre retiros</button>
            <button onclick="seleccionarOpcionChat('🎁 Pregunta sobre Racha/Bonos')" class="bg-white border border-blue-300 text-blue-700 text-xs font-bold p-2 rounded-lg text-left hover:bg-blue-50 transition shadow-md">🎁 Pregunta sobre Racha/Bonos</button>
            <button onclick="seleccionarOpcionChat('💬 Hablar con un operador')" class="bg-white border border-blue-300 text-blue-700 text-xs font-bold p-2 rounded-lg text-left hover:bg-blue-50 transition shadow-md">💬 Hablar con un operador</button>
        </div>
    `);
    ...
    if(area) {
        area.innerHTML = `
            <div class="bg-white border border-gray-200 p-2 rounded-lg rounded-tl-none w-11/12 shadow-sm">
                <p class="text-xs text-gray-700 font-bold">¡Hola! 🤖 ¿En qué podemos ayudarte el día de hoy?</p>
            </div>
        `;
    }
    inyectarBotonesPreseleccion(); // Pintamos los botones frescos
...
window.borrarHistorialChat = function() {
    if(confirm("¿Deseas vaciar este chat para iniciar una nueva consulta?")) {
        ...
        if(user) {
            firebase.database().ref('soporte_chats/' + user.uid).remove(); 
        }
        limpiarChatLocal(); 
    }
};
...
window.seleccionarOpcionChat = function(textoOpcion) {
    ...
    if(preselecciones) preselecciones.remove(); // Borra los botones al tocar
...
window.enviarMensajeDesdeUsuario = function() {
    ...
    const preselecciones = document.getElementById('opciones-preseleccion');
    if(preselecciones) preselecciones.remove(); // Borra los botones al enviar
...
function cargarHistorialChat() {
    ...
    firebase.database().ref('soporte_chats/' + user.uid).limitToLast(20).on('child_added', (snapshot) => {
        ...
        if (msg.remitente === 'admin') {
            ...
            // MAGIA AQUÍ: Si el mensaje de Admin es el de cierre, reiniciamos el bot
            if (msg.texto.includes("SOLUCIONADO")) {
                autoRespuestaEnviada = false; // Permite que el bot vuelva a responder horarios
                inyectarBotonesPreseleccion(); // Vuelve a poner los botones al final del chat
            }

        } else if (msg.remitente === 'usuario') {
            const preselecciones = document.getElementById('opciones-preseleccion');
            if(preselecciones) preselecciones.remove();

            autoRespuestaEnviada = true; // Bloquea el bot porque el usuario ya habló
    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
            ...
            if (currentChatListenerUid !== user.uid) {
                ...
                limpiarChatLocal(); 
                currentChatListenerUid = user.uid; 
            }
