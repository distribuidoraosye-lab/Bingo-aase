/* ============================================================
   BINGO TEXIER - MÓDULO RACHA (V10 - CÓDIGO CERO ERRORES)
   Lector universal de milisegundos y autocompletado de precios.
   ============================================================ */

const dbRacha = firebase.database();
let configTasa = 500; // Tasa base indestructible

// MEMORIA DE SESIÓN PROTEGIDA
let rachaState = {
    deposito: 0,
    bingo: 0,
    torneo: 0,
...
// 2. EL TRADUCTOR UNIVERSAL DE FECHAS (Adiós al error de caer a 0)
function isRecordFromToday(dbTimeValue, todayStr) {
    if (!dbTimeValue) return false;
    
    // Si ya viene en formato DD-MM-YYYY perfecto
    if (typeof dbTimeValue === 'string' && dbTimeValue.includes(todayStr)) return true;
    
    // Si viene en MILISEGUNDOS gigantes (Ej: 1776180505112)
    let ts = parseInt(dbTimeValue);
    if (!isNaN(ts) && ts > 1000000000000) {
        let d = new Date(ts);
        if(d.getHours() >= 20) d.setDate(d.getDate() + 1);
        let convertedStr = `${String(d.getDate()).padStart(2,'0')}-${String(d.getMonth()+1).padStart(2,'0')}-${d.getFullYear()}`;
        return convertedStr === todayStr;
    }
    return false;
}
...
// 5. LOS ESPÍAS DE BASE DE DATOS
function initStreakMonitors(uid) {
    const todayStr = getSystemDateStr();

    // ESPÍA 1: DEPÓSITOS (Busca en amount, amt y monto. Lee milisegundos)
    ...
    const cbDep = refDep.on('value', snap => {
        ...
        if(snap.exists()) {
            snap.forEach(child => {
                let data = child.val();
                let timeVal = data.timestamp || data.date || data.fecha || data.request_timestamp || data.time;
                if (isRecordFromToday(timeVal, todayStr)) {
    ...
    // ESPÍA 2: BINGO (Si no hay precio, inyecta la Tasa Base)
    ...
    const cbBin = refBin.on('value', snap => {
        ...
        if(snap.exists()) {
            snap.forEach(child => {
                let data = child.val();
                let timeVal = data.date || data.timestamp || data.time;
                if (isRecordFromToday(timeVal, todayStr) && data.payment_method !== 'GRATIS') {
                    let precio = parseFloat(data.precio_carton);
                    // Si el precio no existe o es texto inválido, sumamos configTasa (500)
                    sumaBin += !isNaN(precio) ? precio : configTasa;
                }
            });
        }
    ...
    // ESPÍA 3: TORNEO (Con inyección de Tasa Base)
    ...
    const cbTor = refTor.on('value', snap => {
        ...
        if(snap.exists()) {
            snap.forEach(child => {
                let data = child.val();
                let precio = parseFloat(data.amount || data.monto);
                sumaTor += !isNaN(precio) ? precio : configTasa;
            });
        }
...
// 6. PROCESAMIENTO MATEMÁTICO Y DE UI
function procesarRacha(uid, todayStr) {
    ...
    let cartonesDepositados = depActual / tasaActiva;
    let cartonesJugados = jugActual / tasaActiva;

    // Tolerancia Matemática: 0.95 equivale a "jugó 1 cartón completo"
    let nivelHoy = 0;
    ...
    dbRacha.ref(`users/${uid}/racha_data`).once('value', s => {
        ...
        // Quitar penalización si salvó el día
        ...
        if (!isPenalized && salvoElDia) {
            ...
            // Marcarlo activo para que no pierda la racha por error
        ...
        // Actualizar visual de la pantalla
        document.getElementById('today-deposit-text').textContent = "Bs " + depActual.toLocaleString('es-VE', {minimumFractionDigits: 2});
        document.getElementById('today-played-text').textContent = "Bs " + jugActual.toLocaleString('es-VE', {minimumFractionDigits: 2});
        ...
        // Mostrar advertencia si depositó pero no ha jugado el 95%
...
// 7. PROTECCIÓN DE RETIROS (Con tolerancia del 5%)
function protectWithdrawals(uid) {
    ...
    form.onsubmit = async (e) => {
        ...
        // Si depositó y jugó menos del 95% de ese depósito
