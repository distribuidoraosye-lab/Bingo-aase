...
// Inyección de la API de YouTube dinámica
...
let aprobadosGratis = {};
let currentDrawType = 'ESTELAR'; // 'ESTELAR' o 'GRATIS'
let nextDrawTarget = 'GRATIS';   // Para el contador
...
function updateCountdown() {
    ...
    // El reloj decide si el próximo sorteo es Gratis o Estelar (18:00 y 21:00)
    if (now.getHours() < 18) {
...
// --- ACTUALIZADOR DINÁMICO DE PREMIOS ---
...
function evaluateActiveDraw() {
    ...
    // Si el sorteo gratis es más nuevo, mostramos el gratis
    if (tsG > tsE) {
        ...
        if(activeData.cartones_snapshot) {
            Object.values(activeData.cartones_snapshot).forEach(c => { 
                if(c.numbers) { 
                    ...
                    // MARCAMOS LOS CARTONES ACTIVOS CON EL TIPO DE SORTEO QUE ESTÁN JUGANDO
...
function updateNextCards() {
    ...
    // Dependiendo de lo que marque el reloj (18h o 21h), buscamos en esa base de datos
    let sourceObj = nextDrawTarget === 'GRATIS' ? aprobadosGratis : aprobadosEstelar;
    ...
    Object.values(sourceObj).forEach(c => { 
        if(c.numbers && c.date === dateStr) { 
            c.numbers = c.numbers.map(Number); 
            c.drawType = nextDrawTarget; // Para saber qué diseño aplicarle
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(blocker) blocker.addEventListener('click', () => togglePause());
});
...
function evaluateSystemState() {
    ...
    if (viewMode === 'NEXT') {
        ...
        if (videoId && btnToggleCountdown) {
            ...
            btnToggleCountdown.onclick = startVideoPlayback;
        }
        ...
        if (shouldDefaultToNext && isPlayingVideo) {
            if (btnToggle) {
                ...
                btnToggle.onclick = backToWaiting;
            }
        } else {
            if (btnToggle) btnToggle.classList.add('hidden');
        }
...
function initData() {
    ['usuarios','users'].forEach(p => onValue(ref(db, p), s => { 
        if(s.exists()) { 
            usersMap={...usersMap, ...s.val()}; 
            renderRanking(); 
            if(currentUser) evaluateSystemState(); 
        } 
    }));
    
    onValue(ref(db, 'sorteo_en_juego'), s => {
        dataLoaded.sorteoEstelar = true;
        dataEstelar = s.val() || null;
        evaluateActiveDraw();
    });
    
    onValue(ref(db, 'sorteo_en_juego_gratis'), s => {
        dataLoaded.sorteoGratis = true;
        dataGratis = s.val() || null;
        evaluateActiveDraw();
    });
    
    onValue(ref(db, 'bingo_aprobados_estelar'), s => {
        dataLoaded.aprobadosEstelar = true;
        aprobadosEstelar = s.val() || {};
        updateNextCards();
    });

    onValue(ref(db, 'bingo_aprobados_gratis'), s => {
        dataLoaded.aprobadosGratis = true;
...
function renderRanking() {
    ...
    if(!cardsToRank.length) { 
        ...
        document.getElementById('pavoso-name').textContent = '...';
        document.getElementById('pavoso-score').textContent = '0';
    ...
    if (pavosoContainer) {
        if (currentDrawType === 'ESTELAR') {
            ...
            pavosoContainer.classList.add('hidden');
        }
    }
