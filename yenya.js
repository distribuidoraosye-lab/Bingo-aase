/* ============================================================
   BINGO TEXIER - MÓDULO INICIORACHABUENA (V11 - LÓGICA AUTÓNOMA)
   Independiente del Panel Admin. Basado en hora real de compra.
   ============================================================ */
...
// 2. MOTOR DE TIEMPO (A PRUEBA DE FALLOS Y HORARIOS ADMINISTRATIVOS)

// Obtiene la hora en la que se compró el cartón leyendo el ID de Firebase
function getTimestampFromPushKey(pushKey) {
    if (!pushKey || pushKey.length < 8) return 0;
    const PUSH_CHARS = '-0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz';
    let timestamp = 0;
    for (let i = 0; i < 8; i++) {
        const n = PUSH_CHARS.indexOf(pushKey.charAt(i));
        if (n === -1) return 0;
        timestamp = timestamp * 64 + n;
    }
    return timestamp;
}

// Verifica si un ticket pertenece al ciclo de Racha activo en este momento
function isTicketInActiveCycle(ticketMs, cutoffHour) {
    if (!ticketMs) return false;
    const now = Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0);
    const d = new Date(now);
    
    // Frontera de hoy (ej. Hoy a las 8:00 PM)
    const cutoffToday = new Date(d.getFullYear(), d.getMonth(), d.getDate(), cutoffHour, 0, 0, 0).getTime();
    
    let startCycle, endCycle;
    if (now >= cutoffToday) {
        startCycle = cutoffToday;
        endCycle = cutoffToday + (24 * 3600000); // Hasta mañana a las 8:00 PM
    } else {
        startCycle = cutoffToday - (24 * 3600000); // Desde ayer a las 8:00 PM
        endCycle = cutoffToday;
    }
    
    return ticketMs >= startCycle && ticketMs < endCycle;
}

function getMasterDateStr(offsetHours) {
...
// 3. OBSERVADORES DE BASES DE DATOS
function initStreakMonitor(uid) {
    
    // A. BINGO (Corte a las 20:00 - 8 PM)
    dbRacha.ref('bingo_aprobados_estelar').orderByChild('uid').equalTo(uid).on('value', s => {
        let countB = 0;
        if (s.exists()) {
            s.forEach(c => {
                if (c.val().payment_method === 'GRATIS') return;
                const ticketTime = getTimestampFromPushKey(c.key);
                if (isTicketInActiveCycle(ticketTime, 20)) {
                    countB++;
                }
            });
        }
    ...
    // B. TORNEO (Corte a las 18:00 - 6 PM)
    setInterval(() => {
        const dReal = new Date(Date.now() + (typeof serverTimeOffset !== 'undefined' ? serverTimeOffset : 0));
        const yest = new Date(dReal); yest.setDate(yest.getDate() - 1);
        const tom = new Date(dReal); tom.setDate(tom.getDate() + 1);
        const formatD = (d) => `${String(d.getDate()).padStart(2,'0')}-${String(d.getMonth()+1).padStart(2,'0')}-${d.getFullYear()}`;
        
        // Buscamos en las carpetas de ayer, hoy y mañana para no perder ningún ticket
        const folders = [formatD(yest), formatD(dReal), formatD(tom)];
        let countT = 0;
        
        let checks = folders.map(folder => dbRacha.ref(`bets_torneo_express/${folder}/${uid}`).once('value').then(s => {
            if (s.exists()) {
                s.forEach(c => {
                    const ticketTime = c.val().timestamp || getTimestampFromPushKey(c.key);
                    if (isTicketInActiveCycle(ticketTime, 18)) {
                        countT++;
                    }
                });
            }
...
function updateStreakUI(uid) {
    ...
    // Usamos el corte de Bingo (8 PM) para definir el día maestro de la racha
    const masterDate = getMasterDateStr(20);
